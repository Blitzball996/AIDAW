...
/** Check if a token looks like a sigil (@plugin.param). */
bool isSigilToken(const juce::String& v) {
...
AutoShape parseShape(const juce::String& s, bool& ok) {
    ok = true;
    if (s.equalsIgnoreCase("sin"))    return AutoShape::Sin;
    if (s.equalsIgnoreCase("tri"))    return AutoShape::Tri;
    if (s.equalsIgnoreCase("saw"))    return AutoShape::Saw;
    if (s.equalsIgnoreCase("square")) return AutoShape::Square;
    if (s.equalsIgnoreCase("exp"))    return AutoShape::Exp;
    if (s.equalsIgnoreCase("log"))    return AutoShape::Log;
    if (s.equalsIgnoreCase("line"))   return AutoShape::Line;
    ok = false;
