...
/**
 * Enumerates Lua scripts stored under the per-user AIDAW app data directory.
 *
 * Default layout:
 *   ~/Library/AIDAW/Scripts/Controllers/      (macOS)
 *   %APPDATA%/AIDAW/Scripts/Controllers/      (Windows)
 *   ~/.config/AIDAW/Scripts/Controllers/      (Linux)
 *
 * Per-project storage is deferred to a follow-up; v1 is global only.
 */
class LuaScriptStore {
  public:
    /** Default constructor uses the per-user AIDAW scripts directory. */
    LuaScriptStore();
     ...
     *  invalid juce::File if not found. */
