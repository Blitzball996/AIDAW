...
aidaw::ParameterInfo placeholderForInactive(const FaustParamSlot& slot) {
    ...
    info.scale = aidaw::ParameterScale::Linear;
    info.modulatable = false;
    return info;
}

aidaw::ParameterInfo continuousInfo(const FaustParamSlot& slot) {
    aidaw::ParameterInfo info;
    info.paramIndex = slot.index;
    ...
    info.scale = slot.logScale ? aidaw::ParameterScale::Logarithmic : aidaw::ParameterScale::Linear;
    if (slot.label.equalsIgnoreCase("Mix") && std::abs(slot.minValue) < 1.0e-6f &&
        std::abs(slot.maxValue - 1.0f) < 1.0e-6f)
        info.displayFormat = aidaw::DisplayFormat::Percent;
...
aidaw::ParameterInfo booleanInfo(const FaustParamSlot& slot) {
    aidaw::ParameterInfo info;
    info.paramIndex = slot.index;
    info.name = slot.label;
    info.unit = slot.unit;
    ...
    info.scale = aidaw::ParameterScale::Boolean;
...
aidaw::ParameterInfo discreteInfo(const FaustParamSlot& slot) {
    aidaw::ParameterInfo info;
    info.paramIndex = slot.index;
    info.name = slot.label;
    info.unit = slot.unit;
    info.scale = aidaw::ParameterScale::Discrete;
...
aidaw::ParameterInfo paramInfoFromSlot(const FaustParamSlot& slot) {
