...
void MidiChordEnginePlugin::runDetection() {
    ...
    std::vector<aidaw::music::ChordNote> heldNotes;
    ...
    aidaw::music::ChordEngine::finalizeChord(detected);
    ...
    if (chordChanged) {
        ...
        // Update cached scales from chord history pitch classes
        {
            ...
            if (pitchClasses.size() >= 3) {
                ...
                auto scored = aidaw::music::detectScalesFromPitchClasses(
                    pitchClasses, aidaw::music::getAllScalesWithChordsCached(), preferredRoot);
                cachedScales_.clear();
...
aidaw::music::Chord MidiChordEnginePlugin::getCurrentChord() const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    return currentChord_;
}

std::vector<aidaw::music::Chord> MidiChordEnginePlugin::getRecentChords() const {
    std::lock_guard<std::mutex> lock(stateMutex_);
...
std::vector<aidaw::music::ChordEngine::SuggestionItem> MidiChordEnginePlugin::getSuggestions()
    const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    return cachedSuggestions_;
}

std::vector<aidaw::music::ScaleWithChords> MidiChordEnginePlugin::getDetectedScales(
