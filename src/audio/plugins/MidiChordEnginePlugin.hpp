...
class MidiChordEnginePlugin : public te::Plugin, private juce::Timer {
  public:
    ...
    aidaw::music::Chord getCurrentChord() const;

    /** Recent chord history (most recent last). */
    std::vector<aidaw::music::Chord> getRecentChords() const;
    ...
    std::vector<aidaw::music::ChordEngine::SuggestionItem> getSuggestions() const;

    /** Detected scales sorted by match score (top N). Each entry has the scale and its chords. */
    std::vector<aidaw::music::ScaleWithChords> getDetectedScales(int maxResults = 5) const;

    /** Suggestion parameters — UI can tweak these. */
    aidaw::music::ChordEngine::SuggestionParams& getSuggestionParams() {
        return suggestionParams_;
    }
    const aidaw::music::ChordEngine::SuggestionParams& getSuggestionParams() const {
        return suggestionParams_;
    }
    ...
    struct AIProgression {
        ...
        std::vector<aidaw::music::Chord> chords;
    };
  ...
  private:
    ...
    aidaw::music::ChordSuggestionEngine suggestionEngine_;
    aidaw::music::KeyModeHistogram keyHistogram_;
    aidaw::music::ChordEngine::SuggestionParams suggestionParams_;

    aidaw::music::Chord currentChord_;
    juce::String lastDetectedChordName_;  // persists after note release
    std::vector<aidaw::music::Chord> chordHistory_;
    static constexpr size_t MAX_CHORD_HISTORY = 64;

    std::optional<std::pair<juce::String, juce::String>> cachedKeyMode_;
    std::vector<aidaw::music::ChordEngine::SuggestionItem> cachedSuggestions_;
    std::vector<aidaw::music::ScaleWithChords> cachedScales_;
