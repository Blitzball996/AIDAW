...
aidaw::ParameterInfo parameterInfoForSlot(const CompiledHostSlotInfo& s) {
    aidaw::ParameterInfo info;
...
void MagdaCompressorCompiledPlugin::buildHostParameters() {
    ...
    hostSlotInfo_[kEngineSlot].scale = aidaw::ParameterScale::Discrete;
    ...
    hostSlotInfo_[kThresholdSlot] = {.name = "Threshold",
                                     .unit = "dB",
                                     .scale = aidaw::ParameterScale::Linear,
                                     .minValue = -60.0f,
                                     .maxValue = 0.0f,
                                     .defaultValue = -18.0f};
    hostSlotInfo_[kRatioSlot] = {.name = "Ratio",
                                 .scale = aidaw::ParameterScale::Logarithmic,
                                 .minValue = 1.0f,
                                 .maxValue = 50.0f,
                                 .defaultValue = 4.0f,
                                 .scaleAnchor = 4.0f};
    ...
    hostSlotInfo_[kDetectorSlot].scale = aidaw::ParameterScale::Discrete;
    ...
    hostSlotInfo_[kStyleSlot].scale = aidaw::ParameterScale::Discrete;
    ...
    hostSlotInfo_[kAutogainSlot].scale = aidaw::ParameterScale::Discrete;
    ...
    for (int i = 0; i < kHostSlotCount; ++i) {
        ...
        const float defaultNormalized =
            aidaw::ParameterUtils::realToNormalized(slot.defaultValue, info);
        hostCached_[i].referTo(state, identifier, undoManager, defaultNormalized);

        auto param = addParam(
            id, slot.name, normalisedRange,
            [info](float normalized) {
                const float real = aidaw::ParameterUtils::normalizedToReal(normalized, info);
                return aidaw::ParameterUtils::formatValue(real, info);
            },
            [info](const juce::String& text) {
                auto parsed = aidaw::ParameterUtils::parseValue(text, info);
                if (parsed)
                    return aidaw::ParameterUtils::realToNormalized(*parsed, info);
                return 0.0f;
            });
...
void MagdaCompressorCompiledPlugin::applyToBuffer(const te::PluginRenderContext& fc) {
    ...
    auto realForSlot = [&](int slot) -> float {
        ...
        return aidaw::ParameterUtils::normalizedToReal(norm, info);
    };
...
float MagdaCompressorCompiledPlugin::displayValueToNativeValue(int slotIndex,
    ...
    return aidaw::ParameterUtils::realToNormalized(displayValue, info);
}
...
float MagdaCompressorCompiledPlugin::nativeValueToDisplayValue(int slotIndex,
    ...
    if (slotIndex < 0 || slotIndex >= kHostSlotCount)
        return nativeValue;
    const auto info = parameterInfoForSlot(hostSlotInfo_[static_cast<size_t>(slotIndex)]);
    return aidaw::ParameterUtils::normalizedToReal(nativeValue, info);
}
