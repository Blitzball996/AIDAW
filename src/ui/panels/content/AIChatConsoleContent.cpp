...
void AIChatConsoleContent::RequestThread::run() {
    ...
    std::vector<aidaw::Instruction> musicInstructions;     // IR from music agent
    std::string musicDescription;                          // description from DSL music agent
    std::vector<aidaw::AutoInstruction> autoInstructions;  // IR from automation agent
    std::string error;
    ...
    if (intent == "BOTH") {
        ...
        std::future<aidaw::CommandAgent::GenerateResult> commandFuture;
        std::future<aidaw::MusicAgent::GenerateResult> musicFuture;
        ...
        [safeThis, dsl = std::move(dslCode), musicIR = std::move(musicInstructions),
         ...
         error = std::move(error), anchor, routerMs, agentMs, totalMs]() {
            ...
            if (!error.empty() && !hasContent) {
                ...
                aidaw::ClipManager::BatchScope batchScope;

                // Execute DSL from command agent
                int commandClipId = -1;
                if (!dsl.empty()) {
                    aidaw::dsl::Interpreter interpreter(*safeThis->magdaApi_);
                ...
                if (!musicIR.empty()) {
                    ...
                    aidaw::InstructionExecutor executor(*safeThis->magdaApi_);
                    ...
                    if (executor.execute(musicIR)) {
                        ...
                        if (!musicDesc.empty() && executor.getCurrentClipId() >= 0) {
                            ...
                            aidaw::ClipManager::getInstance().setClipName(
                ...
                if (!autoIR.empty()) {
                    aidaw::AutomationExecutor autoExec(*safeThis->magdaApi_);
...
AIChatConsoleContent::AIChatConsoleContent() {
    ...
    serverToggleButton_ = std::make_unique<aidaw::SvgButton>(
        "ModelToggle", BinaryData::server_play_svg, BinaryData::server_play_svgSize);
    serverToggleButton_->onClick = [this]() {
        auto& mgr = aidaw::LlamaModelManager::getInstance();
        if (mgr.isLoaded()) {
            mgr.unloadModel();
            updateConfigStatus();
        } else {
            auto& config = aidaw::Config::getInstance();
            ...
            aidaw::LlamaModelManager::Config cfg;
            ...
            std::thread([this, cfg]() {
                bool ok = aidaw::LlamaModelManager::getInstance().loadModel(cfg);
    ...
    // selection event fires.
    {
        ...
        sm.addListener(this);
        if (auto clipId = sm.getSelectedClip(); clipId != aidaw::INVALID_CLIP_ID) {
            clipSelectionChanged(clipId);
        } else if (auto trackId = sm.getSelectedTrack(); trackId != aidaw::INVALID_TRACK_ID) {
            trackSelectionChanged(trackId);
        }
    }

    // Register for project lifecycle events
    aidaw::ProjectManager::getInstance().addListener(this);

    // Register for config changes (e.g. preset changed in settings dialog)
    aidaw::Config::getInstance().addListener(this);
    ...
    if (auto* engine = dynamic_cast<aidaw::TracktionEngineWrapper*>(
        ...
        ownedApi_ = std::make_unique<aidaw::MagdaApiLive>();
        magdaApi_ = ownedApi_.get();
    }

    // Create agents
    agent_ = std::make_unique<aidaw::DAWAgent>(*magdaApi_);  // legacy DSL REPL
    agent_->start();
    routerAgent_ = std::make_unique<aidaw::RouterAgent>();
    commandAgent_ = std::make_unique<aidaw::CommandAgent>(*magdaApi_);
    musicAgent_ = std::make_unique<aidaw::MusicAgent>();
    automationAgent_ = std::make_unique<aidaw::AutomationAgent>(*magdaApi_);
    controllerAgent_ = std::make_unique<aidaw::ControllerProfileAgent>();
    fourOscAgent_ = std::make_unique<aidaw::FourOscAgent>();
}
...
AIChatConsoleContent::~AIChatConsoleContent() {
    ...
    aidaw::Config::getInstance().removeListener(this);
    aidaw::ProjectManager::getInstance().removeListener(this);
...
void AIChatConsoleContent::sendMessage(const juce::String& text) {
    ...
    if (text.trimStart().startsWith("/dsl ")) {
        ...
        aidaw::dsl::Interpreter interpreter(*magdaApi_);
...
void AIChatConsoleContent::setupTabButtons() {
    aiTabButton_ =
        std::make_unique<aidaw::SvgButton>("AITab", BinaryData::ai_svg, BinaryData::ai_svgSize);
    ...
    dslTabButton_ = std::make_unique<aidaw::SvgButton>("DSLTab", BinaryData::script_svg,
...
void AIChatConsoleContent::projectOpened(const aidaw::ProjectInfo& /*info*/) {
    // Reset chat history
    chatHistory_.setText(juce::String::charToString(0x25C6) + " MAGDA\n\n");
...
void AIChatConsoleContent::selectionTypeChanged(aidaw::SelectionType newType) {
    if (newType == aidaw::SelectionType::None) {
        contextText_.clear();
...
void AIChatConsoleContent::trackSelectionChanged(aidaw::TrackId trackId) {
    auto* track = aidaw::TrackManager::getInstance().getTrack(trackId);
    contextText_ = track != nullptr ? track->name : juce::String(trackId);
    contextIcon_ = ContextIcon::Track;
    updateContextBar();
}

void AIChatConsoleContent::clipSelectionChanged(aidaw::ClipId clipId) {
    auto* clip = aidaw::ClipManager::getInstance().getClip(clipId);
    if (clip != nullptr) {
        auto* track = aidaw::TrackManager::getInstance().getTrack(clip->trackId);
...
void AIChatConsoleContent::chainNodeSelectionChanged(const aidaw::ChainNodePath& path) {
    auto* track = aidaw::TrackManager::getInstance().getTrack(path.trackId);
    ...
    if (deviceId != aidaw::INVALID_DEVICE_ID) {
        auto* device = aidaw::TrackManager::getInstance().getDevice(path.trackId, deviceId);
        if (device != nullptr)
...
void AIChatConsoleContent::updateConfigStatus() {
    ...
    auto musicCfg = config.getAgentLLMConfig(aidaw::role::MUSIC);
...
bool AIChatConsoleContent::isLocalPreset() const {
    auto& config = aidaw::Config::getInstance();
    auto preset = config.getAIPreset();
    auto commandCfg = config.getAgentLLMConfig(aidaw::role::COMMAND);
    return commandCfg.provider == aidaw::provider::LLAMA_LOCAL ||
           preset == aidaw::preset::LOCAL_EMBEDDED || preset == "local";
}
...
void AIChatConsoleContent::mouseUp(const juce::MouseEvent& event) {
    if (event.originalComponent == &contextLabel_ ||
        ...
        aidaw::dsl::Interpreter::setContextEnabled(contextEnabled_);
        updateContextBar();
    }
}
...
void AIChatConsoleContent::buildAliasList() {
    ...
    for (const auto& entry : aidaw::getInternalPlugins()) {
    ...
    auto aliasFile = aidaw::paths::pluginAliasesFile();
...
std::vector<AIChatConsoleContent::ParamAliasEntry> AIChatConsoleContent::collectParamAliases(
    ...
    auto& registry = aidaw::AliasRegistry::getInstance();
    const juce::String prefix = pluginAlias + ".";
    std::set<juce::String> seenCanonical;
    auto walk = [&](aidaw::AliasLayer layer) {
    ...
    walk(aidaw::AliasLayer::UserProject);
    walk(aidaw::AliasLayer::UserGlobal);
    walk(aidaw::AliasLayer::Curated);
    walk(aidaw::AliasLayer::AutoGen);
...
void AIChatConsoleContent::buildSlashCommands() {
    ...
    slashRegistry_ = std::make_unique<aidaw::daw::ui::SlashCommandRegistry>(
...
juce::String findUniqueProfileId(const juce::String& baseId) {
    auto& reg = aidaw::ControllerProfileRegistry::getInstance();
...
void AIChatConsoleContent::finishControllerGeneration(bool success, const juce::String& errorOrJson,
    ...
    auto userDir = aidaw::ControllerProfileRegistry::userControllersDirectory();
    ...
    auto writeAndPromptPort = [safeThis, userDir](juce::String finalJson, juce::String finalId,
        ...
        if (!safeThis)
            return;
        auto destFile =
            userDir.getChildFile(aidaw::ControllerProfileRegistry::filenameForProfileId(finalId));
        ...
        aidaw::ControllerProfileRegistry::getInstance().load();
        ...
        if (auto profileOpt = aidaw::ControllerProfileRegistry::getInstance().findById(finalId)) {
        ...
        menu.showMenuAsync(
            ...
            [safeThis, finalId, displayName, ports](int result) {
                ...
                auto profileOpt = aidaw::ControllerProfileRegistry::getInstance().findById(finalId);
                ...
                auto& controllerReg = aidaw::ControllerRegistry::getInstance();
                auto& bindingReg = aidaw::BindingRegistry::getInstance();
                for (const auto& existing : controllerReg.all()) {
                    if (existing.inputPort == dev.identifier) {
                        bindingReg.removeAllForController(aidaw::BindingScope::Global, existing.id);
                        bindingReg.removeAllForController(aidaw::BindingScope::Project,
                                                          existing.id);
                    }
                }

                auto mat = aidaw::materialiseControllerFromProfile(*profileOpt, dev.identifier, {},
                                                                   dev.name);

                controllerReg.add(mat.controller);
                for (const auto& b : mat.bindings)
                    bindingReg.add(aidaw::BindingScope::Global, b);

                auto& cfg = aidaw::Config::getInstance();
                cfg.setControllers(aidaw::ControllerRegistry::getInstance().saveToConfig());
                cfg.setGlobalBindings(aidaw::BindingRegistry::getInstance().saveGlobal());
                cfg.save();
    ...
    if (aidaw::ControllerProfileRegistry::getInstance().findById(profileId).has_value()) {
        juce::PopupMenu menu;
...
static juce::String prettyPrintPreset(const aidaw::FourOscAgent::Preset& preset) {
...
// chat. Delegates the actual write to aidaw::applyFourOscPresetToPath.
static juce::String applyFourOscPresetToFocusedDevice(const aidaw::FourOscAgent::Preset& preset) {
    auto& sel = aidaw::SelectionManager::getInstance();
    auto& tm = aidaw::TrackManager::getInstance();

    aidaw::ChainNodePath path;
    aidaw::DeviceInfo* device = nullptr;
    ...
    if (device == nullptr) {
        ...
        newDevice.deviceType = aidaw::DeviceType::Instrument;
        newDevice.format = aidaw::PluginFormat::Internal;

        const auto trackId = tm.createTrack(trackName, aidaw::TrackType::Audio);
    ...
    return preamble + aidaw::applyFourOscPresetToPath(preset, path);
}
...
void AIChatConsoleContent::finishPresetGeneration(bool success, const juce::String& errorOrPretty,
    ...
    if (obj == nullptr)
        return;
    aidaw::FourOscAgent::Preset preset;
