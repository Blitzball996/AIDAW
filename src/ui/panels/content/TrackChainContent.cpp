...
namespace {
bool dragObjectToChainNodePath(const juce::DynamicObject& obj, aidaw::ChainNodePath& path) {
    path = {};
    const auto type = obj.getProperty("type").toString();
    if (type != "chainElement" && type != "chainElements")
        return false;

    path.trackId = static_cast<aidaw::TrackId>(static_cast<int>(obj.getProperty("trackId")));
    path.topLevelDeviceId =
        static_cast<aidaw::DeviceId>(static_cast<int>(obj.getProperty("topLevelDeviceId")));
    ...
    for (int i = 0; i < stepTypes.size(); ++i) {
        ...
        if (typeValue < static_cast<int>(aidaw::ChainStepType::Rack) ||
            typeValue > static_cast<int>(aidaw::ChainStepType::Device))
            return false;
        path.steps.push_back(
            {static_cast<aidaw::ChainStepType>(typeValue), stepIds[i].getIntValue()});
    }
...
bool dragObjectToChainNodePathAt(const juce::DynamicObject& obj, int index,
                                 aidaw::ChainNodePath& path) {
    path = {};
    const auto suffix = juce::String(index);

    path.trackId =
        static_cast<aidaw::TrackId>(static_cast<int>(obj.getProperty("trackId" + suffix)));
    path.topLevelDeviceId = static_cast<aidaw::DeviceId>(
...
std::vector<aidaw::ChainNodePath> dragObjectToChainNodePaths(const juce::DynamicObject& obj) {
    std::vector<aidaw::ChainNodePath> paths;
...
class TrackChainContent::ChainContainer : public juce::Component, public juce::DragAndDropTarget {
  public:
    ...
    bool isInterestedInDragSource(const SourceDetails& details) override {
        if (owner_.selectedTrackId_ == aidaw::INVALID_TRACK_ID) {
            return false;
        }
        ...
        if (auto* obj = details.description.getDynamicObject()) {
            ...
            if (type == "chainElement" || type == "chainElements") {
                ...
                if (!sourcePaths.empty()) {
                    aidaw::ChainNodePath destinationPath;
                    ...
                    juce::MessageManager::callAsync(
                        [safeOwner, sourcePaths, destinationPath, insertIndex]() {
                            auto command = std::make_unique<aidaw::MoveChainElementsCommand>(
                                sourcePaths, destinationPath, insertIndex);
                            auto* moveCommand = command.get();
                            aidaw::UndoManager::getInstance().executeCommand(std::move(command));
            ...
            if (format == "VST3") {
                device.format = aidaw::PluginFormat::VST3;
            } else if (format == "AU") {
                device.format = aidaw::PluginFormat::AU;
            } else if (format == "VST") {
                device.format = aidaw::PluginFormat::VST;
            } else if (format == "Internal") {
                device.format = aidaw::PluginFormat::Internal;
            }

            owner_.scrollToEndAfterNextDeviceChange_ = shouldScrollToEnd;
            owner_.suppressNextImplicitScrollToEnd_ = !shouldScrollToEnd;
            aidaw::TrackManager::getInstance().addDeviceToTrack(owner_.selectedTrackId_, device,
                                                                insertIndex);
...
TrackChainContent::TrackChainContent()
    ...
    globalModsButton_ = std::make_unique<aidaw::SvgButton>("Mod", BinaryData::bare_sine_svg,
    ...
    globalModsButton_->onClick = [this]() {
        ...
        if (auto* track = aidaw::TrackManager::getInstance().getTrack(selectedTrackId_))
    ...
    addRackButton_ =
        std::make_unique<aidaw::SvgButton>("Rack", BinaryData::rack_svg, BinaryData::rack_svgSize);
    ...
    addRackButton_->onClick = [this]() {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            aidaw::TrackManager::getInstance().addRackToTrack(selectedTrackId_);
        }
    };
    ...
    treeViewButton_ =
        std::make_unique<aidaw::SvgButton>("Tree", BinaryData::tree_svg, BinaryData::tree_svgSize);
    ...
    treeViewButton_->onClick = [this]() {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            aidaw::ChainTreeDialog::show(selectedTrackId_);
        }
    };
    ...
    muteButton_.onClick = [this]() {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetTrackMuteCommand>(selectedTrackId_,
    ...
    soloButton_.onClick = [this]() {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetTrackSoloCommand>(selectedTrackId_,
    ...
    volumeLabel_.onValueChange = [this]() {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            ...
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetTrackVolumeCommand>(selectedTrackId_, gain));
        }
    };
    ...
    panLabel_.onValueChange = [this]() {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetTrackPanCommand>(
    ...
    chainBypassButton_ = std::make_unique<aidaw::SvgButton>("Power", BinaryData::power_on_svg,
    ...
    chainBypassButton_->onClick = [this]() {
        ...
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            aidaw::TrackManager::getInstance().setChainBypassed(selectedTrackId_, !active);
        }
    ...
    aidaw::LinkModeManager::getInstance().addListener(this);

    // Check if there's already a selected track
    selectedTrackId_ = aidaw::TrackManager::getInstance().getSelectedTrack();
...
TrackChainContent::~TrackChainContent() {
    ...
    aidaw::LinkModeManager::getInstance().removeListener(this);
}
...
void TrackChainContent::initGlobalModsPanel() {
    ...
    globalModsPanel_->onModTargetChanged = [this](int modIndex, aidaw::ControlTarget target) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID)
            aidaw::TrackManager::getInstance().setModTarget(
    ...
    globalModsPanel_->onModNameChanged = [this](int modIndex, juce::String name) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID)
            aidaw::TrackManager::getInstance().setModName(
    ...
    globalModsPanel_->onAddModRequested = [this](int slotIndex, aidaw::ModType type,
                                                 aidaw::LFOWaveform waveform) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            aidaw::TrackManager::getInstance().addMod(ChainNodePath::trackLevel(selectedTrackId_),
                                                      slotIndex, type, waveform);
            aidaw::TrackManager::getInstance().notifyTrackDevicesChanged(selectedTrackId_);
        }
    };
    ...
    globalModEditorPanel_->onRateChanged = [this](float rate) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalModIndex_ >= 0)
            aidaw::TrackManager::getInstance().setModRate(
                ChainNodePath::trackLevel(selectedTrackId_), selectedGlobalModIndex_, rate);
    };
    globalModEditorPanel_->onWaveformChanged = [this](aidaw::LFOWaveform waveform) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalModIndex_ >= 0)
            aidaw::TrackManager::getInstance().setModWaveform(
    ...
    globalModEditorPanel_->onTempoSyncChanged = [this](bool tempoSync) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalModIndex_ >= 0)
            aidaw::TrackManager::getInstance().setModTempoSync(
                ChainNodePath::trackLevel(selectedTrackId_), selectedGlobalModIndex_, tempoSync);
    };
    globalModEditorPanel_->onSyncDivisionChanged = [this](aidaw::SyncDivision division) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalModIndex_ >= 0)
            aidaw::TrackManager::getInstance().setModSyncDivision(
                ChainNodePath::trackLevel(selectedTrackId_), selectedGlobalModIndex_, division);
    };
    globalModEditorPanel_->onTriggerModeChanged = [this](aidaw::LFOTriggerMode mode) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalModIndex_ >= 0)
            aidaw::TrackManager::getInstance().setModTriggerMode(
    ...
    globalModEditorPanel_->onAudioAttackChanged = [this](float ms) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalModIndex_ >= 0)
            aidaw::TrackManager::getInstance().setModAudioAttack(
    ...
    globalModEditorPanel_->onAudioReleaseChanged = [this](float ms) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalModIndex_ >= 0)
            aidaw::TrackManager::getInstance().setModAudioRelease(
                ChainNodePath::trackLevel(selectedTrackId_), selectedGlobalModIndex_, ms);
    };
    globalModEditorPanel_->onCurveChanged = [this]() {
        ...
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalModIndex_ >= 0)
            aidaw::TrackManager::getInstance().notifyModCurveChanged(
                ChainNodePath::trackLevel(selectedTrackId_));
    };
    globalModEditorPanel_->onModLinkDeleted = [this](int modIndex, aidaw::ControlTarget target) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID)
            aidaw::TrackManager::getInstance().removeModLink(
                ChainNodePath::trackLevel(selectedTrackId_), modIndex, target);
    };
    globalModEditorPanel_->onModLinkBipolarChanged =
        [this](int modIndex, aidaw::ControlTarget target, bool bipolar) {
            if (selectedTrackId_ != aidaw::INVALID_TRACK_ID)
                aidaw::TrackManager::getInstance().setModLinkBipolar(
                    ChainNodePath::trackLevel(selectedTrackId_), modIndex, target, bipolar);
        };
    globalModEditorPanel_->onModLinkAmountChanged =
        [this](int modIndex, aidaw::ControlTarget target, float amount) {
            if (selectedTrackId_ != aidaw::INVALID_TRACK_ID)
    ...
    globalModEditorPanel_->setParamNameResolver(
        [this](aidaw::DeviceId deviceId, int paramIndex) -> juce::String {
            if (selectedTrackId_ == aidaw::INVALID_TRACK_ID)
                return "P" + juce::String(paramIndex);
            const auto* track = aidaw::TrackManager::getInstance().getTrack(selectedTrackId_);
            if (!track)
                return "P" + juce::String(paramIndex);
            std::function<juce::String(const std::vector<aidaw::ChainElement>&)> findParam;
            findParam = [&](const std::vector<aidaw::ChainElement>& elements) -> juce::String {
                for (const auto& element : elements) {
                    if (aidaw::isDevice(element)) {
                        const auto& device = aidaw::getDevice(element);
                        ...
                        const auto& rack = aidaw::getRack(element);
...
void TrackChainContent::initGlobalMacrosPanel() {
    ...
    globalMacrosPanel_->onMacroValueChanged = [this](int macroIndex, float value) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID)
            aidaw::TrackManager::getInstance().setMacroValue(
                ChainNodePath::trackLevel(selectedTrackId_), macroIndex, value);
    };
    globalMacrosPanel_->onMacroTargetChanged = [this](int macroIndex, aidaw::ControlTarget target) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID)
    ...
    globalMacrosPanel_->onMacroNameChanged = [this](int macroIndex, juce::String name) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID)
            aidaw::TrackManager::getInstance().setMacroName(
                ChainNodePath::trackLevel(selectedTrackId_), macroIndex, name);
    };
    globalMacrosPanel_->onMacroLinkRemoved = [this](int macroIndex, aidaw::ControlTarget target) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
    ...
    globalMacrosPanel_->onMacroAllLinksCleared = [this](int macroIndex) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            aidaw::TrackManager::getInstance().clearAllMacroLinks(
    ...
    globalMacrosPanel_->onAddPageRequested = [this](int) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID)
            aidaw::TrackManager::getInstance().addMacroPage(
                ChainNodePath::trackLevel(selectedTrackId_));
    };
    globalMacrosPanel_->onRemovePageRequested = [this](int) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID)
            aidaw::TrackManager::getInstance().removeMacroPage(
                ChainNodePath::trackLevel(selectedTrackId_));
    };
    globalMacrosPanel_->onPanelClicked = [this]() {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            auto trackPath = aidaw::ChainNodePath::trackLevel(selectedTrackId_);
            aidaw::SelectionManager::getInstance().selectMacrosPanel(trackPath);
        }
    };
    ...
    globalMacroEditorPanel_->onLinkAmountChanged = [this](aidaw::ControlTarget target,
                                                          float amount) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalMacroIndex_ >= 0)
    ...
    globalMacroEditorPanel_->onLinkRemoved = [this](aidaw::ControlTarget target) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalMacroIndex_ >= 0) {
            aidaw::TrackManager::getInstance().removeMacroLink(
                ChainNodePath::trackLevel(selectedTrackId_), selectedGlobalMacroIndex_, target);
            updateGlobalMacrosPanel();
    ...
    globalMacroEditorPanel_->onLinkBipolarToggled = [this](aidaw::ControlTarget target,
                                                           bool bipolar) {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID && selectedGlobalMacroIndex_ >= 0) {
            aidaw::TrackManager::getInstance().setMacroLinkBipolar(
                ChainNodePath::trackLevel(selectedTrackId_), selectedGlobalMacroIndex_, target,
                bipolar);
            updateGlobalMacrosPanel();
        }
    ...
    globalMacroEditorPanel_->setParamNameResolver(
        [this](aidaw::DeviceId deviceId, int paramIndex) -> juce::String {
            ...
            if (!track)
                return "P" + juce::String(paramIndex);
            std::function<juce::String(const std::vector<aidaw::ChainElement>& elements)> findParam;
            findParam = [&](const std::vector<aidaw::ChainElement>& elements) -> juce::String {
                for (const auto& element : elements) {
                    if (aidaw::isDevice(element)) {
                        const auto& device = aidaw::getDevice(element);
    ...
    globalMacroEditorPanel_->setModNameResolver(
        [this](aidaw::ModId modId, int modParamIndex) -> juce::String {
...
void TrackChainContent::updateGlobalModsPanel() {
    if (!globalModsPanel_ || selectedTrackId_ == aidaw::INVALID_TRACK_ID)
        return;
    const auto* track = aidaw::TrackManager::getInstance().getTrack(selectedTrackId_);
    if (!track)
        return;
    ...
    std::vector<std::pair<aidaw::ModId, juce::String>> trackModsList;
    ...
    if (globalModEditorVisible_ && globalModEditorPanel_ && selectedGlobalModIndex_ >= 0 &&
        ...
        globalModEditorPanel_->setModInfo(
            track->mods[idx], &track->mods[idx], [trackId, idx]() -> const aidaw::ModInfo* {
                const auto* t = aidaw::TrackManager::getInstance().getTrack(trackId);
...
void TrackChainContent::updateGlobalMacrosPanel() {
    if (!globalMacrosPanel_ || selectedTrackId_ == aidaw::INVALID_TRACK_ID)
        return;
    const auto* track = aidaw::TrackManager::getInstance().getTrack(selectedTrackId_);
    if (!track)
        return;
    ...
    std::vector<std::pair<aidaw::DeviceId, juce::String>> allDevices;
    std::map<aidaw::DeviceId, std::vector<juce::String>> allDeviceParams;

    std::function<void(const std::vector<aidaw::ChainElement>&)> collectDevices;
    collectDevices = [&](const std::vector<aidaw::ChainElement>& elements) {
        for (const auto& element : elements) {
            if (aidaw::isDevice(element)) {
                const auto& device = aidaw::getDevice(element);
    ...
    std::vector<std::pair<aidaw::ModId, juce::String>> trackMods;
...
void TrackChainContent::showGlobalModEditor(int modIndex) {
    ...
    if (auto* t = aidaw::TrackManager::getInstance().getTrack(selectedTrackId_))
    ...
    globalModEditorPanel_->setOwnerPath(selectedTrackId_,
                                        aidaw::ChainNodePath::trackLevel(selectedTrackId_));
    ...
    globalModEditorPanel_->setModInfo(track->mods[modIndex], &track->mods[modIndex],
                                      [trackId, modIndex]() -> const aidaw::ModInfo* {
                                          const auto* t =
                                              aidaw::TrackManager::getInstance().getTrack(trackId);
...
void TrackChainContent::layoutChainContent() {
    ...
    addDeviceButton_.setVisible(selectedTrackId_ != aidaw::INVALID_TRACK_ID);
...
int TrackChainContent::getOptimalPanelHeight(int windowHeight) const {
    ...
    if (selectedDeviceId_ != aidaw::INVALID_DEVICE_ID &&
        selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
        if (auto* dev =
                aidaw::TrackManager::getInstance().getDevice(selectedTrackId_, selectedDeviceId_)) {
...
void TrackChainContent::trackPropertyChanged(int trackId) {
    if (static_cast<aidaw::TrackId>(trackId) == selectedTrackId_) {
...
void TrackChainContent::trackSelectionChanged(aidaw::TrackId trackId) {
...
void TrackChainContent::trackDevicesChanged(aidaw::TrackId trackId) {
...
void TrackChainContent::macroValueChanged(aidaw::TrackId trackId, aidaw::ChainScope scope,
    ...
    switch (scope) {
        case aidaw::ChainScope::Track:
            ...
            return;
        case aidaw::ChainScope::Rack:
            for (auto& node : nodeComponents_) {
                if (node && node->getNodePath().getRackId() == ownerId)
                    node->updateMacroValueDisplay(macroIndex, value);
            }
            return;
        case aidaw::ChainScope::Device: {
            const aidaw::DeviceId deviceId = ownerId;
            for (auto& node : nodeComponents_) {
...
void TrackChainContent::selectionTypeChanged(aidaw::SelectionType /*newType*/) {
    // Repaint header when selection type changes (Track vs ChainNode)
    // to update the header background color
    repaint();
}

void TrackChainContent::modLinkModeChanged(bool active, const aidaw::ModSelection& /*selection*/) {
...
void TrackChainContent::macroLinkModeChanged(bool active,
                                             const aidaw::MacroSelection& /*selection*/) {
    linkModeLabel_.setVisible(active);
    if (active) {
        linkModeLabel_.setColour(juce::Label::textColourId,
...
void TrackChainContent::updateFromSelectedTrack() {
    if (selectedTrackId_ == aidaw::INVALID_TRACK_ID) {
        ...
        if (track) {
            ...
            aidaw::AutomationTarget volTarget;
            volTarget.kind = aidaw::ControlTarget::Kind::TrackVolume;
            volTarget.devicePath = aidaw::ChainNodePath::trackLevel(selectedTrackId_);
            volumeLabel_.setAutomationTarget(volTarget);
            aidaw::AutomationTarget panTarget;
            panTarget.kind = aidaw::ControlTarget::Kind::TrackPan;
            panTarget.devicePath = aidaw::ChainNodePath::trackLevel(selectedTrackId_);
            panLabel_.setAutomationTarget(panTarget);

            // Check if any device in the chain is not bypassed
            const auto& elements =
                aidaw::TrackManager::getInstance().getChainElements(selectedTrackId_);
            bool anyActive = elements.empty();  // Empty chain = active (not bypassed)
            for (const auto& element : elements) {
                if (aidaw::isDevice(element) && !aidaw::getDevice(element).bypassed) {
                    anyActive = true;
                    break;
                }
                if (aidaw::isRack(element) && !aidaw::getRack(element).bypassed) {
                    anyActive = true;
                    break;
                }
            }
            ...
            if (track->type == aidaw::TrackType::Master) {
...
void TrackChainContent::layoutHeader(juce::Rectangle<int> headerBounds) {
    ...
    const auto* selTrack = aidaw::TrackManager::getInstance().getTrack(selectedTrackId_);
    bool isMaster = selTrack && selTrack->type == aidaw::TrackType::Master;
...
void TrackChainContent::rebuildNodeComponents() {
    ...
    const auto& elements = aidaw::TrackManager::getInstance().getChainElements(selectedTrackId_);
    ...
    for (size_t i = 0; i < elements.size(); ++i) {
        ...
        if (aidaw::isDevice(element)) {
            ...
            slot->setNodePath(aidaw::ChainNodePath::topLevelDevice(selectedTrackId_, device.id));
        ...
        } else if (aidaw::isRack(element)) {
            // Create rack component
            const auto& rack = aidaw::getRack(element);
            auto rackComp = std::make_unique<RackComponent>(selectedTrackId_, rack);
            rackComp->setNodePath(aidaw::ChainNodePath::rack(selectedTrackId_, rack.id));

            // Wire up callbacks
            rackComp->onSelected = [this]() { selectedDeviceId_ = aidaw::INVALID_DEVICE_ID; };
            rackComp->onLayoutChanged = [this]() {
                resized();
                repaint();
            };
            rackComp->onChainSelected = [this](aidaw::TrackId trackId, aidaw::RackId rId,
                                               aidaw::ChainId chainId) {
                onChainSelected(trackId, rId, chainId);
            };
            rackComp->onDeviceSelected = [this](aidaw::DeviceId deviceId) {
                if (deviceId != aidaw::INVALID_DEVICE_ID) {
                    selectedDeviceId_ = aidaw::INVALID_DEVICE_ID;
                    aidaw::SelectionManager::getInstance().selectDevice(
                        selectedTrackId_, selectedRackId_, selectedChainId_, deviceId);
                } else {
                    aidaw::SelectionManager::getInstance().clearDeviceSelection();
                }
            };
    ...
    auto* trackInfo = aidaw::TrackManager::getInstance().getTrack(selectedTrackId_);
    ...
    if (selectedPath.isValid() && selectedPath.trackId == selectedTrackId_) {
        ...
        if (deviceSel.trackId == selectedTrackId_ &&
            deviceSel.deviceId != aidaw::INVALID_DEVICE_ID) {
...
void TrackChainContent::onChainSelected(aidaw::TrackId trackId, aidaw::RackId rackId,
                                        aidaw::ChainId chainId) {
    ...
    aidaw::TrackManager::getInstance().setSelectedChain(selectedTrackId_, rackId, chainId);
...
bool TrackChainContent::hasSelectedTrack() const {
    return selectedTrackId_ != aidaw::INVALID_TRACK_ID;
}

bool TrackChainContent::hasSelectedChain() const {
    return selectedTrackId_ != aidaw::INVALID_TRACK_ID &&
           selectedRackId_ != aidaw::INVALID_RACK_ID && selectedChainId_ != aidaw::INVALID_CHAIN_ID;
}
...
void TrackChainContent::onAddDeviceClicked() {
    ...
    auto internals = aidaw::daw::ui::PluginBrowserContent::getInternalPlugins();
    ...
    if (auto* engine = dynamic_cast<aidaw::TracktionEngineWrapper*>(
            aidaw::TrackManager::getInstance().getAudioEngine())) {
    ...
    auto capturedInternals =
        std::make_shared<std::vector<aidaw::daw::ui::PluginBrowserInfo>>(std::move(internals));
    ...
    menu.showMenuAsync(juce::PopupMenu::Options(), [safeThis, trackId, capturedPlugins,
        ...
        aidaw::TrackManager::getInstance().addDeviceToTrack(trackId, device);
    });
}

void TrackChainContent::addDeviceToSelectedTrack(const aidaw::DeviceInfo& device) {
    if (!hasSelectedTrack()) {
        return;
    }
    scrollToEndAfterNextDeviceChange_ = true;
    aidaw::TrackManager::getInstance().addDeviceToTrack(selectedTrackId_, device);
}

void TrackChainContent::addDeviceToSelectedChain(const aidaw::DeviceInfo& device) {
    if (!hasSelectedChain()) {
        return;
    }
    aidaw::TrackManager::getInstance().addDeviceToChain(selectedTrackId_, selectedRackId_,
...
void TrackChainContent::onDeviceSlotSelected(aidaw::DeviceId deviceId) {
    ...
    // Notify SelectionManager - this will update inspector
    aidaw::SelectionManager::getInstance().selectDevice(selectedTrackId_, deviceId);
}
        ...
        if (path.isValid()) {
            ...
            if (auto* rack = dynamic_cast<RackComponent*>(node.get())) {
                ...
                if (chainIt != savedExpandedChains_.end() &&
                    chainIt->second != aidaw::INVALID_CHAIN_ID) {
...
void TrackChainContent::showPresetMenu() {
    auto& pm = aidaw::PresetManager::getInstance();
    ...
    menu.showMenuAsync(
        ...
        [this, indexCopy](int chosen) {
            ...
            if (chosen == kSaveAs) {
                ...
                aidaw::PresetManager::getInstance().getChainsDirectory().revealToUser();
    ...
    aw->enterModalState(
        true, juce::ModalCallbackFunction::create([aw, self](int result) {
            ...
            auto doSave = [name, self]() {
                if (self == nullptr)
                    return;
                if (self->selectedTrackId_ == aidaw::INVALID_TRACK_ID)
                    return;
                const auto& elements =
                    aidaw::TrackManager::getInstance().getChainElements(self->selectedTrackId_);
                auto& mgr = aidaw::PresetManager::getInstance();
            ...
            if (aidaw::PresetManager::getInstance().getChainPresets().contains(name)) {
...
void TrackChainContent::saveCurrentTrackPreset() {
    if (currentPresetName_.isEmpty() || selectedTrackId_ == aidaw::INVALID_TRACK_ID)
        return;
    const auto& elements = aidaw::TrackManager::getInstance().getChainElements(selectedTrackId_);
    auto& pm = aidaw::PresetManager::getInstance();
