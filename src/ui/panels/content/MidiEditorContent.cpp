...
MidiEditorContent::MidiEditorContent() {
    ...
    timeRuler_->onPhaseDragEnded = [this](double phaseSeconds) {
        ...
        aidaw::ClipManager::getInstance().setClipMidiOffset(editingClipId_, phaseBeats);
    };
...
void MidiEditorContent::scrollToClipStartForTimeMode() {
    ...
    if (!relativeTimeMode_ && clip && clip->view != aidaw::ClipView::Session)
...
void MidiEditorContent::clipPropertyChanged(aidaw::ClipId clipId) {
...
void MidiEditorContent::timelineStateChanged(const aidaw::TimelineState& state,
                                             aidaw::ChangeFlags changes) {
    // Playhead changes
    if (aidaw::hasFlag(changes, aidaw::ChangeFlags::Playhead)) {
        ...
        const auto* editClip = (editingClipId_ != aidaw::INVALID_CLIP_ID)
                                   ? aidaw::ClipManager::getInstance().getClip(editingClipId_)
                                   : nullptr;
    ...
    if (aidaw::hasFlag(changes, aidaw::ChangeFlags::Selection)) {
    ...
    if (aidaw::hasFlag(changes, aidaw::ChangeFlags::Tempo) ||
        aidaw::hasFlag(changes, aidaw::ChangeFlags::Timeline)) {
        updateTimeRuler();
        updateGridSize();
        repaint();
    }
...
void MidiEditorContent::updateGridResolution() {
    ...
    double frac = aidaw::GridConstants::findBeatSubdivision(horizontalZoom_, minPixelSpacing);
    ...
    if (editingClipId_ != aidaw::INVALID_CLIP_ID) {
        ...
        if (clip) {
            ...
            if (clip->gridAutoGrid) {
                ...
                constexpr int minPixelSpacing = 20;
                double frac =
                    aidaw::GridConstants::findBeatSubdivision(horizontalZoom_, minPixelSpacing);
...
void MidiEditorContent::setGridSettingsFromUI(bool autoGrid, int numerator, int denominator) {
    if (editingClipId_ != aidaw::INVALID_CLIP_ID) {
        aidaw::ClipManager::getInstance().setClipGridSettings(editingClipId_, autoGrid, numerator,
                                                              denominator);
...
void MidiEditorContent::setSnapEnabledFromUI(bool enabled) {
    if (editingClipId_ != aidaw::INVALID_CLIP_ID) {
        aidaw::ClipManager::getInstance().setClipSnapEnabled(editingClipId_, enabled);
...
void MidiEditorContent::setupVelocityLane() {
    velocityLane_ = std::make_unique<aidaw::VelocityLaneComponent>();
    velocityLane_->setLeftPadding(GRID_LEFT_PADDING);
    velocityLane_->onVelocityChanged = [this](aidaw::ClipId clipId, size_t noteIndex,
                                              int newVelocity) {
        auto cmd =
            std::make_unique<aidaw::SetMidiNoteVelocityCommand>(clipId, noteIndex, newVelocity);
        aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
        onVelocityEdited();
    };
    velocityLane_->onMultiVelocityChanged = [this](aidaw::ClipId clipId,
                                                   std::vector<std::pair<size_t, int>> velocities) {
        auto cmd = std::make_unique<aidaw::SetMultipleNoteVelocitiesCommand>(clipId,
                                                                             std::move(velocities));
        aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
        onVelocityEdited();
    };
...
void MidiEditorContent::setupMidiDrawer() {
    midiDrawer_ = std::make_unique<aidaw::MidiDrawerComponent>();
    ...
    if (velLane) {
        velLane->onVelocityChanged = [this](aidaw::ClipId clipId, size_t noteIndex,
                                            int newVelocity) {
            auto cmd =
                std::make_unique<aidaw::SetMidiNoteVelocityCommand>(clipId, noteIndex, newVelocity);
            aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
        ...
        velLane->onMultiVelocityChanged = [this](aidaw::ClipId clipId,
                                                 std::vector<std::pair<size_t, int>> velocities) {
            auto cmd = std::make_unique<aidaw::SetMultipleNoteVelocitiesCommand>(
