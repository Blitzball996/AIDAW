...
class WaveformEditorContent::ButtonLookAndFeel : public juce::LookAndFeel_V4 {
  public:
    ...
    juce::Font getTextButtonFont(juce::TextButton&, int /*buttonHeight*/) override {
        return aidaw::FontManager::getInstance().getButtonFont(11.0f);
    }
    ...
    void drawButtonText(juce::Graphics& g, juce::TextButton& button, bool /*isMouseOver*/,
        ...
        auto font = aidaw::FontManager::getInstance().getButtonFont(11.0f);
        g.setFont(font);
  ...
  public:
    ...
    void paint(juce::Graphics& g) override {
        if (getWidth() <= 0 || getHeight() <= 0)
            return;
        if (owner_.editingClipId_ == aidaw::INVALID_CLIP_ID)
            return;

        const auto* clip = aidaw::ClipManager::getInstance().getClip(owner_.editingClipId_);
        if (!clip)
            return;
...
WaveformEditorContent::WaveformEditorContent() {
    ...
    aidaw::UndoManager::getInstance().addListener(this);

    // Create time ruler
    timeRuler_ = std::make_unique<aidaw::TimeRuler>();
    timeRuler_->setDisplayMode(aidaw::TimeRuler::DisplayMode::BarsBeats);
    ...
    timeRuler_->onZoomChanged = [this](double newRulerZoom, double /*anchorTime*/,
                                       int anchorScreenX) {
        double bpm = 120.0;
        auto* controller = aidaw::TimelineController::getCurrent();
        if (controller)
    ...
    auto commitLoopFromDisplay = [this](double displayStart, double displayEnd) {
        ...
        auto* controller = aidaw::TimelineController::getCurrent();
        double bpm = controller ? controller->getState().tempo.bpm : aidaw::DEFAULT_BPM;
        bpm = aidaw::isValidBpm(bpm) ? bpm : aidaw::DEFAULT_BPM;
        ...
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetClipLoopRangeCommand>(editingClipId_, newLoopStart,
                                                             newLoopLength, bpm));
    };
    ...
    gridNumeratorLabel_ =
        std::make_unique<aidaw::DraggableValueLabel>(aidaw::DraggableValueLabel::Format::Integer);
    ...
    gridSlashLabel_->setFont(aidaw::FontManager::getInstance().getUIFont(11.0f));
    ...
    horizontalScrollBar_ =
        std::make_unique<aidaw::ZoomScrollBar>(aidaw::ZoomScrollBar::Orientation::Horizontal);
...
WaveformEditorContent::~WaveformEditorContent() {
    ...
    aidaw::UndoManager::getInstance().removeListener(this);
...
void WaveformEditorContent::clipPropertyChanged(aidaw::ClipId clipId) {
    if (clipId == editingClipId_) {
        ...
        if (clip) {
            ...
            if (auto* tc = aidaw::TimelineController::getCurrent())
            ...
            if (clip->view != aidaw::ClipView::Session && !clip->loopEnabled) {
        ...
        if (transientsCached_ && clip->isAudio() && !clip->audio().source.filePath.isEmpty()) {
            auto* cached = aidaw::AudioThumbnailManager::getInstance().getCachedTransients(
...
void WaveformEditorContent::clipSelectionChanged(aidaw::ClipId clipId) {
...
void WaveformEditorContent::setClip(aidaw::ClipId clipId) {
    ...
    if (gridComponent_) {
        ...
        if (clip && timeRuler_) {
            ...
            if (clip->audio().source.filePath.isNotEmpty()) {
                auto* thumbnail = aidaw::AudioThumbnailManager::getInstance().getThumbnail(
                    clip->audio().source.filePath);
                if (thumbnail) {
            ...
            double bpm = cachedBpm_ > 0.0 ? cachedBpm_ : 120.0;
            auto info = aidaw::ClipDisplayInfo::from(*clip, bpm, fileDuration);
...
void WaveformEditorContent::updateDisplayInfo(const aidaw::ClipInfo& clip) {
    double bpm = 120.0;
    auto* controller = aidaw::TimelineController::getCurrent();
    if (controller) {
        bpm = controller->getState().tempo.bpm;
...
void WaveformEditorContent::refreshWarpMarkers() {
    auto* bridge = getBridge();
    if (bridge && editingClipId_ != aidaw::INVALID_CLIP_ID) {
        auto markers = bridge->getWarpMarkers(editingClipId_);
        gridComponent_->setWarpMarkers(markers);
    }
}

aidaw::AudioBridge* WaveformEditorContent::getBridge() {
...
void WaveformEditorContent::timerCallback() {
    if (transientsCached_ || editingClipId_ == aidaw::INVALID_CLIP_ID) {
        stopTimer();
        return;
    }
...
void WaveformEditorContent::sliceAtWarpMarkers() {
    ...
    aidaw::sliceClipAtWarpMarkers(editingClipId_, tempo, getBridge());
...
void WaveformEditorContent::sliceAtGrid() {
    ...
    aidaw::sliceClipAtGrid(editingClipId_, gridInterval, bpm, getBridge());
...
void WaveformEditorContent::sliceWarpMarkersToDrumGrid() {
    ...
    double tempo = cachedBpm_ > 0.0 ? cachedBpm_ : 120.0;
    aidaw::sliceWarpMarkersToDrumGrid(editingClipId_, tempo, getBridge());
}
...
void WaveformEditorContent::sliceAtGridToDrumGrid() {
    ...
    double gridInterval = gridBeats * 60.0 / bpm;
    aidaw::sliceAtGridToDrumGrid(editingClipId_, gridInterval, bpm, getBridge());
}
