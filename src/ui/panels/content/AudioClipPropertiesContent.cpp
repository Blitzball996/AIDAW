...
double getAudioFileDurationForProperties(const aidaw::ClipInfo& clip) {
    if (!clip.isAudio())
        return 0.0;
...
double getProjectBpmForProperties() {
    if (auto* tc = aidaw::TimelineController::getCurrent()) {
        ...
        return aidaw::isValidBpm(bpm) ? bpm : aidaw::DEFAULT_BPM;
    }
...
SourceDisplayValues getSourceDisplayValues(const aidaw::ClipInfo& clip, aidaw::ClipId clipId) {
    ...
    if (storedBpmLooksDefaulted) {
        ...
        if (cachedBpm > 0.0) {
            ...
            thumbs.requestBPMDetection(clip.audio().source.filePath, [clipId](double bpm) {
                if (bpm > 0.0)
                    aidaw::ClipManager::getInstance().forceNotifyClipPropertyChanged(clipId);
            });
        }
    }
...
void AudioClipPropertiesContent::onActivated() {
    ...
    clipId_ = aidaw::ClipManager::getInstance().getSelectedClip();
    updateFromClip();
}
...
void AudioClipPropertiesContent::clipSelectionChanged(aidaw::ClipId clipId) {
    clipId_ = clipId;
    updateFromClip();
}

void AudioClipPropertiesContent::clipPropertyChanged(aidaw::ClipId clipId) {
    if (clipId == clipId_)
        updateFromClip();
}
...
void AudioClipPropertiesContent::createControls() {
    ...
    warpToggle_->onClick = [this]() {
        ...
        if (!clip)
            return;
        aidaw::ClipManager::getInstance().setClipWarpEnabled(clipId_, !clip->warpEnabled);
    };
    ...
    autoTempoToggle_->onClick = [this]() {
        if (clipId_ == aidaw::INVALID_CLIP_ID)
            return;
        auto* clip = aidaw::ClipManager::getInstance().getClip(clipId_);
        if (!clip)
            return;
            ...
            if (cached > 0.0) {
                ...
                thumbs.requestBPMDetection(
                    clip->audio().source.filePath, [cid](double detectedBPM) {
                        ...
                        double live =
                            aidaw::ProjectManager::getInstance().getCurrentProjectInfo().tempo;
                        bool existingLooksDefaulted =
                            c->audio().interpretation.bpm > 0.0 && aidaw::isValidBpm(live) &&
        ...
        aidaw::ClipManager::getInstance().setAutoTempo(clipId_, enable, bpm);
    };
    ...
    reverseToggle_->onClick = [this]() {
        ...
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetClipReversedCommand>(clipId_, !clip->isReversed));
    };
    ...
    stretchValue_->onValueChange = [this]() {
        if (clipId_ == aidaw::INVALID_CLIP_ID)
            return;
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetClipSpeedRatioCommand>(clipId_, stretchValue_->getValue()));
    };
    ...
    stretchModeCombo_->onChange = [this]() {
        ...
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetClipStretchModeCommand>(clipId_, mode));
    };
    ...
    bpmValue_->onValueChange = [this]() {
        ...
        if (clip->autoTempo) {
            ...
            aidaw::ClipManager::getInstance().applyAudioClipBeats(clipId_, u, bpm);
        } else {
            ...
            aidaw::ClipManager::getInstance().forceNotifyClipPropertyChanged(clipId_);
        }
    };
    ...
    beatsValue_->onValueChange = [this]() {
        ...
        aidaw::ClipManager::getInstance().applyAudioClipBeats(clipId_, u, projectBpm);
    };
    ...
    pitchValue_->onValueChange = [this]() {
        if (clipId_ == aidaw::INVALID_CLIP_ID)
            return;
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetClipPitchCommand>(
    ...
    analogPitchToggle_->onClick = [this]() {
        ...
        if (!clip)
            return;
        aidaw::ClipManager::getInstance().setAnalogPitch(clipId_, !clip->analogPitch);
    };
    ...
    volumeValue_->onValueChange = [this]() {
        if (clipId_ == aidaw::INVALID_CLIP_ID)
            return;
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetClipVolumeDBCommand>(
    ...
    gainValue_->onValueChange = [this]() {
        if (clipId_ == aidaw::INVALID_CLIP_ID)
            return;
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetClipGainDBCommand>(
    ...
    panValue_->onValueChange = [this]() {
        if (clipId_ == aidaw::INVALID_CLIP_ID)
            return;
        aidaw::UndoManager::getInstance().executeCommand(std::make_unique<aidaw::SetClipPanCommand>(
...
void AudioClipPropertiesContent::updateFromClip() {
    ...
    if (hasClip) {
        ...
        bpmValue_->setValue(sourceDisplay.bpm > 0.0 ? sourceDisplay.bpm : aidaw::DEFAULT_BPM,
