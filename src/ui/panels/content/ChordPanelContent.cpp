...
ScaleBlockComponent::ScaleBlockComponent(const aidaw::music::ScaleWithChords& scale)
    : scale_(scale) {
    setName("ScaleBlock");
...
ScaleChordsPopup::ScaleChordsPopup(const aidaw::music::ScaleWithChords& scale) : scale_(scale) {
    ...
    for (size_t i = 0; i < scale_.chords.size(); ++i) {
        ...
        if (i < 7) {
            ...
            if (scale_.chords[i].quality == aidaw::music::ChordQuality::Minor ||
                scale_.chords[i].quality == aidaw::music::ChordQuality::Diminished) {
                degree = degree.toLowerCase();
            }
            if (scale_.chords[i].quality == aidaw::music::ChordQuality::Diminished)
...
BrowseScaleRowComponent::BrowseScaleRowComponent(const aidaw::music::ScaleWithChords& scale)
    : scale_(scale) {
...
void ChordPanelContent::previewChord(const aidaw::music::Chord& chord) {
    stopPreview();
    if (trackId_ == aidaw::INVALID_TRACK_ID || chord.notes.empty())
        return;
    ...
    for (const auto& note : chord.notes) {
        ...
        aidaw::TrackManager::getInstance().previewNote(trackId_, n, 100, true);
        previewingNotes_.push_back(n);
    }
}

void ChordPanelContent::stopPreview() {
    if (previewingNotes_.empty() || trackId_ == aidaw::INVALID_TRACK_ID)
        return;
    for (int noteNum : previewingNotes_)
        aidaw::TrackManager::getInstance().previewNote(trackId_, noteNum, 0, false);
...
void ChordPanelContent::setChordEngine(aidaw::daw::audio::MidiChordEnginePlugin* plugin,
                                       aidaw::TrackId trackId) {
...
void ChordPanelContent::chordChanged(aidaw::daw::audio::MidiChordEnginePlugin*) {
...
void ChordPanelContent::keyModeChanged(aidaw::daw::audio::MidiChordEnginePlugin*) {
    juce::MessageManager::callAsync([safeThis = juce::Component::SafePointer(this)] {
        if (safeThis)
            safeThis->updateFromPlugin();
    });
...
void ChordPanelContent::suggestionsChanged(aidaw::daw::audio::MidiChordEnginePlugin*) {
    juce::MessageManager::callAsync([safeThis = juce::Component::SafePointer(this)] {
        if (safeThis)
            safeThis->updateFromPlugin();
    });
...
void ChordPanelContent::configChanged() {
    juce::MessageManager::callAsync([safeThis = juce::Component::SafePointer(this)] {
        if (safeThis && safeThis->suggestionTab_ == SuggestionTab::AI) {
            auto cfg = Config::getInstance().getAgentLLMConfig(aidaw::role::MUSIC);
...
void ChordPanelContent::rebuildSuggestionBlocks() {
    ...
    for (const auto& item : suggestions_) {
        ...
        block->onClicked = [this](const aidaw::music::Chord& c) { previewChord(c); };
...
void ChordPanelContent::showScalePopup(const aidaw::music::ScaleWithChords& scale,
...
void ChordPanelContent::rebuildBrowseRows() {
    ...
    const auto& allScales = aidaw::music::getAllScalesWithChordsCached();
...
void ChordPanelContent::rebuildAIProgressionRows() {
    ...
    for (int progIdx = 0; progIdx < static_cast<int>(aiProgs.size()); ++progIdx) {
        ...
        row->exportButton = std::make_unique<aidaw::SvgButton>("ExportProg", BinaryData::copy_svg,
        ...
        row->exportButton->onClick = [this, progIdx = row->progressionIndex]() {
            ...
            std::vector<aidaw::MidiNote> notes;
...
void ChordPanelContent::AIRequestThread::run() {
    ...
    auto agentConfig = aidaw::Config::getInstance().getAgentLLMConfig(aidaw::role::MUSIC);
    bool isLocal = agentConfig.provider == aidaw::provider::LLAMA_LOCAL;
...
%ignore WS
    ...
    bool cfg = aidaw::supportsOpenAICFG(agentConfig);

    std::unique_ptr<llm::LLMClient> client;
    if (cfg) {
        auto pc = aidaw::toLLMProviderConfig(agentConfig, "music");
        client = llm::LLMClientFactory::create(pc);
    } else {
        client = aidaw::createLLMClient(agentConfig, "music");
    }
    ...
    while (pos < dsl.length()) {
        ...
        while (chordPos < progBlock.length()) {
            ...
            if (rootStr.isNotEmpty() && qualityStr.isNotEmpty()) {
                ...
                aidaw::music::ChordEngine::finalizeChord(chord);
...
void ChordPanelContent::setupFooterControls() {
    ...
    noveltyLabel_ = std::make_unique<aidaw::DraggableValueLabel>(
        aidaw::DraggableValueLabel::Format::Percentage);
    ...
    scaleFilterBtn_ = std::make_unique<aidaw::SvgButton>("ScaleFilter", BinaryData::funnel_svg,
    ...
    browseBtn_ = std::make_unique<aidaw::SvgButton>("Browse", BinaryData::browser_svg,
    ...
    backBtn_ = std::make_unique<aidaw::SvgButton>("Back", BinaryData::chevron_left_svg,
    ...
    clearHistoryBtn_ = std::make_unique<aidaw::SvgButton>("ClearHistory", BinaryData::delete_svg,
    ...
    ksTabBtn_ = std::make_unique<aidaw::SvgButton>("KSTab", BinaryData::psychology_svg,
    ...
    aiSendBtn_ = std::make_unique<aidaw::SvgButton>("AISend", BinaryData::send_svg,
