...
PianoRollContent::PianoRollContent() {
    ...
    chordToggle_ = std::make_unique<aidaw::SvgButton>("ChordToggle", BinaryData::chord_svg,
    ...
    chordDetectBtn_ = std::make_unique<aidaw::SvgButton>("ChordDetect", BinaryData::refresh_svg,
    ...
    velocityToggle_ = std::make_unique<aidaw::SvgButton>(
    ...
    keyboard_ = std::make_unique<aidaw::PianoRollKeyboard>();
    ...
    keyboard_->onNotePreview = [this](int noteNumber, int velocity, bool isNoteOn) {
        ...
        if (editingClipId_ != aidaw::INVALID_CLIP_ID) {
            const auto* clip = aidaw::ClipManager::getInstance().getClip(editingClipId_);
            if (clip && clip->trackId != aidaw::INVALID_TRACK_ID) {
                DBG("PianoRollContent: Calling TrackManager::previewNote for track "
                    << clip->trackId);
                // Preview note through track's instruments
                aidaw::TrackManager::getInstance().previewNote(clip->trackId, noteNumber, velocity,
                                                               isNoteOn);
            } else {
    ...
    gridComponent_ = std::make_unique<aidaw::PianoRollGridComponent>();
    ...
    if (auto* controller = aidaw::TimelineController::getCurrent()) {
...
void PianoRollContent::installMidiNoteMonitor() {
    auto* engine = aidaw::TrackManager::getInstance().getAudioEngine();
    ...
    midiBridge->onNoteEvent = [safeThis, previousCallback](aidaw::TrackId trackId,
                                                           const aidaw::MidiNoteEvent& event) {
        if (previousCallback)
...
void PianoRollContent::handleMidiNoteEvent(aidaw::TrackId trackId,
                                           const aidaw::MidiNoteEvent& event) {
    if (!midiNoteMonitorInstalled_ || keyboard_ == nullptr ||
        editingClipId_ == aidaw::INVALID_CLIP_ID)
        return;
...
void PianoRollContent::setupGridCallbacks() {
    ...
    gridComponent_->onNoteAdded = [](aidaw::ClipId clipId, double beat, int noteNumber,
                                     double lengthBeats, int velocity) {
        auto cmd = std::make_unique<aidaw::AddMidiNoteCommand>(clipId, beat, noteNumber,
                                                               lengthBeats, velocity);
        aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
        // Note: UI refresh handled via ClipManagerListener::clipPropertyChanged()
    };

    // Handle note movement
    gridComponent_->onNoteMoved = [](aidaw::ClipId clipId, size_t noteIndex, double newBeat,
                                     int newNoteNumber) {
        // Get source clip and note
        auto* sourceClip = aidaw::ClipManager::getInstance().getClip(clipId);
        ...
        auto cmd =
            std::make_unique<aidaw::MoveMidiNoteCommand>(clipId, noteIndex, newBeat, newNoteNumber);
        aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));

        // After moving, check if note is still visible in this clip (considering offset)
        auto* clip = aidaw::ClipManager::getInstance().getClip(clipId);
        if (clip && clip->isMidi() && noteIndex < clip->midiNotes.size()) {
            ...
            double effectiveOffset = (clip->view == aidaw::ClipView::Session || clip->loopEnabled)
                                         ? clip->midiOffset
                                         : 0.0;
            if (note.startBeat < effectiveOffset ||
                ...
                aidaw::ClipId destClipId = aidaw::ClipManager::getInstance().getClipAtPosition(
                    clip->trackId, absoluteSeconds);

                if (destClipId != aidaw::INVALID_CLIP_ID && destClipId != clipId) {
                    DBG("  -> Would be visible in clip " << destClipId << ", moving it there");
                    auto* destClip = aidaw::ClipManager::getInstance().getClip(destClipId);
                    if (destClip && destClip->isMidi()) {
                        ...
                        double destOffset =
                            (destClip->view == aidaw::ClipView::Session || destClip->loopEnabled)
                                ? destClip->midiOffset
                                : 0.0;
                        ...
                        auto moveCmd = std::make_unique<aidaw::MoveMidiNoteBetweenClipsCommand>(
                            clipId, noteIndex, destClipId, relativeNewBeat, note.noteNumber);
                        aidaw::UndoManager::getInstance().executeCommand(std::move(moveCmd));
                    }
                }
            }
        }
    ...
    gridComponent_->onNoteCopied = [this](aidaw::ClipId clipId, size_t noteIndex, double destBeat,
                                          int destNoteNumber) {
        const auto* clip = aidaw::ClipManager::getInstance().getClip(clipId);
        if (!clip || noteIndex >= clip->midiNotes.size())
            return;

        const auto& sourceNote = clip->midiNotes[noteIndex];
        auto cmd = std::make_unique<aidaw::AddMidiNoteCommand>(
        ...
        const auto* updatedClip = aidaw::ClipManager::getInstance().getClip(clipId);
    ...
    gridComponent_->onNoteResized = [](aidaw::ClipId clipId, size_t noteIndex, double newLength) {
        auto cmd = std::make_unique<aidaw::ResizeMidiNoteCommand>(clipId, noteIndex, newLength);
        aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
        // Note: UI refresh handled via ClipManagerListener::clipPropertyChanged()
    };

    // Handle note deletion
    gridComponent_->onNoteDeleted = [](aidaw::ClipId clipId, size_t noteIndex) {
        auto cmd = std::make_unique<aidaw::DeleteMidiNoteCommand>(clipId, noteIndex);
        aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
        // Note: UI refresh handled via ClipManagerListener::clipPropertyChanged()
    };
    ...
    gridComponent_->onMultipleNotesMoved =
        [](aidaw::ClipId clipId, std::vector<aidaw::MoveMultipleMidiNotesCommand::NoteMove> moves) {
            auto cmd =
                std::make_unique<aidaw::MoveMultipleMidiNotesCommand>(clipId, std::move(moves));
            aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
        };

    // Handle batch note resize (single undo step)
    gridComponent_->onMultipleNotesResized =
        [](aidaw::ClipId clipId, std::vector<std::pair<size_t, double>> noteLengths) {
            auto cmd = std::make_unique<aidaw::ResizeMultipleMidiNotesCommand>(
    ...
    gridComponent_->onLeftResizeMultipleNotes =
        [](aidaw::ClipId clipId, std::vector<aidaw::MoveMultipleMidiNotesCommand::NoteMove> moves,
           std::vector<std::pair<size_t, double>> noteLengths) {
            aidaw::CompoundOperationScope scope("Resize Notes From Left");
            auto moveCmd =
                std::make_unique<aidaw::MoveMultipleMidiNotesCommand>(clipId, std::move(moves));
            aidaw::UndoManager::getInstance().executeCommand(std::move(moveCmd));
            auto resizeCmd = std::make_unique<aidaw::ResizeMultipleMidiNotesCommand>(
                clipId, std::move(noteLengths));
            aidaw::UndoManager::getInstance().executeCommand(std::move(resizeCmd));
        };

    // Handle note selection - update SelectionManager
    gridComponent_->onNoteSelected = [](aidaw::ClipId clipId, size_t noteIndex, bool isAdditive) {
        if (isAdditive) {
            aidaw::SelectionManager::getInstance().addNoteToSelection(clipId, noteIndex);
        } else {
            aidaw::SelectionManager::getInstance().selectNote(clipId, noteIndex);
        }
    };

    // Handle batch note selection changes (lasso, deselect-all, Cmd+click toggle)
    gridComponent_->onNoteSelectionChanged = [this](aidaw::ClipId clipId,
                                                    std::vector<size_t> noteIndices) {
        if (noteIndices.empty()) {
            // Clear note selection — preserve clip selection
            aidaw::SelectionManager::getInstance().clearNoteSelection();
            ...
            aidaw::SelectionManager::getInstance().selectNotes(clipId, noteIndices);
        }
    };

    // Forward note drag preview to velocity lane for position sync
    gridComponent_->onNoteDragging = [this](aidaw::ClipId /*clipId*/, size_t noteIndex,
    ...
    gridComponent_->onQuantizeNotes = [](aidaw::ClipId clipId, std::vector<size_t> noteIndices,
                                         aidaw::QuantizeMode mode, double gridBeats) {
        auto cmd = std::make_unique<aidaw::QuantizeMidiNotesCommand>(clipId, std::move(noteIndices),
                                                                     gridBeats, mode);
        aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
    };

    // Handle copy from context menu
    gridComponent_->onCopyNotes = [](aidaw::ClipId clipId, std::vector<size_t> noteIndices) {
        aidaw::ClipManager::getInstance().copyNotesToClipboard(clipId, noteIndices);
    };

    // Handle paste from context menu
    gridComponent_->onPasteNotes = [](aidaw::ClipId clipId) {
        auto& clipManager = aidaw::ClipManager::getInstance();
        ...
        std::vector<aidaw::MidiNote> notesToPaste;
        notesToPaste.reserve(clipboard.size());
        for (const auto& note : clipboard) {
            aidaw::MidiNote n = note;
            n.startBeat += pasteOffset;
            notesToPaste.push_back(n);
        }

        auto cmd = std::make_unique<aidaw::AddMultipleMidiNotesCommand>(
        ...
        if (!inserted.empty()) {
            aidaw::SelectionManager::getInstance().selectNotes(
                clipId, std::vector<size_t>(inserted.begin(), inserted.end()));
        }
    };

    // Handle duplicate from context menu
    gridComponent_->onDuplicateNotes = [this](aidaw::ClipId clipId,
                                              std::vector<size_t> noteIndices) {
        auto& clipManager = aidaw::ClipManager::getInstance();
        const auto* clip = clipManager.getClip(clipId);
        if (!clip || !clip->isMidi())
        ...
        double maxEnd = 0.0;
        std::vector<aidaw::MidiNote> notesToDuplicate;
    ...
    gridComponent_->onDeleteNotes = [](aidaw::ClipId clipId, std::vector<size_t> noteIndices) {
        auto cmd =
            std::make_unique<aidaw::DeleteMultipleMidiNotesCommand>(clipId, std::move(noteIndices));
        aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
        aidaw::SelectionManager::getInstance().clearNoteSelection();
    };
    ...
    gridComponent_->onChordDropped = [](aidaw::ClipId clipId, double beat, double noteLength,
        ...
        auto* clipData = aidaw::ClipManager::getInstance().getClip(clipId);
        int groupId = clipData ? clipData->nextChordGroupId++ : 0;

        // Compound: MIDI notes + chord annotation undo as one step
        aidaw::CompoundOperationScope scope("Add Chord");

        std::vector<aidaw::MidiNote> midiNotes;
        ...
        auto noteCmd = std::make_unique<aidaw::AddMultipleMidiNotesCommand>(
            clipId, std::move(midiNotes), "Add chord notes");
        aidaw::UndoManager::getInstance().executeCommand(std::move(noteCmd));

        if (chordName.isNotEmpty()) {
            aidaw::ClipInfo::ChordAnnotation annotation;
            annotation.beatPosition = beat;
            annotation.lengthBeats = noteLength;
            annotation.chordName = chordName;
            annotation.chordGroup = groupId;
            auto chordCmd = std::make_unique<aidaw::AddChordAnnotationCommand>(clipId, annotation);
            aidaw::UndoManager::getInstance().executeCommand(std::move(chordCmd));
        }
    };
}
...
void PianoRollContent::updateGridSize() {
    auto& clipManager = aidaw::ClipManager::getInstance();
    const auto* clip =
        editingClipId_ != aidaw::INVALID_CLIP_ID ? clipManager.getClip(editingClipId_) : nullptr;
    ...
    if (selectedClipIds.size() > 1) {
        ...
        for (aidaw::ClipId id : selectedClipIds) {
        ...
        if (clip->loopEnabled || clip->view == aidaw::ClipView::Session) {
        ...
        if (editingClipId_ != aidaw::INVALID_CLIP_ID) {
            ...
            if (clip && clip->isMidi()) {
                ...
                if (relative) {
                    ...
                    std::vector<aidaw::ClipId> allMidiClips;
                    for (aidaw::ClipId id : allClipsOnTrack) {
                        auto* c = clipManager.getClip(id);
                        if (c && c->isMidi()) {
...
void PianoRollContent::onActivated() {
    ...
    aidaw::ClipId selectedClip = aidaw::ClipManager::getInstance().getSelectedClip();
    if (selectedClip != aidaw::INVALID_CLIP_ID) {
        const auto* clip = aidaw::ClipManager::getInstance().getClip(selectedClip);
        if (clip && clip->isMidi()) {
            ...
            bool forceRelative = (clip->view == aidaw::ClipView::Session) || clip->loopEnabled;
            if (forceRelative) {
                setRelativeTimeMode(true);
            }

            // Auto-show chord row if track has a chord engine
            if (!showChordRow_ && clip->trackId != aidaw::INVALID_TRACK_ID) {
                auto* trackInfo = aidaw::TrackManager::getInstance().getTrack(clip->trackId);
                if (trackInfo) {
                    for (const auto& elem : trackInfo->chainElements) {
                        if (aidaw::isDevice(elem)) {
                            const auto& dev = aidaw::getDevice(elem);
                            if (dev.pluginId.containsIgnoreCase(
                                    aidaw::daw::audio::MidiChordEnginePlugin::xmlTypeName)) {
...
void PianoRollContent::clipsChanged() {
    if (editingClipId_ != aidaw::INVALID_CLIP_ID) {
        ...
        const auto* clip = clipManager.getClip(editingClipId_);
        if (!clip) {
            gridComponent_->setClip(aidaw::INVALID_CLIP_ID);
            if (midiDrawer_)
                midiDrawer_->setClip(aidaw::INVALID_CLIP_ID);
            else if (velocityLane_)
                velocityLane_->setClip(aidaw::INVALID_CLIP_ID);
        } else {
    ...
    if (isDisplayed) {
        ...
        juce::MessageManager::callAsync([safeThis, clipId]() {
            if (auto* self = safeThis.getComponent()) {
                ...
                if (clip && clip->isMidi()) {
                    bool forceRelative =
                        (clip->view == aidaw::ClipView::Session) || clip->loopEnabled;
                    if (forceRelative) {
                ...
                if (clip && clip->midiNotes.empty() && !clip->chordAnnotations.empty()) {
                    aidaw::ClipManager::getInstance().clearChordAnnotations(clipId);
                }
...
void PianoRollContent::clipSelectionChanged(aidaw::ClipId clipId) {
    if (clipId == aidaw::INVALID_CLIP_ID) {
    ...
    if (clipId != aidaw::INVALID_CLIP_ID) {
        ...
        if (clip && clip->isMidi()) {
            ...
            bool forceRelative = (clip->view == aidaw::ClipView::Session);
            if (forceRelative) {
                setRelativeTimeMode(true);
            }
...
void PianoRollContent::clipDragPreview(aidaw::ClipId clipId, double previewStartTime,
...
void PianoRollContent::selectionTypeChanged(aidaw::SelectionType /*newType*/) {
    // Selection type changed - refresh the view
    repaint();
}

void PianoRollContent::multiClipSelectionChanged(const std::unordered_set<aidaw::ClipId>& clipIds) {
    ...
    aidaw::ClipId firstClipId = *clipIds.begin();
    const auto* firstClip = clipManager.getClip(firstClipId);
    if (!firstClip || !firstClip->isMidi()) {
        return;
    }

    aidaw::TrackId trackId = firstClip->trackId;

    // Filter selected clips to only MIDI clips on this track
    std::vector<aidaw::ClipId> selectedMidiClips;
    for (aidaw::ClipId id : clipIds) {
        auto* c = clipManager.getClip(id);
        if (c && c->isMidi() && c->trackId == trackId) {
            selectedMidiClips.push_back(id);
        }
    ...
    // Session clips are locked to relative mode
    bool forceRelative = (firstClip->view == aidaw::ClipView::Session);
    if (forceRelative) {
        setRelativeTimeMode(true);
    }
...
void PianoRollContent::noteSelectionChanged(const aidaw::NoteSelection& selection) {
...
void PianoRollContent::setClip(aidaw::ClipId clipId) {
...
void PianoRollContent::drawChordRow(juce::Graphics& g, juce::Rectangle<int> area) {
    ...
    const auto* clip = (editingClipId_ != aidaw::INVALID_CLIP_ID)
                           ? aidaw::ClipManager::getInstance().getClip(editingClipId_)
                           : nullptr;
    ...
    double clipStartBeats = 0.0;
    if (!relativeTimeMode_ && clip->view != aidaw::ClipView::Session) {
        clipStartBeats = clip->placement.startBeat;
    }
...
void PianoRollContent::syncChordAnnotations(aidaw::ClipId clipId) {
    if (isSyncingChords_)
        return;
    ...
    auto& engine = aidaw::music::ChordEngine::getInstance();
    ...
    for (auto it = clip->chordAnnotations.begin(); it != clip->chordAnnotations.end();) {
        ...
        std::vector<aidaw::music::ChordNote> chordNotes;
...
void PianoRollContent::detectChordsFromNotes() {
    ...
    auto* clip = aidaw::ClipManager::getInstance().getClip(editingClipId_);
    if (!clip || clip->midiNotes.empty())
        return;

    int beatsPerBar = aidaw::DEFAULT_TIME_SIGNATURE_NUMERATOR;
    if (auto* controller = aidaw::TimelineController::getCurrent())
    ...
    aidaw::CompoundOperationScope scope("Detect Chords");

    auto clearCmd = std::make_unique<aidaw::ClearChordAnnotationsCommand>(editingClipId_);
    aidaw::UndoManager::getInstance().executeCommand(std::move(clearCmd));
    ...
    for (size_t i = 0; i < detected.size(); ++i) {
        ...
        auto cmd = std::make_unique<aidaw::AddChordAnnotationCommand>(editingClipId_, annotation);
        aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
    ...
    if (!noteGroupAssignments.empty()) {
        auto groupCmd = std::make_unique<aidaw::SetNoteChordGroupsCommand>(
            editingClipId_, std::move(noteGroupAssignments));
        aidaw::UndoManager::getInstance().executeCommand(std::move(groupCmd));
    }
...
void PianoRollContent::drawVelocityHeader(juce::Graphics& g, juce::Rectangle<int> area) {
    ...
    g.setFont(aidaw::FontManager::getInstance().getUIFont(11.0f));
...
void PianoRollContent::updateVelocityLane() {
    ...
    if (midiDrawer_) {
        ...
        const auto& selectedClipIds =
            gridComponent_ ? gridComponent_->getSelectedClipIds() : std::vector<aidaw::ClipId>{};
        if (selectedClipIds.size() > 1) {
