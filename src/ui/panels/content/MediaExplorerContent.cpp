...
class MediaExplorerContent::PreviewAudioCallback : public juce::AudioIODeviceCallback {
  public:
    ...
    void audioDeviceIOCallbackWithContext(
        ...
        int offset = aidaw::Config::getInstance().getPreviewOutputChannel();
        if (offset < 0)
            offset = 0;
  ...
  public:
    ...
    void setFile(const juce::File& file) {
        ...
        if (file.existsAsFile()) {
            currentThumbnail_ =
                aidaw::AudioThumbnailManager::getInstance().getThumbnail(file.getFullPathName());
            if (currentThumbnail_ != nullptr) {
    ...
    void paint(juce::Graphics& g) override {
        ...
        if (currentFile_.existsAsFile()) {
            ...
            if (thumbnail != nullptr && thumbnail->getTotalLength() > 0.0) {
                ...
                aidaw::AudioThumbnailManager::getInstance().drawWaveform(
...
class FavoriteButton : public aidaw::SvgButton {
  public:
...
class MediaExplorerContent::SidebarComponent : public juce::Component {
  public:
    SidebarComponent() {
        // Setup icon buttons
        projectButton_ = std::make_unique<aidaw::SvgButton>("Project", BinaryData::project_home_svg,
        ...
        projectButton_->onClick = [this]() {
            auto& pm = aidaw::ProjectManager::getInstance();
        ...
        diskButton_ = std::make_unique<aidaw::SvgButton>("Disk", BinaryData::harddrive_svg,
        ...
        diskButton_->onClick = [this]() {
            ...
            if (onLocationSelected) {
                auto defaultDir = aidaw::Config::getInstance().getBrowserDefaultDirectory();
                if (!defaultDir.empty()) {
        ...
        // libraryButton_ = std::make_unique<aidaw::SvgButton>("Library", BinaryData::database_svg,
    ...
    void rebuildFavoriteButtons() {
        ...
        auto favorites = aidaw::Config::getInstance().getBrowserFavorites();
  ...
  private:
        ...
        menu.showMenuAsync(juce::PopupMenu::Options(), [this, path](int result) {
            if (result == 1) {
                ...
                favorites.erase(std::remove(favorites.begin(), favorites.end(), path),
                                favorites.end());
                aidaw::Config::getInstance().setBrowserFavorites(favorites);
                aidaw::Config::getInstance().save();
                rebuildFavoriteButtons();
            } else if (result == 2) {
                // Set as default
                aidaw::Config::getInstance().setBrowserDefaultDirectory(path);
                aidaw::Config::getInstance().save();
            }
        });
    }

    void selectButton(aidaw::SvgButton* selected) {
    ...
    std::unique_ptr<aidaw::SvgButton> projectButton_;
    std::unique_ptr<aidaw::SvgButton> diskButton_;
    // TODO: Library/DB button — uncomment when database feature is implemented
    // std::unique_ptr<aidaw::SvgButton> libraryButton_;
...
MediaExplorerContent::MediaExplorerContent() {
    ...
    audioFilterActive_ = aidaw::Config::getInstance().getBrowserFilterAudio();
    midiFilterActive_ = aidaw::Config::getInstance().getBrowserFilterMidi();

    auto activeBackground = DarkTheme::getColour(DarkTheme::ACCENT_BLUE).withAlpha(0.25f);

    audioFilterButton_ = std::make_unique<aidaw::SvgButton>("Audio", BinaryData::AUDIO_svg,
    ...
    audioFilterButton_->onClick = [this]() {
        ...
        aidaw::Config::getInstance().setBrowserFilterAudio(audioFilterActive_);
        aidaw::Config::getInstance().save();
        updateMediaFilter();
    };
    ...
    midiFilterButton_ =
        std::make_unique<aidaw::SvgButton>("MIDI", BinaryData::MIDI_svg, BinaryData::MIDI_svgSize);
    ...
    midiFilterButton_->onClick = [this]() {
        ...
        aidaw::Config::getInstance().setBrowserFilterMidi(midiFilterActive_);
        aidaw::Config::getInstance().save();
        updateMediaFilter();
    };
    ...
    playButton_ = std::make_unique<aidaw::SvgButton>(
    ...
    stopButton_ = std::make_unique<aidaw::SvgButton>(
...
void MediaExplorerContent::setAudioEngine(aidaw::AudioEngine* engine) {
...
void MediaExplorerContent::fileClicked(const juce::File& file, const juce::MouseEvent& e) {
    ...
    if (e.mods.isPopupMenu() && file.isDirectory()) {
        ...
        menu.showMenuAsync(juce::PopupMenu::Options(), [this, path](int result) {
            if (result == 1) {
                auto favs = aidaw::Config::getInstance().getBrowserFavorites();
                favs.erase(std::remove(favs.begin(), favs.end(), path), favs.end());
                aidaw::Config::getInstance().setBrowserFavorites(favs);
                aidaw::Config::getInstance().save();
