...
void ClipInspector::updateAudioSourceValueDisplays(const aidaw::ClipInfo& clip) {
    if (!clip.isAudio()) {
        return;
    }
    ...
    if (showAudioProps) {
        ...
        if (displayBPM <= 0.0 || (!clip.autoTempo && std::abs(displayBPM - projectBPM) < 0.1)) {
            displayBPM = aidaw::AudioThumbnailManager::getInstance().getCachedBPM(
                clip.audio().source.filePath);
        }
...
void ClipInspector::updateLoopValueDisplays(const aidaw::ClipInfo& clip, double projectBPM,
                                            int beatsPerBar) {
    ...
    double loopBpm = aidaw::isValidBpm(projectBPM) ? projectBPM : aidaw::DEFAULT_BPM;

    if (clip.isAudio()) {
        const auto info =
            aidaw::ClipDisplayInfo::from(clip, loopBpm, getAudioFileDurationForInspector(clip));
        const bool loopOn =
            clip.view == aidaw::ClipView::Session || clip.loopEnabled || clip.autoTempo;

        clipLoopStartValue_->setValue(
            aidaw::TimelineUtils::secondsToBeats(info.loopStartPositionSeconds, loopBpm),
            juce::dontSendNotification);
        clipLoopEndValue_->setValue(
            aidaw::TimelineUtils::secondsToBeats(info.loopEndPositionSeconds, loopBpm),
            juce::dontSendNotification);
        ...
        clipLoopPhaseValue_->setValue(
            aidaw::TimelineUtils::secondsToBeats(juce::jmax(0.0, displayPositionSeconds), loopBpm),
            juce::dontSendNotification);
        return;
    }

    const double loopStartBeats = aidaw::TimelineUtils::secondsToBeats(clip.loopStart, loopBpm);
    ...
    if (clip.autoTempo && clip.loopLengthBeats > 0.0) {
        ...
        loopLengthDisplayBeats = aidaw::TimelineUtils::secondsToBeats(sourceLength, loopBpm);
    }
    clipLoopEndValue_->setValue(loopStartBeats + loopLengthDisplayBeats,
                                juce::dontSendNotification);

    const bool loopOn = clip.view == aidaw::ClipView::Session || clip.loopEnabled || clip.autoTempo;
    clipLoopPhaseLabel_.setText(loopOn ? "phase" : "offset", juce::dontSendNotification);
    ...
    if (clip.isMidi()) {
        ...
        const double sourcePositionBeats =
            aidaw::TimelineUtils::secondsToBeats(juce::jmax(0.0, sourcePositionSeconds), loopBpm);
...
void ClipInspector::updateFromSelectedClip() {
    ...
    if (!isMulti) {
        auto* mutableClip = aidaw::ClipManager::getInstance().getClip(pid);
    ...
    const auto* clip = aidaw::ClipManager::getInstance().getClip(pid);
    if (clip) {
        ...
        if (clip->isAudio() && clip->audio().source.filePath.isNotEmpty() &&
            clip->view != aidaw::ClipView::Session && !isMulti) {
            ...
            if (displayBPM <= 0.0 ||
                ...
                if (displayBPM <= 0.0) {
                    ...
                    thumbs.requestBPMDetection(clip->audio().source.filePath, [cid](double bpm) {
                        if (bpm <= 0.0)
                            return;
                        aidaw::ClipManager::getInstance().forceNotifyClipPropertyChanged(cid);
                    });
                }
            }
        ...
        bool isSessionClip = (clip->view == aidaw::ClipView::Session);
        ...
        const bool showFollowControls =
            isSessionClip && clip->followAction != aidaw::FollowAction::None;
...
void ClipInspector::showClipControls(bool show) {
    ...
    if (!show) {
        ...
        bool isSession = clip && clip->view == aidaw::ClipView::Session;
...
void ClipInspector::computeClipRange() {
    ...
    for (auto cid : selectedClipIds_) {
        ...
        if (c->view != aidaw::ClipView::Arrangement)
            clipRange_.allArrangement = false;
        if (c->view != aidaw::ClipView::Session)
