...
void ClipFadesSection::initControls() {
    ...
    fadeInValue_ =
        std::make_unique<aidaw::DraggableValueLabel>(aidaw::DraggableValueLabel::Format::Raw);
    ...
    fadeInValue_->onValueChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Fade In", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isAudio() && c->view != aidaw::ClipView::Session) {
                double newVal = juce::jmax(0.0, c->fadeIn + delta);
                batch.execute(std::make_unique<aidaw::SetClipFadeInCommand>(cid, newVal));
            }
        }
    ...
    fadeOutValue_->onValueChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Fade Out", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isAudio() && c->view != aidaw::ClipView::Session) {
                double newVal = juce::jmax(0.0, c->fadeOut + delta);
                batch.execute(std::make_unique<aidaw::SetClipFadeOutCommand>(cid, newVal));
            }
        }
    ...
    auto setupTypeBtn = [this](std::unique_ptr<aidaw::SvgButton>& btn, const FadeTypeIcon& icon) {
        btn = std::make_unique<aidaw::SvgButton>(icon.name, icon.data, icon.size);
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        fadeInTypeButtons_[i]->onClick = [this, i, fadeType]() {
            aidaw::ClipBatchEdit batch("Set Clip Fade In Type", selectedClipIds_.size());
            for (auto cid : selectedClipIds_)
                batch.execute(std::make_unique<aidaw::SetClipFadeInTypeCommand>(cid, fadeType));
            for (int j = 0; j < 4; ++j)
        ...
        fadeOutTypeButtons_[i]->onClick = [this, i, fadeType]() {
            aidaw::ClipBatchEdit batch("Set Clip Fade Out Type", selectedClipIds_.size());
            for (auto cid : selectedClipIds_)
                batch.execute(std::make_unique<aidaw::SetClipFadeOutTypeCommand>(cid, fadeType));
            for (int j = 0; j < 4; ++j)
    ...
    auto setupBehaviourBtn = [this](std::unique_ptr<aidaw::SvgButton>& btn,
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        fadeInBehaviourButtons_[i]->onClick = [this, i]() {
            aidaw::ClipBatchEdit batch("Set Clip Fade In Behaviour", selectedClipIds_.size());
            for (auto cid : selectedClipIds_)
                batch.execute(std::make_unique<aidaw::SetClipFadeInBehaviourCommand>(cid, i));
            for (int j = 0; j < 2; ++j)
        ...
        fadeOutBehaviourButtons_[i]->onClick = [this, i]() {
            aidaw::ClipBatchEdit batch("Set Clip Fade Out Behaviour", selectedClipIds_.size());
            for (auto cid : selectedClipIds_)
                batch.execute(std::make_unique<aidaw::SetClipFadeOutBehaviourCommand>(cid, i));
            for (int j = 0; j < 2; ++j)
    ...
    autoCrossfadeToggle_.onClick = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Auto Crossfade", selectedClipIds_.size());
        for (auto cid : selectedClipIds_)
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Auto Crossfade", [newState](auto& manager, aidaw::ClipId id) {
    ...
    launchFadeValue_->onValueChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Launch Fade", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isAudio() && c->view == aidaw::ClipView::Session)
                batch.execute(
                    std::make_unique<aidaw::SetClipLaunchFadeSamplesCommand>(cid, samples));
        }
    };
...
void ClipFadesSection::setClip(aidaw::ClipId clipId) {
    std::unordered_set<aidaw::ClipId> s;
    if (clipId != aidaw::INVALID_CLIP_ID)
        s.insert(clipId);
    setSelectedClips(s);
}

void ClipFadesSection::setSelectedClips(const std::unordered_set<aidaw::ClipId>& clipIds) {
...
void ClipFadesSection::update() {
    ...
    bool isSession = (clip->view == aidaw::ClipView::Session);
