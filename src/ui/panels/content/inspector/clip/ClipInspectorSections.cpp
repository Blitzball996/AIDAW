...
double getAudioFileDurationForInspector(const aidaw::ClipInfo& clip) {
    if (!clip.isAudio() || clip.audio().source.filePath.isEmpty())
        return 0.0;

    if (auto* thumbnail = aidaw::AudioThumbnailManager::getInstance().getThumbnail(
...
double displayBeatsToAudioSourceSeconds(const aidaw::ClipInfo& clip, double displayBeats,
                                        double projectBpm) {
    const double bpm = projectBpm > 0.0 ? projectBpm : 120.0;
    const auto info =
        aidaw::ClipDisplayInfo::from(clip, bpm, getAudioFileDurationForInspector(clip));
    const double displaySeconds = aidaw::TimelineUtils::beatsToSeconds(displayBeats, bpm);
    return info.timelineToSource(displaySeconds);
}

double timelineStartSeconds(const aidaw::ClipInfo& clip, double bpm) {
    return clip.getTimelineStart(bpm);
}

double timelineLengthSeconds(const aidaw::ClipInfo& clip, double bpm) {
...
void ClipInspector::initClipPropertiesSection() {
    ...
    clipNameValue_.onTextChange = [this]() {
        if (primaryClipId() != aidaw::INVALID_CLIP_ID) {
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetClipNameCommand>(primaryClipId(),
    ...
    // Colour swatch
    colourSwatch_ = std::make_unique<aidaw::ColourSwatch>();
    auto* swatch = static_cast<aidaw::ColourSwatch*>(colourSwatch_.get());
    swatch->onColourClicked = [this, swatch]() {
        auto pid = primaryClipId();
        if (pid == aidaw::INVALID_CLIP_ID)
            return;
        ...
        // Default colours
        for (size_t i = 0; i < aidaw::Config::defaultColourPalette.size(); ++i) {
            auto colour = juce::Colour(aidaw::Config::defaultColourPalette[i].colour);
            menu.addItem(static_cast<int>(i + 3), aidaw::Config::defaultColourPalette[i].name, true,
                         false, makeChip(colour));
        }

        // Custom colours from Config
        const auto customPalette = aidaw::Config::getInstance().getTrackColourPalette();
        const int customOffset = static_cast<int>(aidaw::Config::defaultColourPalette.size()) + 3;
        ...
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(swatch), [this, swatch,
            ...
            if (pid == aidaw::INVALID_CLIP_ID)
                return;
            const int customOff = static_cast<int>(aidaw::Config::defaultColourPalette.size()) + 3;

            if (result == 1) {
                // "None"
                swatch->clearColour();
                aidaw::ClipBatchEdit batch("Set Clip Colour", selectedClipIds_.size());
                for (auto cid : selectedClipIds_) {
                    batch.execute(std::make_unique<aidaw::SetClipColourCommand>(
                ...
                aidaw::ClipBatchEdit batch("Set Clip Colour", selectedClipIds_.size());
                for (auto cid : selectedClipIds_) {
                    const auto* clip = aidaw::ClipManager::getInstance().getClip(cid);
                    if (clip) {
                        const auto* track =
                            aidaw::TrackManager::getInstance().getTrack(clip->trackId);
                        if (track) {
                            if (cid == pid)
                                swatch->setColour(track->colour);
                            batch.execute(
                                std::make_unique<aidaw::SetClipColourCommand>(cid, track->colour));
                        }
                    }
                }
            } else if (result >= 3 && result < customOff) {
                auto colour = juce::Colour(aidaw::Config::getDefaultColour(result - 3));
                swatch->setColour(colour);
                aidaw::ClipBatchEdit batch("Set Clip Colour", selectedClipIds_.size());
                for (auto cid : selectedClipIds_) {
                    batch.execute(std::make_unique<aidaw::SetClipColourCommand>(cid, colour));
                }
            } else {
    ...
    clipTypeIcon_ = std::make_unique<aidaw::SvgButton>("Type", BinaryData::audio_clip_svg,
    ...
    clipViewIcon_ = std::make_unique<aidaw::SvgButton>("View", BinaryData::Arrangement_svg,
    ...
    clipBpmValue_.onTextChange = [this]() {
        auto* clip = aidaw::ClipManager::getInstance().getClip(primaryClipId());
        ...
        if (auto* thumb = aidaw::AudioThumbnailManager::getInstance().getThumbnail(
                clip->audio().source.filePath)) {
        ...
        if (clip->autoTempo) {
            aidaw::ClipManager::AudioClipBeatsUpdate u;
            ...
            aidaw::ClipManager::getInstance().applyAudioClipBeats(primaryClipId(), u, bpm);
        } else {
            ...
            aidaw::ClipManager::getInstance().forceNotifyClipPropertyChanged(primaryClipId());
        }
    ...
    clipBeatsLengthValue_->onValueChange = [this]() {
        if (primaryClipId() != aidaw::INVALID_CLIP_ID) {
            ...
            if (clip && clip->autoTempo) {
                ...
                aidaw::ClipManager::getInstance().applyAudioClipBeats(primaryClipId(), u,
                                                                      projectBpm);
            }
        }
    };
    ...
    clipPositionIcon_ = std::make_unique<aidaw::SvgButton>("Position", BinaryData::position_svg,
    ...
    clipStartValue_ = std::make_unique<aidaw::BarsBeatsTicksLabel>();
    ...
    clipStartValue_->onValueChange = [this]() {
        ...
        double deltaSeconds = aidaw::TimelineUtils::beatsToSeconds(deltaBeats, bpm);
        aidaw::ClipBatchEdit batch("Move Clips", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->view != aidaw::ClipView::Session) {
                double newStart = juce::jmax(0.0, timelineStartSeconds(*c, bpm) + deltaSeconds);
                batch.execute(std::make_unique<aidaw::MoveClipCommand>(
                    cid, aidaw::BeatPosition{newStart * bpm / 60.0}, bpm));
            }
        }
    ...
    clipEndValue_ = std::make_unique<aidaw::BarsBeatsTicksLabel>();
    ...
    clipEndValue_->onValueChange = [this]() {
        ...
        double deltaSeconds = aidaw::TimelineUtils::beatsToSeconds(deltaBeats, bpm);
        aidaw::ClipBatchEdit batch("Resize Clips", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->view != aidaw::ClipView::Session) {
                double newLength = juce::jmax(0.0, timelineLengthSeconds(*c, bpm) + deltaSeconds);
                batch.execute(std::make_unique<aidaw::ResizeClipCommand>(
                    cid, aidaw::BeatDuration{newLength * bpm / 60.0}, false, bpm));
            }
        }
    ...
    clipLengthValue_ = std::make_unique<aidaw::BarsBeatsTicksLabel>();
    ...
    clipLengthValue_->onValueChange = [this]() {
        ...
        const double deltaSeconds = aidaw::TimelineUtils::beatsToSeconds(deltaBeats, bpm);
        aidaw::ClipBatchEdit batch("Resize Clips", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->view != aidaw::ClipView::Session) {
                const double newLength = juce::jmax(aidaw::ClipOperations::MIN_CLIP_LENGTH,
    ...
    clipLoopToggle_ = std::make_unique<aidaw::SvgButton>(
    ...
    clipLoopToggle_->onClick = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Loop", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Loop", [newState, bpm](auto& manager, aidaw::ClipId id) {
    ...
    clipWarpToggle_.onClick = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Warp", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Warp", [newState](auto& manager, aidaw::ClipId id) {
    ...
    auto getSelectedAudioClipIds = [this]() {
        std::vector<aidaw::ClipId> clipIds;
        for (auto cid : selectedClipIds_) {
            const auto* clip = aidaw::ClipManager::getInstance().getClip(cid);
            if (clip && clip->isAudio())
    ...
    auto seedSourceInterpretation = [](aidaw::ClipId cid, auto& clip, double bpm) {
        ...
        const bool sourceInterpretationBpmLooksDefaulted =
            ...
            (aidaw::isValidBpm(bpm) && std::abs(clip.audio().interpretation.bpm - bpm) < 0.1);
        ...
        auto& thumbs = aidaw::AudioThumbnailManager::getInstance();
        ...
        if (cached > 0.0) {
            ...
            thumbs.requestBPMDetection(clip.audio().source.filePath, [cid](double detectedBPM) {
                if (detectedBPM <= 0.0)
                    return;
                auto& mgr = aidaw::ClipManager::getInstance();
                ...
                double live = aidaw::ProjectManager::getInstance().getCurrentProjectInfo().tempo;
                bool existingLooksDefaulted = c->audio().interpretation.bpm > 0.0 &&
                                              aidaw::isValidBpm(live) &&
    ...
    auto applyAutoTempo =
        [this, seedSourceInterpretation](bool enable, const std::vector<aidaw::ClipId>& clipIds) {
            double bpm = 120.0;
            if (timelineController_) {
                bpm = timelineController_->getState().tempo.bpm;
            }

            aidaw::ClipBatchEdit batch("Set Clip Beat Mode", clipIds.size());
            for (auto cid : clipIds) {
                auto* clip = aidaw::ClipManager::getInstance().getClip(cid);
                if (!clip || !clip->isAudio())
                    continue;
                batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                    cid, "Set Clip Beat Mode",
                    [enable, bpm, seedSourceInterpretation](auto& manager, aidaw::ClipId id) {
    ...
    clipAutoTempoToggle_.onClick = [this, getSelectedAudioClipIds, applyAutoTempo]() {
        ...
        for (auto cid : targetClipIds) {
            const auto* selectedClip = aidaw::ClipManager::getInstance().getClip(cid);
    ...
    clipStretchValue_->onValueChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Speed", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isAudio()) {
                double newVal = juce::jlimit(0.25, 4.0, c->speedRatio + delta);
                batch.execute(std::make_unique<aidaw::SetClipSpeedRatioCommand>(cid, newVal));
            }
        }
    ...
    stretchModeCombo_.onChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Stretch Mode", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipStretchModeCommand>(cid, mode));
        }
    };
    ...
    clipLoopStartValue_ = std::make_unique<aidaw::BarsBeatsTicksLabel>();
    ...
    clipLoopStartValue_->onValueChange = [this]() {
        if (primaryClipId() == aidaw::INVALID_CLIP_ID)
            return;
        const auto* clip = aidaw::ClipManager::getInstance().getClip(primaryClipId());
        if (!clip)
            return;
        ...
        double newLoopStartSeconds =
            clip->isAudio() ? displayBeatsToAudioSourceSeconds(*clip, newLoopStartBeats, bpm)
                            : aidaw::TimelineUtils::beatsToSeconds(newLoopStartBeats, bpm);
        ...
        aidaw::UndoManager::getInstance().beginCompoundOperation("Set Clip Loop Start");
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetClipLoopStartCommand>(primaryClipId(), newLoopStartSeconds,
                                                             bpm));
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetClipOffsetCommand>(primaryClipId(), newOffset));
        aidaw::UndoManager::getInstance().endCompoundOperation();
    };
    ...
    clipLoopEndValue_ = std::make_unique<aidaw::BarsBeatsTicksLabel>();
    ...
    clipLoopEndValue_->onValueChange = [this]() {
        ...
        if (clip->isAudio()) {
            ...
            double loopStartBeats = aidaw::TimelineUtils::secondsToBeats(clip->loopStart, bpm);
            double newLoopLengthBeats = newLoopEndBeats - loopStartBeats;
            if (newLoopLengthBeats < 0.25)
                newLoopLengthBeats = 0.25;
            newLoopLengthSeconds = aidaw::TimelineUtils::beatsToSeconds(newLoopLengthBeats, bpm);
        }
        newLoopLengthSeconds =
            juce::jmax(aidaw::ClipOperations::MIN_SOURCE_LENGTH, newLoopLengthSeconds);

        bool shouldResizeClip = false;
        double resizeLengthSeconds = 0.0;

        if (clip->view == aidaw::ClipView::Session) {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Loop End", shouldResizeClip ? 2u : 1u);
        if (shouldResizeClip) {
            batch.execute(std::make_unique<aidaw::ResizeClipCommand>(
                primaryClipId(), aidaw::BeatDuration{resizeLengthSeconds * bpm / 60.0}, false,
                bpm));
        }

        batch.execute(std::make_unique<aidaw::SetClipLoopLengthCommand>(primaryClipId(),
    ...
    clipLoopPhaseValue_ = std::make_unique<aidaw::BarsBeatsTicksLabel>();
    ...
    clipLoopPhaseValue_->onValueChange = [this]() {
        ...
        const bool loopOn =
            clip->view == aidaw::ClipView::Session || clip->loopEnabled || clip->autoTempo;
        if (clip->isMidi()) {
            // MIDI phase/offset lives in midiOffset (beats)
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetClipOffsetCommand>(primaryClipId(),
            ...
            if (loopOn) {
                aidaw::UndoManager::getInstance().executeCommand(
                    std::make_unique<aidaw::SetClipLoopPhaseCommand>(primaryClipId(), newSeconds));
            } else {
                aidaw::UndoManager::getInstance().executeCommand(
                    std::make_unique<aidaw::SetClipOffsetCommand>(primaryClipId(), newSeconds));
            }
        }
    };
...
void ClipInspector::initSessionLaunchSection() {
    ...
    launchModeCombo_.onChange = [this]() {
        if (selectedClipIds_.empty())
            return;
        auto mode = static_cast<aidaw::LaunchMode>(launchModeCombo_.getSelectedId() - 1);
        aidaw::ClipBatchEdit batch("Set Clip Launch Mode", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Launch Mode",
                [mode](auto& manager, aidaw::ClipId id) { manager.setClipLaunchMode(id, mode); }));
        }
    };
    ...
    launchQuantizeCombo_.onChange = [this]() {
        if (selectedClipIds_.empty())
            return;
        auto quantize =
            static_cast<aidaw::LaunchQuantize>(launchQuantizeCombo_.getSelectedId() - 1);
        aidaw::ClipBatchEdit batch("Set Clip Launch Quantize", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Launch Quantize", [quantize](auto& manager, aidaw::ClipId id) {
    ...
    followActionCombo_.onChange = [this]() {
        if (selectedClipIds_.empty())
            return;
        auto action = static_cast<aidaw::FollowAction>(followActionCombo_.getSelectedId() - 1);
        aidaw::ClipBatchEdit batch("Set Clip Follow Action", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Follow Action", [action](auto& manager, aidaw::ClipId id) {
    ...
    followActionDelaySlider_.onValueChanged = [this](double delayBeats) {
        if (selectedClipIds_.empty())
            return;
        aidaw::ClipBatchEdit batch("Set Clip Follow Delay", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Follow Delay", [delayBeats](auto& manager, aidaw::ClipId id) {
    ...
    followActionLoopCountSlider_.onValueChanged = [this](double value) {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Follow Loops", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Follow Loops", [loopCount](auto& manager, aidaw::ClipId id) {
...
void ClipInspector::initPitchSection() {
    ...
    autoPitchToggle_.onClick = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Auto Pitch", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Auto Pitch", [newState](auto& manager, aidaw::ClipId id) {
    ...
    analogPitchToggle_.onClick = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Analog Pitch", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Analog Pitch", [newState](auto& manager, aidaw::ClipId id) {
    ...
    autoPitchModeCombo_.onChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Auto Pitch Mode", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Auto Pitch Mode",
                [mode](auto& manager, aidaw::ClipId id) { manager.setAutoPitchMode(id, mode); }));
        }
    };
    ...
    auto transposeAction = [this](int direction, bool shift) {
        ...
        aidaw::ClipBatchEdit batch("Transpose MIDI Clips", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isMidi()) {
                batch.execute(std::make_unique<aidaw::TransposeMidiClipCommand>(cid, semitones));
            }
        }
    };
    ...
    pitchChangeValue_->onValueChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Pitch", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isAudio()) {
                float newVal =
                    juce::jlimit(-48.0f, 48.0f, c->pitchChange + static_cast<float>(delta));
                batch.execute(std::make_unique<aidaw::SetClipPitchCommand>(cid, newVal));
            }
        }
...
void ClipInspector::initGrooveSection() {
    ...
    grooveStrengthValue_->onValueChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Groove Strength", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isMidi()) {
                batch.execute(
                    std::make_unique<aidaw::SetClipGrooveStrengthCommand>(cid, newStrength));
            }
        }
    };
...
void ClipInspector::showGroovePicker() {
    ...
    if (!selectedClipIds_.empty()) {
        auto* c = aidaw::ClipManager::getInstance().getClip(*selectedClipIds_.begin());
        if (c)
            currentTemplate = c->grooveTemplate;
    }

    auto* audioEngine = aidaw::TrackManager::getInstance().getAudioEngine();
    auto* teWrapper = dynamic_cast<aidaw::TracktionEngineWrapper*>(audioEngine);
...
void ClipInspector::onGrooveTemplateSelected(const juce::String& templateName) {
    ...
    aidaw::ClipBatchEdit batch("Set Clip Groove Template", selectedClipIds_.size());
    for (auto cid : selectedClipIds_) {
        const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
        if (c && c->isMidi()) {
            batch.execute(std::make_unique<aidaw::SetClipGrooveTemplateCommand>(cid, name));
        }
    }
...
void ClipInspector::initMixSection() {
    ...
    clipVolumeValue_->onValueChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Volume", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isAudio()) {
                float newVal = juce::jlimit(-100.0f, 0.0f, c->volumeDB + static_cast<float>(delta));
                batch.execute(std::make_unique<aidaw::SetClipVolumeDBCommand>(cid, newVal));
            }
        }
    ...
    clipPanValue_->onValueChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Pan", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isAudio()) {
                float newVal = juce::jlimit(-1.0f, 1.0f, c->pan + static_cast<float>(delta));
                batch.execute(std::make_unique<aidaw::SetClipPanCommand>(cid, newVal));
            }
        }
    ...
    clipGainValue_->onValueChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Gain", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isAudio()) {
                float newVal = juce::jlimit(0.0f, 24.0f, c->gainDB + static_cast<float>(delta));
                batch.execute(std::make_unique<aidaw::SetClipGainDBCommand>(cid, newVal));
            }
        }
...
void ClipInspector::initPlaybackSection() {
    ...
    reverseToggle_.onClick = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Reverse", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipReversedCommand>(cid, newState));
        }
    };
    ...
    autoDetectBeatsToggle_.onClick = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Auto Detect Beats", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Auto Detect Beats", [newState](auto& manager, aidaw::ClipId id) {
    ...
    beatSensitivityValue_->onValueChange = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Beat Sensitivity", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            const auto* c = aidaw::ClipManager::getInstance().getClip(cid);
            if (c && c->isAudio()) {
                batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                    cid, "Set Clip Beat Sensitivity",
                    [sensitivity](auto& manager, aidaw::ClipId id) {
...
void ClipInspector::initChannelsSection() {
    ...
    leftChannelToggle_.onClick = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Left Channel", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Left Channel", [newState](auto& manager, aidaw::ClipId id) {
    ...
    rightChannelToggle_.onClick = [this]() {
        ...
        aidaw::ClipBatchEdit batch("Set Clip Right Channel", selectedClipIds_.size());
        for (auto cid : selectedClipIds_) {
            batch.execute(std::make_unique<aidaw::SetClipPropertyCommand>(
                cid, "Set Clip Right Channel", [newState](auto& manager, aidaw::ClipId id) {
