 ...
 */
class NoteInspector : public BaseInspector, private aidaw::ClipManagerListener {
  public:
    NoteInspector();
     ...
     */
    void setSelectedNotes(const aidaw::NoteSelection& selection);

  private:
    // Current selection
    aidaw::NoteSelection noteSelection_;

    // Note properties
    juce::Label noteCountLabel_;
    juce::Label notePitchLabel_;
    std::unique_ptr<aidaw::DraggableValueLabel> notePitchValue_;
    juce::Label noteVelocityLabel_;
    std::unique_ptr<aidaw::DraggableValueLabel> noteVelocityValue_;
    juce::Label noteStartLabel_;
    std::unique_ptr<aidaw::DraggableValueLabel> noteStartValue_;
    juce::Label noteLengthLabel_;
    std::unique_ptr<aidaw::DraggableValueLabel> noteLengthValue_;
