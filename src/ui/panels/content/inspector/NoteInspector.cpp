...
NoteInspector::NoteInspector() {
    ...
    notePitchValue_ =
        std::make_unique<aidaw::DraggableValueLabel>(aidaw::DraggableValueLabel::Format::MidiNote);
    ...
    notePitchValue_->onValueChange = [this]() {
        ...
        const auto* clip = aidaw::ClipManager::getInstance().getClip(noteSelection_.clipId);
        if (!clip)
            return;

        std::vector<aidaw::MoveMultipleMidiNotesCommand::NoteMove> moves;
        ...
        if (!moves.empty()) {
            auto cmd = std::make_unique<aidaw::MoveMultipleMidiNotesCommand>(noteSelection_.clipId,
                                                                             std::move(moves));
    ...
    noteStartValue_ =
        std::make_unique<aidaw::DraggableValueLabel>(aidaw::DraggableValueLabel::Format::BarsBeats);
...
void NoteInspector::setSelectedNotes(const aidaw::NoteSelection& selection) {
...
void NoteInspector::clipPropertyChanged(aidaw::ClipId clipId) {
