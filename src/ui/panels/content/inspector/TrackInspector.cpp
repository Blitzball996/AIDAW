...
TrackInspector::TrackInspector() {
    ...
    trackNameValue_.onTextChange = [this]() {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetTrackNameCommand>(selectedTrackId_,
    ...
    swatch->onColourClicked = [this, swatch]() {
        if (selectedTrackId_ == aidaw::INVALID_TRACK_ID && selectedTrackIds_.empty())
            return;
        ...
        for (size_t i = 0; i < aidaw::Config::defaultColourPalette.size(); ++i) {
            ...
            menu.addItem(static_cast<int>(i + 2), aidaw::Config::defaultColourPalette[i].name, true,
        ...
        const int customOffset = static_cast<int>(aidaw::Config::defaultColourPalette.size()) + 2;
        ...
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(swatch), [this, swatch,
            ...
            if (result == 0)
                return;
            const int customOff = static_cast<int>(aidaw::Config::defaultColourPalette.size()) + 2;
            auto trackIds = selectedTrackIds_.empty()
                                ? std::unordered_set<aidaw::TrackId>{selectedTrackId_}
                                : selectedTrackIds_;
            if (result == 1) {
                // "None"
                swatch->clearColour();
                for (auto tid : trackIds) {
                    aidaw::UndoManager::getInstance().executeCommand(
                        std::make_unique<aidaw::SetTrackColourCommand>(tid,
                                                                       juce::Colour(0xFF444444)));
                }
            } else if (result >= 2 && result < customOff) {
                // Default colour
                auto colour = juce::Colour(aidaw::Config::getDefaultColour(result - 2));
                swatch->setColour(colour);
                for (auto tid : trackIds) {
                    aidaw::UndoManager::getInstance().executeCommand(
                        std::make_unique<aidaw::SetTrackColourCommand>(tid, colour));
                }
            } else {
                // Custom colour
    ...
    muteButton_.setLookAndFeel(&aidaw::daw::ui::SmallButtonLookAndFeel::getInstance());
    ...
    muteButton_.onClick = [this]() {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            if (selectedTrackId_ == aidaw::MASTER_TRACK_ID)
                aidaw::UndoManager::getInstance().executeCommand(
                    std::make_unique<aidaw::SetMasterMuteCommand>(muteButton_.getToggleState()));
            else
                aidaw::UndoManager::getInstance().executeCommand(
    ...
    speakerButton_->onClick = [this]() {
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetMasterMuteCommand>(speakerButton_->getToggleState()));
    };
    addChildComponent(*speakerButton_);  // Hidden by default

    // Solo button (TCP style)
    soloButton_.setButtonText("S");
    soloButton_.setLookAndFeel(&aidaw::daw::ui::SmallButtonLookAndFeel::getInstance());
    ...
    recordButton_.setLookAndFeel(&aidaw::daw::ui::SmallButtonLookAndFeel::getInstance());
    ...
    recordButton_.onClick = [this]() {
        ...
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            aidaw::TrackManager::getInstance().setTrackRecordArmed(selectedTrackId_,
    ...
    monitorButton_.setLookAndFeel(&aidaw::daw::ui::SmallButtonLookAndFeel::getInstance());
    ...
    monitorButton_.onClick = [this]() {
        if (selectedTrackId_ == aidaw::INVALID_TRACK_ID ||
            selectedTrackId_ == aidaw::MASTER_TRACK_ID)
            return;
        auto* track = aidaw::TrackManager::getInstance().getTrack(selectedTrackId_);
        if (!track)
            return;
        aidaw::InputMonitorMode nextMode;
        switch (track->inputMonitor) {
            case aidaw::InputMonitorMode::Off:
                nextMode = aidaw::InputMonitorMode::In;
                break;
            case aidaw::InputMonitorMode::In:
                nextMode = aidaw::InputMonitorMode::Auto;
                break;
            case aidaw::InputMonitorMode::Auto:
                nextMode = aidaw::InputMonitorMode::Off;
                break;
        }
        aidaw::UndoManager::getInstance().executeCommand(
            std::make_unique<aidaw::SetTrackInputMonitorCommand>(selectedTrackId_, nextMode));
    };
    addAndMakeVisible(monitorButton_);

    // Gain label (TCP style - draggable dB display)
    gainLabel_ =
        std::make_unique<aidaw::DraggableValueLabel>(aidaw::DraggableValueLabel::Format::Decibels);
    ...
    gainLabel_->onValueChange = [this]() {
        if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
            ...
            if (selectedTrackId_ == aidaw::MASTER_TRACK_ID)
                aidaw::UndoManager::getInstance().executeCommand(
                    std::make_unique<aidaw::SetMasterVolumeCommand>(gain));
            else
                aidaw::UndoManager::getInstance().executeCommand(
    ...
    panLabel_ =
        std::make_unique<aidaw::DraggableValueLabel>(aidaw::DraggableValueLabel::Format::Pan);
    ...
    panLabel_->onValueChange = [this]() {
        ...
        if (panLabel_->isDragging()) {
            ...
            if (selectedTrackId_ == aidaw::MASTER_TRACK_ID)
                aidaw::TrackManager::getInstance().setMasterPan(pan);
            else
                aidaw::TrackManager::getInstance().setTrackPan(selectedTrackId_, pan);
        } else {
            // Non-drag changes (keyboard edit, double-click reset)
            if (selectedTrackId_ == aidaw::MASTER_TRACK_ID)
                aidaw::UndoManager::getInstance().executeCommand(
                    std::make_unique<aidaw::SetMasterPanCommand>(pan));
            else
                aidaw::UndoManager::getInstance().executeCommand(
                    std::make_unique<aidaw::SetTrackPanCommand>(selectedTrackId_, pan));
        }
    };
    panLabel_->onDragEnd = [this](double startValue) {
        ...
        if (selectedTrackId_ == aidaw::MASTER_TRACK_ID)
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetMasterPanCommand>(oldPan, newPan));
        else
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetTrackPanCommand>(selectedTrackId_, oldPan, newPan));
    };
    ...
    inputTypeSelector_ = std::make_unique<aidaw::InputTypeSelector>();

    // Audio input selector
    audioInputSelector_ =
        std::make_unique<aidaw::RoutingSelector>(aidaw::RoutingSelector::Type::AudioIn);
    ...
    // MIDI input selector
    inputSelector_ = std::make_unique<aidaw::RoutingSelector>(aidaw::RoutingSelector::Type::MidiIn);
    addAndMakeVisible(*inputSelector_);

    // Audio output selector
    outputSelector_ =
        std::make_unique<aidaw::RoutingSelector>(aidaw::RoutingSelector::Type::AudioOut);
    addAndMakeVisible(*outputSelector_);

    // MIDI output selector
    midiOutputSelector_ =
        std::make_unique<aidaw::RoutingSelector>(aidaw::RoutingSelector::Type::MidiOut);
...
void TrackInspector::setSelectedTrack(aidaw::TrackId trackId) {
    ...
    if (wasMulti) {
        muteButton_.onClick = [this]() {
            if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
                if (selectedTrackId_ == aidaw::MASTER_TRACK_ID)
                    aidaw::UndoManager::getInstance().executeCommand(
                        std::make_unique<aidaw::SetMasterMuteCommand>(
                            muteButton_.getToggleState()));
                else
                    aidaw::UndoManager::getInstance().executeCommand(
                        std::make_unique<aidaw::SetTrackMuteCommand>(selectedTrackId_,
        ...
        recordButton_.onClick = [this]() {
            if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
                aidaw::TrackManager::getInstance().setTrackRecordArmed(
...
void TrackInspector::setSelectedTracks(const std::unordered_set<aidaw::TrackId>& trackIds) {
...
void TrackInspector::trackPropertyChanged(int trackId) {
    if (isMultiTrackMode_) {
        ...
        if (gainLabel_->isDragging() || panLabel_->isDragging())
            return;
        if (selectedTrackIds_.count(static_cast<aidaw::TrackId>(trackId)) > 0) {
            updateFromMultiTrackSelection();
        }
        return;
    }
...
void TrackInspector::trackDevicesChanged(aidaw::TrackId trackId) {
    if (trackId == selectedTrackId_) {
        ...
        if (latencyLabel_.isVisible()) {
            double latency =
                aidaw::TrackManager::getInstance().getTrackLatencySeconds(selectedTrackId_);
...
void TrackInspector::trackSelectionChanged(aidaw::TrackId trackId) {
...
void TrackInspector::deviceParameterChanged(aidaw::DeviceId deviceId, int paramIndex,
                                            float newValue) {
...
void TrackInspector::updateFromSelectedTrack() {
    ...
    if (selectedTrackId_ == aidaw::MASTER_TRACK_ID) {
        const auto& master = aidaw::TrackManager::getInstance().getMasterChannel();
    ...
    if (track) {
        ...
        monitorButton_.setToggleState(track->inputMonitor != aidaw::InputMonitorMode::Off,
                                      juce::dontSendNotification);
        ...
        // Update clip count
        auto clips = aidaw::ClipManager::getInstance().getClipsOnTrack(selectedTrackId_);
...
void TrackInspector::updateFromMultiTrackSelection() {
    ...
    for (auto tid : selectedTrackIds_) {
        ...
        if (track->inputMonitor == aidaw::InputMonitorMode::Off)
            allMonitorOn = false;
    }
    ...
    muteButton_.onClick = [this]() {
        ...
        for (auto tid : selectedTrackIds_) {
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetTrackMuteCommand>(tid, newState));
        }
    };
    soloButton_.onClick = [this]() {
        ...
        for (auto tid : selectedTrackIds_) {
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetTrackSoloCommand>(tid, newState));
        }
    };
    recordButton_.onClick = [this]() {
        bool newState = recordButton_.getToggleState();
        for (auto tid : selectedTrackIds_) {
            aidaw::TrackManager::getInstance().setTrackRecordArmed(tid, newState);
        }
    };
    monitorButton_.onClick = [this]() {
        // Cycle all selected tracks to the same next mode based on current button state
        auto nextMode = monitorButton_.getToggleState() ? aidaw::InputMonitorMode::In
                                                        : aidaw::InputMonitorMode::Off;
        for (auto tid : selectedTrackIds_) {
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetTrackInputMonitorCommand>(tid, nextMode));
        }
    };
    ...
    gainLabel_->onValueChange = [this]() {
        ...
        if (multiTrackBaseVolumes_.empty()) {
            auto& tmInner = aidaw::TrackManager::getInstance();
            for (auto tid : selectedTrackIds_) {
        ...
        for (auto& [tid, baseVol] : multiTrackBaseVolumes_) {
            ...
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetTrackVolumeCommand>(tid, newGain));
        }
        gainLabel_->clearTextOverride();
    ...
    panLabel_->onValueChange = [this]() {
        ...
        for (auto& [tid, basePan] : multiTrackBasePans_) {
            ...
            aidaw::TrackManager::getInstance().setTrackPan(tid, newPan);
        }
        panLabel_->clearTextOverride();
        if (!panLabel_->isDragging())
    ...
    panLabel_->onDragEnd = [this](double /*startValue*/) {
        ...
        for (auto& [tid, basePan] : multiTrackBasePans_) {
            auto* track = aidaw::TrackManager::getInstance().getTrack(tid);
            if (track)
                aidaw::UndoManager::getInstance().executeCommand(
                    std::make_unique<aidaw::SetTrackPanCommand>(tid, basePan, track->pan));
        }
        multiTrackBasePans_.clear();
    };
...
void TrackInspector::showTrackControls(bool show) {
    bool isMaster = show && selectedTrackId_ == aidaw::MASTER_TRACK_ID;
    bool isAux = false;
    bool isMultiOut = false;
    if (show && selectedTrackId_ != aidaw::INVALID_TRACK_ID &&
        selectedTrackId_ != aidaw::MASTER_TRACK_ID) {
        const auto* track = aidaw::TrackManager::getInstance().getTrack(selectedTrackId_);
        if (track && track->type == aidaw::TrackType::Aux)
            isAux = true;
        if (track && track->type == aidaw::TrackType::MultiOut)
            isMultiOut = true;
    }
...
void TrackInspector::rebuildSendsUI() {
    ...
    if (track->type == aidaw::TrackType::Aux)
        return;

    for (const auto& send : track->sends) {
        // Destination name label
        auto destLabel = std::make_unique<juce::Label>();
        const auto* destTrack = aidaw::TrackManager::getInstance().getTrack(send.destTrackId);
        ...
        auto levelLabel = std::make_unique<aidaw::DraggableValueLabel>(
            aidaw::DraggableValueLabel::Format::Decibels);
        ...
        aidaw::TrackId srcId = selectedTrackId_;
        ...
        levelLabel->onValueChange = [srcId, busIndex, levelLabelPtr]() {
            ...
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::SetSendLevelCommand>(srcId, busIndex, gain));
        };
        ...
        aidaw::AutomationTarget sendTarget;
        sendTarget.kind = aidaw::ControlTarget::Kind::SendLevel;
        sendTarget.devicePath = aidaw::ChainNodePath::trackLevel(srcId);
        sendTarget.sendBusIndex = busIndex;
        levelLabel->setAutomationTarget(sendTarget);

        levelLabel->onRightClick = [srcId, busIndex]() {
            auto& autoMgr = aidaw::AutomationManager::getInstance();
            aidaw::AutomationTarget target;
            target.kind = aidaw::ControlTarget::Kind::SendLevel;
            target.devicePath.trackId = srcId;
            target.sendBusIndex = busIndex;

            auto laneId = autoMgr.getLaneForTarget(target);
            const bool hasLane = laneId != aidaw::INVALID_AUTOMATION_LANE_ID;
            ...
            menu.showMenuAsync(juce::PopupMenu::Options(), [target](int result) {
                auto& mgr = aidaw::AutomationManager::getInstance();
                if (result == 1) {
                    auto id = mgr.getOrCreateLane(target, aidaw::AutomationLaneType::Absolute);
                    mgr.setLaneVisible(id, true);
                } else if (result == 2) {
                    auto id = mgr.getLaneForTarget(target);
                    if (id != aidaw::INVALID_AUTOMATION_LANE_ID)
                        mgr.deleteLane(id);
                }
            });
        };
        ...
        deleteBtn->onClick = [srcId, busIndex]() {
            aidaw::TrackManager::getInstance().removeSend(srcId, busIndex);
        };
...
void TrackInspector::showAddSendMenu() {
    ...
    const auto* currentTrack = aidaw::TrackManager::getInstance().getTrack(selectedTrackId_);
    if (!currentTrack)
        return;

    juce::PopupMenu menu;
    auto& trackManager = aidaw::TrackManager::getInstance();
    const auto& allTracks = trackManager.getTracks();

    // Collect descendants to prevent routing cycles
    std::vector<aidaw::TrackId> descendants;
    if (selectedTrackId_ != aidaw::INVALID_TRACK_ID) {
        descendants = trackManager.getAllDescendants(selectedTrackId_);
    }

    int itemId = 1;
    std::vector<aidaw::TrackId> destTrackIds;

    auto addTracksOfType = [&](aidaw::TrackType type) {
    ...
    addTracksOfType(aidaw::TrackType::Aux);
    addTracksOfType(aidaw::TrackType::Group);
    addTracksOfType(aidaw::TrackType::Audio);
    ...
    menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(&addSendButton_),
                       [sourceTrackId, destTrackIds](int result) {
                           if (result > 0 && result <= static_cast<int>(destTrackIds.size())) {
                               aidaw::TrackManager::getInstance().addSend(sourceTrackId,
...
void TrackInspector::populateRoutingSelectors() {
    ...
    audioInputSelector_->onEnabledChanged = [this](bool enabled) {
        ...
        if (enabled) {
            ...
            aidaw::TrackManager::getInstance().setTrackMidiInput(selectedTrackId_, "");
            ...
            if (trackInfo && trackInfo->audioInputDevice.startsWith("track:"))
                aidaw::TrackManager::getInstance().setTrackAudioInput(selectedTrackId_,
                                                                      trackInfo->audioInputDevice);
            else
                aidaw::TrackManager::getInstance().setTrackAudioInput(selectedTrackId_, "default");
        } else {
            aidaw::TrackManager::getInstance().setTrackAudioInput(selectedTrackId_, "");
        }
    };
    ...
    audioInputSelector_->onSelectionChanged = [this](int selectedId) {
        ...
        if (selectedId == 1) {
            ...
            if (it != inputTrackMapping_.end()) {
                aidaw::TrackManager::getInstance().setTrackAudioInput(
            ...
            if (it != inputChannelMapping_.end()) {
                ...
                aidaw::TrackManager::getInstance().setTrackAudioInput(selectedTrackId_, deviceName);
            } else {
    ...
    inputSelector_->onEnabledChanged = [this, midiBridge](bool enabled) {
        ...
        if (enabled) {
            ...
            if (selectedId == 1) {
                aidaw::TrackManager::getInstance().setTrackMidiInput(selectedTrackId_, "all");
                ...
                if (deviceIndex >= 0 && deviceIndex < static_cast<int>(midiInputs.size())) {
                    aidaw::TrackManager::getInstance().setTrackMidiInput(
    ...
    outputSelector_->onEnabledChanged = [this](bool enabled) {
        ...
        if (enabled) {
            aidaw::TrackManager::getInstance().setTrackAudioOutput(selectedTrackId_, "master");
        } else {
            aidaw::TrackManager::getInstance().setTrackAudioOutput(selectedTrackId_, "");
        }
    };
    ...
    outputSelector_->onSelectionChanged = [this](int selectedId) {
        ...
        if (selectedId == 1) {
            ...
            if (it != outputTrackMapping_.end()) {
                aidaw::TrackManager::getInstance().setTrackAudioOutput(
                    selectedTrackId_, "track:" + juce::String(it->second));
            }
        } else if (selectedId >= 10) {
            // Hardware output
    ...
    midiOutputSelector_->onEnabledChanged = [this](bool enabled) {
        ...
        if (!enabled) {
            aidaw::TrackManager::getInstance().setTrackMidiOutput(selectedTrackId_, "");
        }
    ...
    midiOutputSelector_->onSelectionChanged = [this, midiBridge](int selectedId) {
        ...
        if (selectedId == 1) {
            ...
            if (it != midiOutputTrackMapping_.end()) {
                aidaw::TrackManager::getInstance().setTrackMidiOutput(
                    selectedTrackId_, "track:" + juce::String(it->second));
            }
        } else if (selectedId >= 10 && midiBridge) {
...
void TrackInspector::populateAudioInputOptions() {
    ...
    aidaw::RoutingSyncHelper::populateAudioInputOptions(
...
void TrackInspector::populateAudioOutputOptions() {
    ...
    aidaw::RoutingSyncHelper::populateAudioOutputOptions(
...
void TrackInspector::populateMidiInputOptions() {
    if (!inputSelector_ || !audioEngine_)
        return;
    aidaw::RoutingSyncHelper::populateMidiInputOptions(inputSelector_.get(),
...
void TrackInspector::populateMidiOutputOptions() {
    if (!midiOutputSelector_ || !audioEngine_)
        return;
    aidaw::RoutingSyncHelper::populateMidiOutputOptions(
...
void TrackInspector::updateRoutingSelectorsFromTrack() {
    if (selectedTrackId_ == aidaw::INVALID_TRACK_ID || !audioEngine_)
        return;
    ...
    aidaw::RoutingSyncHelper::syncSelectorsFromTrack(
