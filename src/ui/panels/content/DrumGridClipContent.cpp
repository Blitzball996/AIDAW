...
daw::audio::DrumGridPlugin* findDrumGridForTrack(aidaw::TrackId trackId) {
...
class DrumGridClipGrid : public juce::Component,
                         public aidaw::NoteGridHost,
                         public aidaw::ClipManagerListener {
  public:
    DrumGridClipGrid() {
    ...
    void setClipId(aidaw::ClipId id) {
        clipId_ = id;
    }
    ...
    std::function<void(aidaw::ClipId, double, int, double, int)> onNoteAdded;
    std::function<void(aidaw::ClipId, size_t)> onNoteDeleted;
    std::function<void(aidaw::ClipId, size_t, double, int)> onNoteMoved;
    std::function<void(aidaw::ClipId, size_t, double)> onNoteResized;
    std::function<void(aidaw::ClipId, size_t, double, int)> onNoteCopied;
    std::function<void(aidaw::ClipId, size_t, bool)> onNoteSelected;
    std::function<void(aidaw::ClipId, std::vector<size_t>)> onNoteSelectionChanged;
    std::function<void(aidaw::ClipId, std::vector<size_t>, aidaw::QuantizeMode, double)>
        onQuantizeNotes;
    std::function<void(aidaw::ClipId, std::vector<size_t>)> onCopyNotes;
    std::function<void(aidaw::ClipId)> onPasteNotes;
    std::function<void(aidaw::ClipId, std::vector<size_t>)> onDuplicateNotes;
    std::function<void(aidaw::ClipId, std::vector<size_t>)> onDeleteNotes;
    ...
    void refreshNotes() {
        ...
        if (pendingSelectPositions_.empty()) {
            const auto& noteSel = aidaw::SelectionManager::getInstance().getNoteSelection();
        ...
        if (clipId_ == aidaw::INVALID_CLIP_ID || !padRows_ || padRows_->empty()) {
            repaint();
            return;
        }
        ...
        if (!pendingPositions.empty()) {
            ...
            const auto* clip = aidaw::ClipManager::getInstance().getClip(clipId_);
            if (clip) {
    ...
    void updateNotePosition(aidaw::NoteComponent* note, double beat, int noteNumber,
                            double length) override {
        if (!note || !padRows_)
            return;

        const auto* clip = aidaw::ClipManager::getInstance().getClip(note->getSourceClipId());
        if (clip) {
            aidaw::MidiNote previewNote;
            previewNote.startBeat = beat;
            previewNote.noteNumber = noteNumber;
            previewNote.lengthBeats = length;
            if (!aidaw::ClipOperations::constrainMidiNoteToVisibleRange(*clip, previewNote)) {
    ...
    void setCopyDragPreview(double beat, int noteNumber, double length, juce::Colour colour,
        ...
        const auto* srcClip = aidaw::ClipManager::getInstance().getClip(clipId_);
        ...
        auto addGhost = [&](double clipBeat, int ghostNote, double ghostLength) {
            ...
            if (!aidaw::ClipOperations::constrainMidiNoteToVisibleRange(*srcClip, previewNote))
                return;
    ...
    void updateSelectedNotePositions(aidaw::NoteComponent* draggedNote, double beatDelta,
                                     int noteDelta) override {
        if (!draggedNote)
            return;
        aidaw::ClipId dragClipId = draggedNote->getSourceClipId();
        const auto* clip = aidaw::ClipManager::getInstance().getClip(dragClipId);
        if (!clip)
            return;
    ...
    void updateSelectedNoteLengths(aidaw::NoteComponent* draggedNote, double lengthDelta) override {
        if (!draggedNote)
            return;
        aidaw::ClipId dragClipId = draggedNote->getSourceClipId();
        const auto* clip = aidaw::ClipManager::getInstance().getClip(dragClipId);
    ...
    void updateSelectedNoteLeftResize(aidaw::NoteComponent* draggedNote,
    ...
    void clipSelectionChanged(aidaw::ClipId) override {}
    ...
    void paint(juce::Graphics& g) override {
        ...
        if (clipId_ != aidaw::INVALID_CLIP_ID) {
            const auto* offsetClip = aidaw::ClipManager::getInstance().getClip(clipId_);
            if (offsetClip && offsetClip->loopEnabled) {
                ...
                if (offsetX >= 0 && offsetX <= bounds.getWidth()) {
                    aidaw::paintPhaseMarker(g, offsetClip, offsetX, numRows * rowHeight_,
    ...
    void mouseMove(const juce::MouseEvent& e) override {
        if (e.mods.isShiftDown()) {
            setMouseCursor(aidaw::CursorManager::getInstance().getNoteRepeatCursor());
        ...
        if (clipId_ != aidaw::INVALID_CLIP_ID) {
            const auto* clip = aidaw::ClipManager::getInstance().getClip(clipId_);
            nearPhaseMarker_ = aidaw::isNearPhaseMarker(e.x, GRID_LEFT_PADDING, clip);
        }
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        ...
        if (!padRows_ || padRows_->empty() || clipId_ == aidaw::INVALID_CLIP_ID)
            return;
        ...
        if (e.mods.isPopupMenu()) {
            ...
            menu.addItem(11, "Paste", aidaw::ClipManager::getInstance().hasNotesInClipboard());
            ...
            menu.showMenuAsync(
                juce::PopupMenu::Options(), [this, indices = std::move(selectedIndices),
                    ...
                    if (result == 10 && onCopyNotes)
                        ...
                        const aidaw::QuantizeMode modes[] = {aidaw::QuantizeMode::StartOnly,
                                                             aidaw::QuantizeMode::LengthOnly,
                                                             aidaw::QuantizeMode::StartAndLength};
    ...
    bool keyPressed(const juce::KeyPress& key) override {
        ...
        if (!key.getModifiers().isAltDown() && (key.getKeyCode() == juce::KeyPress::upKey ||
            ...
            const auto* clip = aidaw::ClipManager::getInstance().getClip(noteSel.clipId);
            if (!clip || !clip->isMidi())
                return false;
            ...
            for (size_t idx : noteSel.noteIndices) {
                const auto& note = clip->midiNotes[idx];
                auto cmd = std::make_unique<aidaw::MoveMidiNoteCommand>(
  ...
  private:
    ...
    double clipBeatToDisplayBeat(double beat) const {
        ...
        if (const auto* clip = aidaw::ClipManager::getInstance().getClip(clipId_))
            visibleStart = aidaw::ClipOperations::getMidiVisibleRange(*clip).startBeat;
    ...
    int rowHeight_ = 24;
    aidaw::ClipId clipId_ = aidaw::INVALID_CLIP_ID;
    ...
    // Note components
    std::vector<std::unique_ptr<aidaw::NoteComponent>> noteComponents_;
    ...
    void stampRepeatedNotes() {
        ...
        if (beats.size() > 1)
            aidaw::UndoManager::getInstance().beginCompoundOperation("Stamp Drum Notes");
    ...
    void createNoteComponents() {
        ...
        for (size_t i = 0; i < clip->midiNotes.size(); i++) {
            ...
            if (!aidaw::ClipOperations::clipMidiNoteToVisibleRange(*clip, visibleNote))
                continue;

            auto noteComp = std::make_unique<aidaw::NoteComponent>(i, this, clipId_);
                ...
                menu.showMenuAsync(
                    juce::PopupMenu::Options(), [this, indices = std::move(selectedIndices),
                        ...
                        if (result == 10 && onCopyNotes)
                            ...
                            const aidaw::QuantizeMode modes[] = {
                                aidaw::QuantizeMode::StartOnly, aidaw::QuantizeMode::LengthOnly,
                                aidaw::QuantizeMode::StartAndLength};
                            onQuantizeNotes(clipId_, indices, modes[result - 1], gridRes);
                        } else if (result >= 20 && result <= 61 && onQuantizeNotes) {
                            // clang-format off
    ...
    void updateNoteComponentBounds() {
        ...
        for (auto& noteComp : noteComponents_) {
            ...
            if (!aidaw::ClipOperations::clipMidiNoteToVisibleRange(*clip, note)) {
...
class DrumGridRowLabels : public juce::Component {
  public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        auto font = aidaw::FontManager::getInstance().getUIFont(11.0f);
        g.setFont(font);
...
DrumGridClipContent::DrumGridClipContent() {
    ...
    controlsToggle_ = std::make_unique<aidaw::SvgButton>(
    ...
    rowLabels_->onNotePreview = [this](int noteNumber, bool isNoteOn) {
        ...
        if (clip && clip->trackId != aidaw::INVALID_TRACK_ID) {
            aidaw::TrackManager::getInstance().previewNote(clip->trackId, noteNumber,
    ...
    gridComponent_->onNoteAdded = [](aidaw::ClipId clipId, double beat, int noteNumber,
        ...
        auto cmd =
            std::make_unique<aidaw::AddMidiNoteCommand>(clipId, beat, noteNumber, length, velocity);
        aidaw::UndoManager::getInstance().executeCommand(std::move(cmd));
    };
    ...
    gridComponent_->onNoteCopied = [](aidaw::ClipId clipId, size_t noteIndex, double destBeat,
                                      int destNoteNumber) {
...
void DrumGridClipContent::clipsChanged() {
    if (editingClipId_ != aidaw::INVALID_CLIP_ID) {
        const auto* clip = aidaw::ClipManager::getInstance().getClip(editingClipId_);
        if (!clip) {
            gridComponent_->setClipId(aidaw::INVALID_CLIP_ID);
            if (midiDrawer_)
...
void DrumGridClipContent::clipSelectionChanged(aidaw::ClipId clipId) {
...
void DrumGridClipContent::setClip(aidaw::ClipId clipId) {
