    ...
    if (luaName.isNotEmpty()) {
        ...
        if (ports.dawInputPort.isNotEmpty()) {
            ...
            for (const auto& dev : liveInputs_) {
                if (aidaw::midi::matches(ports.dawInputPort, dev.identifier, dev.name)) {
                    b.connected = true;
                    break;
                }
            }
        ...
        for (const auto& c : ControllerRegistry::getInstance().all()) {
            ...
            // with varying case, depending on platform — aidaw::midi::matches
            // handles all of those consistently.
            for (const auto& dev : liveInputs_) {
                if (aidaw::midi::matches(c.inputPort, dev.identifier, dev.name)) {
                    b.connected = true;
                    break;
                }
            }
...
void FooterBar::setupButtons() {
    ...
    for (size_t i = 0; i < NUM_MODES; ++i) {
        ...
        modeButtons[i] = aidaw::ManagedChild<SvgButton>::create(icons[i].name, icons[i].data,
