...
void setupTextSlider(juce::Component& owner, aidaw::daw::ui::TextSlider& slider, juce::Label& label,
                     const juce::String& labelText, double min, double max, double interval,
                     int decimals = 0, const juce::String& suffix = {}) {
    label.setText(labelText, juce::dontSendNotification);
    label.setFont(aidaw::FontManager::getInstance().getUIFont(12.0f));
    label.setColour(juce::Label::textColourId,
                    aidaw::DarkTheme::getColour(aidaw::DarkTheme::TEXT_PRIMARY));
    label.setJustificationType(juce::Justification::centredLeft);
    owner.addAndMakeVisible(label);

    slider.setRange(min, max, interval);
    slider.setOrientation(aidaw::daw::ui::TextSlider::Orientation::Horizontal);
...
void setupToggle(juce::Component& owner, juce::ToggleButton& toggle, const juce::String& text) {
    ...
    toggle.setColour(juce::ToggleButton::tickColourId,
                     aidaw::DarkTheme::getColour(aidaw::DarkTheme::ACCENT_BLUE));
    toggle.setColour(juce::ToggleButton::tickDisabledColourId,
                     aidaw::DarkTheme::getColour(aidaw::DarkTheme::TEXT_DIM));
...
void setupSectionHeader(juce::Component& owner, juce::Label& header, const juce::String& text) {
    ...
    header.setColour(juce::Label::textColourId,
                     aidaw::DarkTheme::getColour(aidaw::DarkTheme::TEXT_SECONDARY));
    header.setFont(aidaw::FontManager::getInstance().getUIFontBold(14.0f));
...
class GeneralPage : public juce::Component {
  public:
    ...
    void loadSettings(Config& config) {
        ...
        auto langDir = aidaw::StringTable::findLangDirectory();
  ...
  private:
    ...
    static void layoutTextSliderRow(juce::Rectangle<int>& bounds, juce::Label& label,
                                    aidaw::daw::ui::TextSlider& slider, int rowH, int sliderH) {
    ...
    aidaw::daw::ui::TextSlider zoomInSensitivitySlider, zoomOutSensitivitySlider,
        zoomShiftSensitivitySlider;
    juce::Label zoomInLabel, zoomOutLabel, zoomShiftLabel;
    aidaw::daw::ui::TextSlider timelineLengthSlider, viewDurationSlider;
    ...
    aidaw::daw::ui::TextSlider autoSaveIntervalSlider;
    ...
    aidaw::daw::ui::TextSlider fontScaleSlider;
};
  ...
  public:
    PathsPage() {
        ...
        dataReveal_.onClick = [this]() {
            revealConfigured(dataPath_, aidaw::paths::alwaysOSDefault());
        };
  ...
  private:
    ...
    void pickFolder(Kind kind, const juce::String& title) {
        const juce::File defaultDir =
            (kind == Kind::Data) ? aidaw::paths::alwaysOSDefault() : presetsDefaultDir();
    ...
    void updateDisplay() {
        if (dataPath_.empty()) {
            dataValue_.setText("default: " + aidaw::paths::alwaysOSDefault().getFullPathName(),
                               juce::dontSendNotification);
        } else {
...
void PreferencesDialog::applySettings() {
    ...
    const juce::File dataFrom(juce::String(pathsPage->getOriginalDataPath()).isEmpty()
                                  ? aidaw::paths::alwaysOSDefault().getFullPathName()
