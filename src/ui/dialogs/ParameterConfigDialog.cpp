...
struct CachedPluginParams {
    ...
    std::vector<aidaw::ParameterScanInput> scanInputs;
};
static std::map<juce::String, CachedPluginParams> parameterCache_;

static juce::String scaleToXmlString(aidaw::ParameterScale scale) {
    switch (scale) {
        case aidaw::ParameterScale::Linear:
            return "linear";
        case aidaw::ParameterScale::Logarithmic:
            return "logarithmic";
        case aidaw::ParameterScale::Exponential:
            return "exponential";
        case aidaw::ParameterScale::Discrete:
            return "discrete";
        case aidaw::ParameterScale::Boolean:
            return "boolean";
        case aidaw::ParameterScale::FaderDB:
            return "fader_db";
    }
    return "linear";
}

static aidaw::ParameterScale xmlStringToScale(const juce::String& str) {
    if (str == "logarithmic")
        return aidaw::ParameterScale::Logarithmic;
    if (str == "exponential")
        return aidaw::ParameterScale::Exponential;
    if (str == "discrete")
        return aidaw::ParameterScale::Discrete;
    if (str == "boolean")
        return aidaw::ParameterScale::Boolean;
    if (str == "fader_db")
        return aidaw::ParameterScale::FaderDB;
    return aidaw::ParameterScale::Linear;
}
...
class ParameterConfigDialog::RangeCell : public juce::Component {
  public:
    ...
    void update(int row) {
        ...
        if (param.scale == aidaw::ParameterScale::Boolean) {
            editor_.setText("on / off", false);
            editor_.setEnabled(false);
        } else if (param.scale == aidaw::ParameterScale::Discrete) {
            juce::String text;
  ...
  private:
    void commitEdit() {
        ...
        if (param.scale == aidaw::ParameterScale::Discrete) {
...
void ParameterConfigDialog::loadParameters(const juce::String& uniqueId) {
    ...
    auto* tracktionEngine = dynamic_cast<aidaw::TracktionEngineWrapper*>(audioEngine);
    if (!tracktionEngine) {
    ...
    for (int i = 0; i < numParams; ++i) {
        ...
        aidaw::ParameterScanInput scanInput;
...
void ParameterConfigDialog::resetParameterConfiguration() {
    ...
    if (!pluginUniqueId_.isEmpty()) {
        auto configFile = aidaw::paths::pluginConfigsDir().getChildFile(
    ...
    for (auto& param : parameters_) {
        param.unit = "%";
        param.scale = aidaw::ParameterScale::Linear;
        param.rangeMin = 0.0f;
        param.rangeMax = 1.0f;
...
void ParameterConfigDialog::runHeuristicDetection() {
    ...
    std::vector<aidaw::ParameterScanInput> visibleInputs;
    ...
    auto results = aidaw::ParameterDetector::detect(visibleInputs);
...
void ParameterConfigDialog::runDetection() {
    ...
    if (ambiguousCount > 0) {
        ...
        auto agentConfig = aidaw::Config::getInstance().getAgentLLMConfig("command");
        if (!agentConfig.provider.empty()) {
            ...
            aidaw::ParameterDetector::detectWithAI(
                ...
                // onComplete
                [safeThis](std::vector<aidaw::DetectedParameterInfo> aiResults) {
                    if (!safeThis)
                        return;
...
void ParameterConfigDialog::applyDetectionResults(
    const std::vector<aidaw::DetectedParameterInfo>& results) {
    for (const auto& r : results) {
        ...
        if (r.scale == aidaw::ParameterScale::Discrete) {
            param.unit = "discrete";
        } else if (r.scale == aidaw::ParameterScale::Boolean) {
            param.unit = "boolean";
        } else {
...
void ParameterConfigDialog::saveParameterConfiguration() {
    ...
    auto configDir = aidaw::paths::pluginConfigsDir();
