...
const std::vector<ProviderInfo>& getKnownProviders() {
    static const std::vector<ProviderInfo> providers = {
        {aidaw::provider::OPENAI_CHAT, "OpenAI", aidaw::provider::OPENAI_CHAT, "",
         aidaw::model::GPT_4_1_MINI, BinaryData::openai_svg, BinaryData::openai_svgSize},
        {aidaw::provider::ANTHROPIC, "Anthropic", aidaw::provider::ANTHROPIC, "",
         aidaw::model::CLAUDE_HAIKU, BinaryData::anthropic_svg, BinaryData::anthropic_svgSize},
        {aidaw::provider::GEMINI, "Gemini", aidaw::provider::GEMINI, "", aidaw::model::GEMINI_FLASH,
         BinaryData::gemini_svg, BinaryData::gemini_svgSize},
        {aidaw::provider::DEEPSEEK, "DeepSeek", aidaw::provider::DEEPSEEK, "",
         aidaw::model::DEEPSEEK_CHAT, BinaryData::deepseek_svg, BinaryData::deepseek_svgSize},
        {aidaw::provider::OPENROUTER, "OpenRouter", aidaw::provider::OPENROUTER, "",
         aidaw::model::LLAMA_70B, BinaryData::openrouter_svg, BinaryData::openrouter_svgSize},
    };
    return providers;
}
...
class AISettingsDialog::ConfigPage : public juce::Component {
  public:
    ...
    void load(Config& config) {
        ...
        if (presetId.starts_with("local") || presetId == aidaw::preset::LOCAL_EMBEDDED) {
            modeCombo_.setSelectedId(1, juce::dontSendNotification);
        } else if (presetId.starts_with("hybrid")) {
            modeCombo_.setSelectedId(3, juce::dontSendNotification);
            if (presetId == aidaw::preset::HYBRID_SPEED)
            ...
            auto cmdCfg = config.getAgentLLMConfig(aidaw::role::COMMAND);
        ...
        auto musicCfg = config.getAgentLLMConfig(aidaw::role::MUSIC);
        if (musicCfg.provider == aidaw::provider::ANTHROPIC)
            savedProviderDisplay_ = "Anthropic";
        else if (musicCfg.provider == aidaw::provider::GEMINI)
            savedProviderDisplay_ = "Gemini";
        else if (musicCfg.provider == aidaw::provider::DEEPSEEK)
            savedProviderDisplay_ = "DeepSeek";
        else if (musicCfg.provider == aidaw::provider::OPENROUTER)
            savedProviderDisplay_ = "OpenRouter";
        else if (musicCfg.provider == aidaw::provider::OPENAI_CHAT ||
                 musicCfg.provider == aidaw::provider::OPENAI_RESPONSES)
    ...
    void apply(Config& config) {
        ...
        if (mode == 1) {
            // Local
            config.setAIPreset(aidaw::preset::LOCAL_EMBEDDED);
            auto* preset = aidaw::findPreset(aidaw::preset::LOCAL_EMBEDDED);
            if (preset)
            ...
            auto* preset = aidaw::findPreset(presetId);
            if (preset) {
            ...
            if (optimize == "Cost") {
                auto routerCfg = config.getAgentLLMConfig(aidaw::role::ROUTER);
                applyCheaperModel(routerCfg, presetId);
                config.setAgentLLMConfig(aidaw::role::ROUTER, routerCfg);
            }
        } else {
            // Hybrid
            std::string hybridPresetId =
                optimize == "Speed" ? aidaw::preset::HYBRID_SPEED : aidaw::preset::HYBRID_QUALITY;
            config.setAIPreset(hybridPresetId);

            // Router is always local
            Config::AgentLLMConfig localCfg;
            localCfg.provider = aidaw::provider::LLAMA_LOCAL;
            config.setAgentLLMConfig(aidaw::role::ROUTER, localCfg);

            // Music always uses cloud
            auto musicCfg = makeCloudConfig(aidaw::role::MUSIC, presetId);
            config.setAgentLLMConfig(aidaw::role::MUSIC, musicCfg);

            // Command: cloud for speed, local for cost
            if (optimize == "Speed") {
                auto cmdCfg = makeCloudConfig(aidaw::role::COMMAND, presetId);
                config.setAgentLLMConfig(aidaw::role::COMMAND, cmdCfg);
            } else {
                Config::AgentLLMConfig cmdLocal;
                cmdLocal.provider = aidaw::provider::LLAMA_LOCAL;
                config.setAgentLLMConfig(aidaw::role::COMMAND, cmdLocal);
            }
        }
    }
  ...
  private:
    ...
    static std::string providerDisplayToPresetId(const juce::String& display) {
        if (display == "Anthropic")
            return aidaw::preset::CLOUD_ANTHROPIC;
        if (display == "Gemini")
            return aidaw::preset::CLOUD_GEMINI;
        if (display == "DeepSeek")
            return aidaw::preset::CLOUD_DEEPSEEK;
        if (display == "OpenRouter")
            return aidaw::preset::CLOUD_OPENROUTER;
        return aidaw::preset::CLOUD_OPENAI;
    }

    static Config::AgentLLMConfig makeCloudConfig(const std::string& role,
                                                  const std::string& presetId) {
        if (auto* preset = aidaw::findPreset(presetId)) {
    ...
    static void applyCheaperModel(Config::AgentLLMConfig& cfg, const std::string& presetId) {
        if (presetId == aidaw::preset::CLOUD_OPENAI)
            cfg.model = aidaw::model::GPT_4_1_MINI;
        else if (presetId == aidaw::preset::CLOUD_ANTHROPIC)
            cfg.model = aidaw::model::CLAUDE_HAIKU;
        else if (presetId == aidaw::preset::CLOUD_GEMINI)
            cfg.model = aidaw::model::GEMINI_FLASH;
        else if (presetId == aidaw::preset::CLOUD_DEEPSEEK)
            cfg.model = aidaw::model::DEEPSEEK_CHAT;
