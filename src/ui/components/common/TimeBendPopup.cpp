...
TimeBendPopup::TimeBendPopup(aidaw::ClipId clipId, std::vector<size_t> noteIndices)
    ...
    if (clip && clip->isMidi()) {
        originalStartBeats_ = aidaw::collectMidiNoteStartBeats(*clip, noteIndices_);
    }
...
void TimeBendPopup::applyPreview(float depth, float skew, int cycles, float quantize,
    ...
    const auto bentStarts = aidaw::calculateBentMidiNoteStartBeats(
...
bool TimeBendPopup::syncSelectionFromManager() {
    const auto& selection = aidaw::SelectionManager::getInstance().getNoteSelection();
