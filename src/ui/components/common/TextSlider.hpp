 ...
 */
class TextSlider : public juce::Component, public aidaw::AutomationManagerListener {
  public:
    ...
    void setParameterInfo(const aidaw::ParameterInfo& info) {
        ...
        if (info.displayFormat == aidaw::DisplayFormat::Percent && info.minValue >= -1.0e-6f &&
        ...
        useLogProjection_ =
            (info.scale == aidaw::ParameterScale::Logarithmic && info.minValue > 0.0f);
        skewFactor_ = 1.0;
        ...
        if (!hasExplicitFormatter_) {
            valueFormatter_ = [this](double real) {
                return aidaw::ParameterUtils::formatValue(static_cast<float>(real), paramInfoCopy_);
            };
        }
        valueParser_ = [this](const juce::String& text) {
            auto parsed = aidaw::ParameterUtils::parseValue(text, paramInfoCopy_);
    ...
    void setAutomationTarget(const aidaw::AutomationTarget& target) {
    ...
    aidaw::AutomationVisualState automationVisualState() const {
        return automationVisualState_;
    }
    bool isAutomated() const {
        return automationVisualState_ != aidaw::AutomationVisualState::None;
    }
    ...
    void automationLanePropertyChanged(aidaw::AutomationLaneId /*laneId*/) override {
        refreshAutomationVisualState();
    }
    ...
    void paint(juce::Graphics& g) override {
        ...
        if ((orientation_ == Orientation::Vertical || meterPeakL_ > 0.001f ||
             meterPeakR_ > 0.001f) &&
            automationVisualState_ != aidaw::AutomationVisualState::None) {
            auto boundsF = getLocalBounds().toFloat();
            const juce::Colour tint =
                automationVisualState_ == aidaw::AutomationVisualState::Overridden
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        if (!valueControl_.isEditing() && e.mods.isLeftButtonDown()) {
            ...
            if (hasAutomationTarget_ && isAutomated()) {
                aidaw::AutomationManager::getInstance().setTargetTouchSuppressed(automationTarget_,
                                                                                 true);
            }
            ...
            if (hasAutomationTarget_) {
                aidaw::AutomationManager::getInstance().setTargetUserTouched(automationTarget_,
                                                                             true);
                ...
                aidaw::ParameterInfo info = aidaw::getParameterInfoForTarget(automationTarget_);
                double normalized = static_cast<double>(aidaw::ParameterUtils::realToNormalized(
                    static_cast<float>(dragStartValue_), info));
                aidaw::AutomationManager::getInstance().setTouchBaseline(automationTarget_,
                                                                         normalized);
            }
        } else {
            isLeftButtonDrag_ = false;
    ...
    void mouseDrag(const juce::MouseEvent& e) override {
        ...
        if (dx > 3 || dy > 3) {
            ...
            if (!isShiftDrag_ && !overrideLatchedThisGesture_ && hasAutomationTarget_ &&
                isAutomated() && !aidaw::AutomationManager::getInstance().isWriteModeEnabled()) {
                aidaw::AutomationManager::getInstance().setTargetOverridden(automationTarget_,
                                                                            true);
                setAutomationVisualState(aidaw::AutomationVisualState::Overridden);
  ...
  private:
    ...
    void latchAutomationOverride() {
        ...
        if (!hasAutomationTarget_ || !isAutomated())
            return;
        if (aidaw::AutomationManager::getInstance().isWriteModeEnabled())
            return;

        aidaw::AutomationManager::getInstance().setTargetOverridden(automationTarget_, true);
        setAutomationVisualState(aidaw::AutomationVisualState::Overridden);
        overrideLatchedThisGesture_ = true;
    }
    ...
    aidaw::ParameterInfo paramInfoCopy_;  // Populated by setParameterInfo
    ...
    // Automation state
    aidaw::AutomationTarget automationTarget_;
    bool hasAutomationTarget_ = false;
    aidaw::AutomationVisualState automationVisualState_ = aidaw::AutomationVisualState::None;
    bool listeningToAutomation_ = false;

    void refreshAutomationVisualState() {
        auto newState =
            hasAutomationTarget_
                ? aidaw::AutomationManager::getInstance().getVisualState(automationTarget_)
                : aidaw::AutomationVisualState::None;
    ...
    void setAutomationVisualState(aidaw::AutomationVisualState state) {
    ...
    static ValueLabelControl::TintState toControlTintState(aidaw::AutomationVisualState state) {
        switch (state) {
            case aidaw::AutomationVisualState::Overridden:
                return ValueLabelControl::TintState::Overridden;
            case aidaw::AutomationVisualState::Active:
                return ValueLabelControl::TintState::Automated;
            case aidaw::AutomationVisualState::None:
            default:
