...
class TimeBendPopup : public juce::Component {
  public:
    TimeBendPopup(aidaw::ClipId clipId, std::vector<size_t> noteIndices);
    ...
    std::function<void(aidaw::ClipId clipId, std::vector<size_t> noteIndices, float depth,
  ...
  private:
    ...
    aidaw::ClipId clipId_;
    std::vector<size_t> noteIndices_;
    std::vector<aidaw::MidiNoteStartBeat> originalStartBeats_;
    bool applied_ = false;
