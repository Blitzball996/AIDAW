...
LinkableTextSlider::LinkableTextSlider(TextSlider::Format format) : slider_(format) {
    ...
    slider_.onShiftDragStart = [this](float /*startValue*/) {
        ...
        aidaw::ControlTarget thisTarget =
            aidaw::ControlTarget::pluginParam(devicePath_, paramIndex_);
    ...
    slider_.onRightClicked = [this]() {
        if (deviceId_ != aidaw::INVALID_DEVICE_ID) {
    ...
    onMidiLearn = [](aidaw::ChainNodePath path, int paramIdx, juce::String paramName) {
        aidaw::MidiLearnCoordinator::getInstance().beginLearn(
            aidaw::ControlTarget::pluginParam(path, paramIdx), paramName);
    };
    onMidiClear = [](aidaw::ChainNodePath path, int paramIdx) {
        aidaw::MidiLearnCoordinator::getInstance().clearMappings(
            aidaw::ControlTarget::pluginParam(path, paramIdx));
    };
...
LinkableTextSlider::~LinkableTextSlider() {
    ...
    if (isInMidiLearnMode_) {
        aidaw::MidiLearnCoordinator::getInstance().cancelLearn();
    }
...
void LinkableTextSlider::setParameterInfo(const aidaw::ParameterInfo& info) {
...
void LinkableTextSlider::setLinkContext(aidaw::DeviceId deviceId, int paramIndex,
...
void LinkableTextSlider::setAvailableMods(const aidaw::ModArray* mods) {
    availableMods_ = mods;
}

void LinkableTextSlider::setAvailableMacros(const aidaw::MacroArray* macros) {
    availableMacros_ = macros;
}

void LinkableTextSlider::setAvailableRackMods(const aidaw::ModArray* rackMods) {
    availableRackMods_ = rackMods;
}

void LinkableTextSlider::setAvailableRackMacros(const aidaw::MacroArray* rackMacros) {
    availableRackMacros_ = rackMacros;
}

void LinkableTextSlider::setAvailableTrackMods(const aidaw::ModArray* trackMods) {
    availableTrackMods_ = trackMods;
}

void LinkableTextSlider::setAvailableTrackMacros(const aidaw::MacroArray* trackMacros) {
...
void LinkableTextSlider::modLinkModeChanged(bool active, const aidaw::ModSelection& selection) {
    ...
    if (active && isInScope) {
        ...
        activeMacro_ = aidaw::MacroSelection{};
    } else {
        activeMod_ = aidaw::ModSelection{};
    }
...
void LinkableTextSlider::macroLinkModeChanged(bool active, const aidaw::MacroSelection& selection) {
    bool isInScope = isInScopeOf(devicePath_, selection.parentPath);
...
void LinkableTextSlider::midiLearnStateChanged(const aidaw::ChainNodePath& path, int paramIndex,
                                               aidaw::ControlTarget::Kind owner, bool learning) {
    const auto wantedOwner =
        isModRate_ ? aidaw::ControlTarget::Kind::ModParam : aidaw::ControlTarget::Kind::PluginParam;
...
void LinkableTextSlider::midiLearnCompleted(const aidaw::ChainNodePath& path, int paramIndex,
...
void LinkableTextSlider::midiLearnCleared(const aidaw::ChainNodePath& path, int paramIndex,
...
void LinkableTextSlider::bindingRegistryChanged(aidaw::BindingScope) {
    refreshMidiBindingState();
}

void LinkableTextSlider::refreshMidiBindingState() {
    const auto target = isModRate_
                            ? aidaw::ControlTarget::modParam(devicePath_, modId_, modParamIndex_)
                            : aidaw::ControlTarget::pluginParam(devicePath_, paramIndex_);
    const bool newState = aidaw::BindingRegistry::getInstance().hasActiveBindingFor(target);
...
void LinkableTextSlider::setModRateContext(const aidaw::ChainNodePath& path, aidaw::ModId modId,
                                           int modParamIndex) {
    isModRate_ = true;
    devicePath_ = path;
    modId_ = modId;
...
void LinkableTextSlider::mouseDown(const juce::MouseEvent& e) {
    ...
    if (e.mods.isPopupMenu() && deviceId_ != aidaw::INVALID_DEVICE_ID) {
        showParamLinkMenu(this, buildLinkContext(),
                          {.onModUnlinked = onModUnlinked,
                           .onRackModUnlinked = onRackModUnlinked,
                           .onTrackModUnlinked = onTrackModUnlinked,
