...
void WaveformGridComponent::paintWaveform(juce::Graphics& g, const aidaw::ClipInfo& clip) {
...
WaveformGridComponent::WaveformLayout WaveformGridComponent::computeWaveformLayout(
    const aidaw::ClipInfo& clip) const {
...
void WaveformGridComponent::paintWaveformBackground(juce::Graphics& g, const aidaw::ClipInfo& clip,
...
void WaveformGridComponent::paintWaveformThumbnail(juce::Graphics& g, const aidaw::ClipInfo& clip,
                                                   const WaveformLayout& layout) {
    ...
    auto& thumbnailManager = aidaw::AudioThumbnailManager::getInstance();
...
void WaveformGridComponent::paintWaveformOverlays(juce::Graphics& g, const aidaw::ClipInfo& clip,
                                                  const WaveformLayout& layout) {
    auto visibleRect = layout.visibleRect;
    if (visibleRect.isEmpty())
        return;
...
void WaveformGridComponent::paintBeatGrid(juce::Graphics& g, const aidaw::ClipInfo& clip) {
    juce::ignoreUnused(clip);
...
void WaveformGridComponent::paintWarpedWaveform(juce::Graphics& g, const aidaw::ClipInfo& clip,
...
void WaveformGridComponent::paintTransientMarkers(juce::Graphics& g, const aidaw::ClipInfo& clip) {
    juce::ignoreUnused(clip);
    auto bounds = getLocalBounds().reduced(LEFT_PADDING, TOP_PADDING);
    if (bounds.getWidth() <= 0 || bounds.getHeight() <= 0)
        return;
...
void WaveformGridComponent::setClip(aidaw::ClipId clipId) {
...
void WaveformGridComponent::setDisplayInfo(const aidaw::ClipDisplayInfo& info) {
    displayInfo_ = info;
    updateGridSize();
...
void WaveformGridComponent::setTimeRuler(aidaw::TimeRuler* ruler) {
    timeRuler_ = ruler;
    repaint();
}
...
void WaveformGridComponent::setWarpMarkers(const std::vector<aidaw::WarpMarkerInfo>& markers) {
...
void WaveformGridComponent::mouseDown(const juce::MouseEvent& event) {
    ...
    auto* thumbnail =
        aidaw::AudioThumbnailManager::getInstance().getThumbnail(clip->audio().source.filePath);
    if (thumbnail) {
...
void WaveformGridComponent::mouseDrag(const juce::MouseEvent& event) {
    ...
    if (dragMode_ == DragMode::Zoom) {
        ...
        if (deltaY > 0) {
            setMouseCursor(aidaw::CursorManager::getInstance().getZoomInCursor());
        } else if (deltaY < 0) {
            setMouseCursor(aidaw::CursorManager::getInstance().getZoomOutCursor());
        }
        if (onZoomDrag) {
    ...
    switch (dragMode_) {
        case DragMode::ResizeLeft: {
            ...
            double bpm = timeRuler_ ? timeRuler_->getTempo() : aidaw::DEFAULT_BPM;
            if (clip->loopEnabled) {
                aidaw::ClipOperations::moveLoopStart(*clip, newOffset, dragStartFileDuration_, bpm);
            } else {
                ...
                aidaw::ClipOperations::setAudioOffsetPreservingSourceRegion(
        ...
        case DragMode::PhaseMarker: {
            ...
            aidaw::ClipOperations::setAudioLoopPhaseClamped(*clip, newPhase);
            break;
        }
        case DragMode::ResizeRight: {
            double newLength = dragStartClipLength_ + deltaSeconds;
            newLength = juce::jmax(aidaw::ClipOperations::MIN_CLIP_LENGTH, newLength);
            ...
            if (snapEnabled_) {
                ...
                newLength =
                    juce::jmax(aidaw::ClipOperations::MIN_CLIP_LENGTH, activeEnd - offsetPosition);
            }
            ...
            if (dragStartFileDuration_ > 0.0) {
                ...
                newLength = juce::jlimit(
                    aidaw::ClipOperations::MIN_CLIP_LENGTH,
                    juce::jmax(aidaw::ClipOperations::MIN_CLIP_LENGTH, maxTimelineLength),
                    newLength);
            }

            double bpm = timeRuler_ ? timeRuler_->getTempo() : 120.0;
            aidaw::ClipOperations::resizeContainerFromRight(*clip, newLength, bpm);
            break;
        }
        case DragMode::StretchRight: {
            ...
            newExtent = juce::jmax(aidaw::ClipOperations::MIN_CLIP_LENGTH, newExtent);
            double stretchRatio = newExtent / dragStartLength_;
            double newSpeedRatio = dragStartSpeedRatio_ / stretchRatio;
            newSpeedRatio = juce::jlimit(aidaw::ClipOperations::MIN_SPEED_RATIO,
                                         aidaw::ClipOperations::MAX_SPEED_RATIO, newSpeedRatio);
    ...
    {
        double bpm = timeRuler_ ? timeRuler_->getTempo() : 120.0;
        displayInfo_ = aidaw::ClipDisplayInfo::from(*clip, bpm, dragStartFileDuration_);
        clipLength_ = clip->getTimelineLength(bpm);
        clipStartTime_ = clip->getTimelineStart(bpm);
    }
    ...
    if (currentTime - lastDragUpdateTime_ >= DRAG_UPDATE_INTERVAL_MS) {
        ...
        aidaw::ClipManager::getInstance().forceNotifyClipPropertyChanged(editingClipId_);
    }
...
void WaveformGridComponent::mouseUp(const juce::MouseEvent& /*event*/) {
    ...
    if (dragMode_ != DragMode::None && editingClipId_ != aidaw::INVALID_CLIP_ID) {
...
void WaveformGridComponent::mouseMove(const juce::MouseEvent& event) {
    ...
    if (warpMode_) {
        ...
        if (newHovered >= 0 && event.mods.isAltDown()) {
            ...
            setMouseCursor(aidaw::CursorManager::getInstance().getZoomCursor());
        } else if (isInsideWaveform(x, *clip)) {
...
bool WaveformGridComponent::isNearLeftEdge(int x, const aidaw::ClipInfo& clip) const {
    juce::ignoreUnused(clip);
...
bool WaveformGridComponent::isNearRightEdge(int x, const aidaw::ClipInfo& clip) const {
...
bool WaveformGridComponent::isNearPhaseMarker(int x, const aidaw::ClipInfo& clip) const {
...
bool WaveformGridComponent::isInsideWaveform(int x, const aidaw::ClipInfo& clip) const {
    juce::ignoreUnused(clip);
    double displayStartTime = getDisplayStartTime();
...
const aidaw::ClipInfo* WaveformGridComponent::getClip() const {
    return aidaw::ClipManager::getInstance().getClip(editingClipId_);
}
...
void WaveformGridComponent::paintWarpMarkers(juce::Graphics& g, const aidaw::ClipInfo& clip) {
    juce::ignoreUnused(clip);
    auto bounds = getLocalBounds().reduced(LEFT_PADDING, TOP_PADDING);
    if (bounds.getWidth() <= 0 || bounds.getHeight() <= 0)
        return;
...
void WaveformGridComponent::mouseDoubleClick(const juce::MouseEvent& event) {
    if (!warpMode_ || editingClipId_ == aidaw::INVALID_CLIP_ID)
        return;
