...
class ChordBlockComponent : public juce::Component {
  public:
    explicit ChordBlockComponent(const aidaw::music::Chord& chord);
    ...
    std::function<void(const aidaw::music::Chord&)> onClicked;
    /** Called on mouse release (for stopping preview). */
    std::function<void()> onReleased;

    const aidaw::music::Chord& getChord() const {
        return chord_;
    }
