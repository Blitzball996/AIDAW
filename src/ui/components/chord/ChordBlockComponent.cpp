...
ChordBlockComponent::ChordBlockComponent(const aidaw::music::Chord& chord) : chord_(chord) {
    setName("ChordBlock");
}
...
void ChordBlockComponent::mouseDrag(const juce::MouseEvent& e) {
    if (!dragging_ && e.getDistanceFromDragStart() > 4) {
        ...
        double tempo = aidaw::ProjectManager::getInstance().getCurrentProjectInfo().tempo;
        if (tempo <= 0.0)
            tempo = 120.0;
