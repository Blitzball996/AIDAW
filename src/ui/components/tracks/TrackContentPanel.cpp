...
void TrackContentPanel::paintEditCursor(juce::Graphics& g) {
    ...
    if (cursorTrackIndex < 0) {
        auto selectedClipId = aidaw::SelectionManager::getInstance().getSelectedClip();
        if (selectedClipId != aidaw::INVALID_CLIP_ID) {
            const auto* clip = aidaw::ClipManager::getInstance().getClip(selectedClipId);
            if (clip) {
                for (size_t i = 0; i < visibleTrackIds_.size(); ++i) {
