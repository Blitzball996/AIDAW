...
using aidaw::daw::audio::FaustCustomViewKind;
...
class FaustCustomUIRegistry {
  public:
    using Factory =
        std::function<std::unique_ptr<FaustCustomView>(aidaw::daw::audio::FaustPlugin&)>;
    ...
    std::unique_ptr<FaustCustomView> create(FaustCustomViewKind kind,
                                            aidaw::daw::audio::FaustPlugin& plugin) const;
