...
class StepSequencerUI : public juce::Component,
  ...
  private:
    ...
    // --- MIDI controls ---
    std::unique_ptr<aidaw::SvgButton> midiThruButton_;
    std::unique_ptr<aidaw::SvgButton> stepRecordButton_;

    // --- Pattern generation ---
    std::unique_ptr<aidaw::SvgButton> randomButton_;
    juce::TextButton aiButton_{"AI"};
    juce::TextEditor aiPromptEditor_;
    juce::Label aiModelLabel_;
    std::unique_ptr<aidaw::SvgButton> aiIcon_;
    std::unique_ptr<aidaw::SvgButton> aiClearButton_;
