...
namespace {
juce::String encodeStepTypes(const aidaw::ChainNodePath& path) {
...
juce::String encodeStepIds(const aidaw::ChainNodePath& path) {
    juce::StringArray values;
    for (const auto& step : path.steps)
...
void writePathToDragInfo(juce::DynamicObject& obj, const aidaw::ChainNodePath& path,
...
NodeComponent::NodeComponent() {
    ...
    aidaw::ControllerRegistry::getInstance().addListener(this);
    // === HEADER ===

    // Bypass button (power icon)
    bypassButton_ = std::make_unique<aidaw::SvgButton>("Power", BinaryData::power_on_svg,
...
NodeComponent::~NodeComponent() {
    ...
    aidaw::ControllerRegistry::getInstance().removeListener(this);
}
...
void NodeComponent::setNodePath(const aidaw::ChainNodePath& path) {
    nodePath_ = path;
...
void NodeComponent::selectionTypeChanged(aidaw::SelectionType newType) {
    ...
    if (newType != aidaw::SelectionType::ChainNode &&
        newType != aidaw::SelectionType::MultiChainNode &&
        newType != aidaw::SelectionType::Device) {
        setSelected(false);
    }
}

void NodeComponent::chainNodeSelectionChanged(const aidaw::ChainNodePath& /*path*/) {
...
void NodeComponent::chainNodeReselected(const aidaw::ChainNodePath& /*path*/) {
    // Not used - we handle collapse toggle directly in mouseUp
}

void NodeComponent::paramSelectionChanged(const aidaw::ParamSelection& /*selection*/) {}
        ...
        if (nodePath_.isValid()) {
            if (auto* container = juce::DragAndDropContainer::findParentDragContainerFor(this)) {
                ...
                auto paths = selection.isChainNodeSelected(nodePath_)
                                 ...
                                 : std::vector<aidaw::ChainNodePath>{nodePath_};
                if (paths.empty())
...
void NodeComponent::mouseUp(const juce::MouseEvent& e) {
    ...
    auto& linkMgr = aidaw::LinkModeManager::getInstance();
...
void NodeComponent::initializeModsMacrosPanels() {
    ...
    modsPanel_->onModTargetChanged = [this](int modIndex, aidaw::ControlTarget target) {
    ...
    modsPanel_->onAddModRequested = [this](int slotIndex, aidaw::ModType type,
    ...
    modsPanel_->onPanelClicked = [this]() {
        aidaw::SelectionManager::getInstance().selectModsPanel(nodePath_);
    };
    ...
    macroPanel_->onMacroTargetChanged = [this](int macroIndex, aidaw::ControlTarget target) {
    ...
    macroPanel_->onMacroLinkRemoved = [this](int macroIndex, aidaw::ControlTarget target) {
    ...
    macroPanel_->onPanelClicked = [this]() {
        aidaw::SelectionManager::getInstance().selectMacrosPanel(nodePath_);
    };
    ...
    modulatorEditorPanel_->onWaveformChanged = [this](aidaw::LFOWaveform waveform) {
        if (selectedModIndex_ >= 0) {
    ...
    modulatorEditorPanel_->onSyncDivisionChanged = [this](aidaw::SyncDivision division) {
        if (selectedModIndex_ >= 0) {
            onModSyncDivisionChangedInternal(selectedModIndex_, division);
        }
    };
    modulatorEditorPanel_->onTriggerModeChanged = [this](aidaw::LFOTriggerMode mode) {
        if (selectedModIndex_ >= 0) {
    ...
    modulatorEditorPanel_->onAdvancedClicked = [this]() {
        ...
        auto* device = aidaw::TrackManager::getInstance().getDeviceInChainByPath(nodePath_);
        auto* rack = device ? nullptr : aidaw::TrackManager::getInstance().getRackByPath(nodePath_);
        if (!device && !rack)
            return;
        ...
        const bool isAudioMode =
            (selectedModIndex_ >= 0 && selectedModIndex_ < (int)mods.size() &&
             mods[(size_t)selectedModIndex_].triggerMode == aidaw::LFOTriggerMode::Audio);
        const auto sidechainType =
            isAudioMode ? aidaw::SidechainConfig::Type::Audio : aidaw::SidechainConfig::Type::MIDI;

        juce::PopupMenu menu;

        bool hasSidechain =
            sidechain.type == sidechainType && sidechain.sourceTrackId != aidaw::INVALID_TRACK_ID;
        ...
        int itemId = 10;
        for (const auto& track : aidaw::TrackManager::getInstance().getTracks()) {
        ...
        auto deviceId = device ? device->id : aidaw::INVALID_DEVICE_ID;
        ...
        menu.showMenuAsync(juce::PopupMenu::Options(), [safeThis, isDeviceTarget, deviceId,
            ...
            if (result == 1) {
                if (isDeviceTarget)
                    ...
                    aidaw::TrackManager::getInstance().clearRackSidechain(rackPath);
            } else {
                int index = result - 10;
                if (index >= 0 && index < (int)trackEntries->size()) {
                    if (isDeviceTarget) {
                        ...
                        aidaw::TrackManager::getInstance().setRackSidechainSource(
    ...
    modulatorEditorPanel_->setParamNameResolver([this](aidaw::DeviceId deviceId,
                                                       int paramIndex) -> juce::String {
        auto* device = aidaw::TrackManager::getInstance().getDeviceInChainByPath(nodePath_);
        if (!device) {
            // Try rack: look up device by ID across all chains
            auto* rack = aidaw::TrackManager::getInstance().getRackByPath(nodePath_);
            if (rack) {
                for (auto& chain : rack->chains) {
                    for (auto& element : chain.elements) {
                        if (aidaw::isDevice(element) && aidaw::getDevice(element).id == deviceId) {
                            device = &aidaw::getDevice(element);
                            break;
                        }
                    }
                    if (device)
    ...
    modulatorEditorPanel_->onModLinkDeleted = [this](int modIndex, aidaw::ControlTarget target) {
        auto* device = aidaw::TrackManager::getInstance().getDeviceInChainByPath(nodePath_);
        if (device) {
    ...
    modulatorEditorPanel_->onModLinkBipolarChanged =
        [this](int modIndex, aidaw::ControlTarget target, bool bipolar) {
            auto* device = aidaw::TrackManager::getInstance().getDeviceInChainByPath(nodePath_);
            if (device) {
                aidaw::TrackManager::getInstance().setModLinkBipolar(nodePath_, modIndex, target,
                                                                     bipolar);
            } else {
                aidaw::TrackManager::getInstance().setModLinkBipolar(nodePath_, modIndex, target,
                                                                     bipolar);
    ...
    modulatorEditorPanel_->onModLinkAmountChanged =
        [this](int modIndex, aidaw::ControlTarget target, float amount) {
            auto* device = aidaw::TrackManager::getInstance().getDeviceInChainByPath(nodePath_);
            if (device) {
                aidaw::TrackManager::getInstance().setModLinkAmount(nodePath_, modIndex, target,
                                                                    amount);
            } else {
                aidaw::TrackManager::getInstance().setModLinkAmount(nodePath_, modIndex, target,
                                                                    amount);
    ...
    macroEditorPanel_->onLinkAmountChanged = [this](aidaw::ControlTarget target, float amount) {
        if (selectedMacroIndex_ >= 0) {
            onMacroLinkAmountChangedInternal(selectedMacroIndex_, target, amount);
        }
    };
    macroEditorPanel_->onLinkRemoved = [this](aidaw::ControlTarget target) {
        if (selectedMacroIndex_ >= 0) {
            onMacroLinkRemovedInternal(selectedMacroIndex_, target);
            updateMacroEditor();
        }
    };
    macroEditorPanel_->onLinkBipolarToggled = [this](aidaw::ControlTarget target, bool bipolar) {
        if (selectedMacroIndex_ >= 0) {
...
void NodeComponent::updateModsPanel() {
    ...
    std::vector<std::pair<aidaw::ModId, juce::String>> modList;
    if (mods) {
...
void NodeComponent::updateMacroPanel() {
    ...
    std::vector<std::pair<aidaw::ModId, juce::String>> mods;
...
void NodeComponent::updateModulatorEditor() {
    ...
    if (selectedModIndex_ < static_cast<int>(mods->size())) {
        ...
        auto getter = [this, modIdx]() -> const aidaw::ModInfo* {
