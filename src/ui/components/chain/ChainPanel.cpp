  ...
  public:
    ...
    void itemDropped(const SourceDetails& details) override {
        ...
        if (validDrop) {
            ...
            aidaw::TrackManager::getInstance().addDeviceToChainByPath(chainPath, device,
                                                                      insertIndex);
...
void ChainPanel::showChain(const aidaw::ChainNodePath& chainPath) {
    ...
    if (!chainPath.steps.empty()) {
        for (const auto& step : chainPath.steps) {
            if (step.type == aidaw::ChainStepType::Rack) {
                rackId_ = step.id;
            } else if (step.type == aidaw::ChainStepType::Chain) {
                chainId_ = step.id;
            }
        }
    }
    ...
    auto resolved = aidaw::TrackManager::getInstance().resolvePath(chainPath);
...
void ChainPanel::showChain(aidaw::TrackId trackId, aidaw::RackId rackId, aidaw::ChainId chainId) {
    // Build the path from IDs and call the path-based version
    auto chainPath = aidaw::ChainNodePath::chain(trackId, rackId, chainId);
    showChain(chainPath);
}
...
void ChainPanel::refresh() {
    ...
    auto resolved = aidaw::TrackManager::getInstance().resolvePath(chainPath_);
    if (resolved.valid && resolved.chain) {
        setNodeName(resolved.chain->name);
    }
...
void ChainPanel::onAddDeviceClicked() {
    ...
    menu.showMenuAsync(juce::PopupMenu::Options(), [safeThis, chainPath, capturedPlugins,
        ...
        if (result == 10000) {
            aidaw::TrackManager::getInstance().addRackToChainByPath(chainPath);
            if (safeThis != nullptr) {
...
void ChainPanel::clearDeviceSelection() {
    ...
    if (onDeviceSelected) {
        onDeviceSelected(aidaw::INVALID_DEVICE_ID);
    }
    // Clear centralized selection so re-selecting the same node works
    aidaw::SelectionManager::getInstance().clearChainNodeSelection();
}

void ChainPanel::onDeviceSlotSelected(aidaw::DeviceId deviceId) {
