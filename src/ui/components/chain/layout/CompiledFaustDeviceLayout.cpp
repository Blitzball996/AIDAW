...
int findParamArrayIndex(const aidaw::DeviceInfo& device, int poolIdx) {
...
bool gateEnabled(const aidaw::DeviceInfo& device, const aidaw::ParameterInfo& param) {
...
int CompiledFaustDeviceLayout::totalPages(const aidaw::DeviceInfo&) const {
    return 1;
}

ParamCell CompiledFaustDeviceLayout::cellFor(const aidaw::DeviceInfo& device, int cellIndex,
                                             int) const {
    ParamCell cell;
    ...
    if (param.scale == aidaw::ParameterScale::Discrete && param.choices.size() <= 1) {
        cell.mode = ParamCell::Mode::Hidden;
        return cell;
    }
