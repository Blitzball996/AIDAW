...
int visibleCountFor(const aidaw::DeviceInfo& device) {
...
int paramArrayIndexFor(const aidaw::DeviceInfo& device, int slotIndex) {
...
int StandardDeviceLayout::totalPages(const aidaw::DeviceInfo& device) const {
    const int total = std::max(1, (visibleCountFor(device) + kCellCount - 1) / kCellCount);
    return total;
}

ParamCell StandardDeviceLayout::cellFor(const aidaw::DeviceInfo& device, int cellIndex,
