...
void AddModButton::showAddMenu() {
    ...
    menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(this), [this](int result) {
        if (result == 1 && onAddMod) {
            ...
            onAddMod(aidaw::ModType::LFO, aidaw::LFOWaveform::Sine);
        } else if (result == 2 && onAddMod) {
            // Curve LFO with custom waveform
            onAddMod(aidaw::ModType::LFO, aidaw::LFOWaveform::Custom);
        }
    });
}
...
ModsPanelComponent::ModsPanelComponent() : PagedControlPanel(aidaw::MODS_PER_PAGE) {
    ...
    onAddPageRequested = [this](int /*itemsPerPage*/) {
        allocatedPages_++;
        ensureSlotCount(allocatedPages_ * aidaw::MODS_PER_PAGE);
        resized();
        repaint();
    };
    ...
    onRemovePageRequested = [this](int /*itemsPerPage*/) {
        if (allocatedPages_ > 1) {
            ...
            int lastPageStartIndex = (allocatedPages_ - 1) * aidaw::MODS_PER_PAGE;
...
void ModsPanelComponent::ensureKnobCount(int count) {
    ...
    while (static_cast<int>(knobs_.size()) < count) {
        ...
        knob->onTargetChanged = [this, i](aidaw::ControlTarget target) {
...
void ModsPanelComponent::ensureSlotCount(int count) {
    ...
    while (static_cast<int>(addButtons_.size()) < count) {
        ...
        addButton->onAddMod = [this, slotIndex](aidaw::ModType type, aidaw::LFOWaveform waveform) {
...
void ModsPanelComponent::setMods(const aidaw::ModArray& mods) {
    ...
    int requiredPages = currentModCount_ > 0
                            ? (currentModCount_ + aidaw::MODS_PER_PAGE - 1) / aidaw::MODS_PER_PAGE
                            : 1;
...
void ModsPanelComponent::setParentPath(const aidaw::ChainNodePath& path) {
    parentPath_ = path;
    for (auto& knob : knobs_) {
...
int ModsPanelComponent::getTotalItemCount() const {
    ...
    return allocatedPages_ * aidaw::MODS_PER_PAGE;
}
