...
class WaveformDisplay : public juce::Component, private juce::Timer {
  public:
    ...
    void setModInfo(const aidaw::ModInfo* mod,
                    std::function<const aidaw::ModInfo*()> getter = nullptr) {
        mod_ = mod;
    ...
    void paint(juce::Graphics& g) override {
        ...
        const aidaw::ModInfo* mod = modGetter_ ? modGetter_() : mod_;
        if (!mod) {
            return;
        }
        ...
        for (int i = 0; i < numPoints; ++i) {
            ...
            float value = aidaw::ModulatorEngine::generateWaveformForMod(*mod, effectivePhase);
  ...
  private:
    ...
    const aidaw::ModInfo* mod_ = nullptr;
    std::function<const aidaw::ModInfo*()> modGetter_;
...
class ModMatrixContent : public juce::Component {
  public:
    ...
    bool updateLinkAmount(aidaw::ControlTarget target, float amount, bool bipolar);

    // Callbacks
    std::function<void(aidaw::ControlTarget target)> onDeleteLink;
    std::function<void(aidaw::ControlTarget target, bool bipolar)> onToggleBipolar;
    std::function<void(aidaw::ControlTarget target, float amount)> onAmountChanged;
...
class ModulatorEditorPanel : public juce::Component,
  ...
  public:
    ...
    void setModInfo(const aidaw::ModInfo& mod, const aidaw::ModInfo* liveMod = nullptr,
                    std::function<const aidaw::ModInfo*()> liveModGetter = nullptr);
    ...
    void setOwnerPath(aidaw::TrackId trackId, const aidaw::ChainNodePath& devicePath);

    // Set a resolver for getting parameter names from device/param IDs
    void setParamNameResolver(std::function<juce::String(aidaw::DeviceId, int)> resolver) {
    ...
    std::function<void(aidaw::LFOWaveform waveform)> onWaveformChanged;
    std::function<void(bool tempoSync)> onTempoSyncChanged;
    std::function<void(aidaw::SyncDivision division)> onSyncDivisionChanged;
    std::function<void(aidaw::LFOTriggerMode mode)> onTriggerModeChanged;
    ...
    std::function<void(int modIndex, aidaw::ControlTarget target)> onModLinkDeleted;
    std::function<void(int modIndex, aidaw::ControlTarget target, bool bipolar)>
    ...
    void macroLinkModeChanged(bool active, const aidaw::MacroSelection& sel) override;
    void modLinkModeChanged(bool active, const aidaw::ModSelection& sel) override;
  ...
  private:
    ...
    aidaw::ModInfo currentMod_;
    aidaw::TrackId ownerTrackId_ = aidaw::INVALID_TRACK_ID;
    aidaw::ChainNodePath ownerDevicePath_;
    ...
    const aidaw::ModInfo* liveModPtr_ = nullptr;  // Pointer to live mod for waveform animation
    std::function<const aidaw::ModInfo*()> liveModGetter_;  // Safe getter for timer callback

    // UI Components
    juce::Label nameLabel_;
    juce::ComboBox waveformCombo_;  // LFO shape selector (Sine, Triangle, etc.)
    WaveformDisplay waveformDisplay_;
    aidaw::LFOCurveEditor curveEditor_;                        // Custom waveform editor
    std::unique_ptr<aidaw::SvgButton> curveEditorButton_;      // Button to open external editor
    ...
    std::unique_ptr<aidaw::SvgButton> savePresetButton_;       // Save preset button
    ...
    std::unique_ptr<aidaw::SvgButton> advancedButton_;
    ...
    std::function<juce::String(aidaw::DeviceId, int)> paramNameResolver_;
