...
MacroPanelComponent::MacroPanelComponent() : PagedControlPanel(aidaw::MACROS_PER_PAGE) {
    ...
    ensureKnobCount(aidaw::NUM_MACROS);
}
...
void MacroPanelComponent::ensureKnobCount(int count) {
    ...
    while (static_cast<int>(knobs_.size()) < count) {
        ...
        knob->onLinkRemoved = [this, i](aidaw::ControlTarget target) {
...
void MacroPanelComponent::setMacros(const aidaw::MacroArray& macros) {
...
void MacroPanelComponent::setParentPath(const aidaw::ChainNodePath& path) {
    parentPath_ = path;
    for (auto& knob : knobs_) {
