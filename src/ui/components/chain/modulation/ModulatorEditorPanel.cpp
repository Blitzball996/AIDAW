...
constexpr aidaw::SyncDivision kSyncDivisionOrder[] = {
    aidaw::SyncDivision::SixteenBars,         // 16 Bars
    aidaw::SyncDivision::EightBars,           // 8 Bars
    aidaw::SyncDivision::FourBars,            // 4 Bars
    aidaw::SyncDivision::TwoBars,             // 2 Bars
    aidaw::SyncDivision::Whole,               // 1 Bar
    aidaw::SyncDivision::DottedHalf,          // 1/2.
    aidaw::SyncDivision::Half,                // 1/2
    aidaw::SyncDivision::TripletHalf,         // 1/2T
    aidaw::SyncDivision::DottedQuarter,       // 1/4.
    aidaw::SyncDivision::Quarter,             // 1/4
    aidaw::SyncDivision::TripletQuarter,      // 1/4T
    aidaw::SyncDivision::DottedEighth,        // 1/8.
    aidaw::SyncDivision::Eighth,              // 1/8
    aidaw::SyncDivision::TripletEighth,       // 1/8T
    aidaw::SyncDivision::DottedSixteenth,     // 1/16.
    aidaw::SyncDivision::Sixteenth,           // 1/16
    aidaw::SyncDivision::TripletSixteenth,    // 1/16T
    aidaw::SyncDivision::DottedThirtySecond,  // 1/32.
    aidaw::SyncDivision::ThirtySecond,        // 1/32
    aidaw::SyncDivision::TripletThirtySecond  // 1/32T
};

int syncDivisionToIndex(aidaw::SyncDivision d) {
    ...
    for (int i = 0; i < static_cast<int>(std::size(kSyncDivisionOrder)); ++i)
        if (kSyncDivisionOrder[i] == aidaw::SyncDivision::Quarter)
            return i;
    return 0;
}

aidaw::SyncDivision indexToSyncDivision(int idx) {
    if (idx < 0 || idx >= static_cast<int>(std::size(kSyncDivisionOrder)))
        return aidaw::SyncDivision::Quarter;
...
juce::String buildQualifiedModRateName(const aidaw::ChainNodePath& path,
                                       const juce::String& modName) {
    juce::String suffix = modName + " Rate";
    auto& tm = aidaw::TrackManager::getInstance();
    switch (path.getType()) {
        case aidaw::ChainNodeType::Track:
            return suffix;
        case aidaw::ChainNodeType::Rack:
            if (auto* rack = tm.getRackByPath(path))
                return rack->name + ": " + suffix;
            break;
        case aidaw::ChainNodeType::TopLevelDevice:
        case aidaw::ChainNodeType::Device:
            if (auto* dev = tm.getDeviceInChainByPath(path))
                return dev->name + ": " + suffix;
            break;
        case aidaw::ChainNodeType::None:
        case aidaw::ChainNodeType::Chain:
...
bool ModMatrixContent::updateLinkAmount(aidaw::ControlTarget target, float amount, bool bipolar) {
...
ModulatorEditorPanel::ModulatorEditorPanel() {
    ...
    aidaw::BindingRegistry::getInstance().addListener(this);
    ...
    waveformCombo_.addItem("Sine", static_cast<int>(aidaw::LFOWaveform::Sine) + 1);
    waveformCombo_.addItem("Triangle", static_cast<int>(aidaw::LFOWaveform::Triangle) + 1);
    waveformCombo_.addItem("Square", static_cast<int>(aidaw::LFOWaveform::Square) + 1);
    waveformCombo_.addItem("Saw", static_cast<int>(aidaw::LFOWaveform::Saw) + 1);
    waveformCombo_.addItem("Reverse Saw", static_cast<int>(aidaw::LFOWaveform::ReverseSaw) + 1);
    ...
    waveformCombo_.onChange = [this]() {
        ...
        if (id > 0 && onWaveformChanged) {
            onWaveformChanged(static_cast<aidaw::LFOWaveform>(id - 1));
        }
    };
    ...
    curveEditorButton_ = std::make_unique<aidaw::SvgButton>("Edit Curve", BinaryData::curve_svg,
    ...
    curveEditorButton_->onClick = [this]() {
        if (!curveEditorWindow_) {
            auto* modInfo = const_cast<aidaw::ModInfo*>(liveModPtr_ ? liveModPtr_ : &currentMod_);
            ...
            curveEditorWindow_->onSyncDivisionChanged = [this](aidaw::SyncDivision div) {
            ...
            curveEditorWindow_->getCurveEditor().setModInfo(
                const_cast<aidaw::ModInfo*>(liveModPtr_ ? liveModPtr_ : &currentMod_));
    ...
    curvePresetCombo_.addItem("Triangle", static_cast<int>(aidaw::CurvePreset::Triangle) + 1);
    curvePresetCombo_.addItem("Sine", static_cast<int>(aidaw::CurvePreset::Sine) + 1);
    curvePresetCombo_.addItem("Ramp Up", static_cast<int>(aidaw::CurvePreset::RampUp) + 1);
    curvePresetCombo_.addItem("Ramp Down", static_cast<int>(aidaw::CurvePreset::RampDown) + 1);
    curvePresetCombo_.addItem("S-Curve", static_cast<int>(aidaw::CurvePreset::SCurve) + 1);
    curvePresetCombo_.addItem("Exp", static_cast<int>(aidaw::CurvePreset::Exponential) + 1);
    curvePresetCombo_.addItem("Log", static_cast<int>(aidaw::CurvePreset::Logarithmic) + 1);
    ...
    curvePresetCombo_.onChange = [this]() {
        ...
        if (id > 0) {
            auto preset = static_cast<aidaw::CurvePreset>(id - 1);
    ...
    savePresetButton_ = std::make_unique<aidaw::SvgButton>("Save Preset", BinaryData::save_svg,
    ...
    syncDivisionSlider_.setValue(
        static_cast<double>(syncDivisionToIndex(aidaw::SyncDivision::Quarter)),
        juce::dontSendNotification);
    ...
    triggerModeCombo_.addItem("Free", static_cast<int>(aidaw::LFOTriggerMode::Free) + 1);
    triggerModeCombo_.addItem("Transport", static_cast<int>(aidaw::LFOTriggerMode::Transport) + 1);
    triggerModeCombo_.addItem("MIDI", static_cast<int>(aidaw::LFOTriggerMode::MIDI) + 1);
    triggerModeCombo_.addItem("Audio", static_cast<int>(aidaw::LFOTriggerMode::Audio) + 1);
    triggerModeCombo_.setSelectedId(static_cast<int>(aidaw::LFOTriggerMode::Free) + 1,
                                    juce::dontSendNotification);
    ...
    advancedButton_ = std::make_unique<aidaw::SvgButton>("Advanced", BinaryData::settings_nobg_svg,
    ...
    modMatrixContent_.onDeleteLink = [this](aidaw::ControlTarget target) {
        if (selectedModIndex_ >= 0 && onModLinkDeleted) {
            onModLinkDeleted(selectedModIndex_, target);
        }
    };
    modMatrixContent_.onToggleBipolar = [this](aidaw::ControlTarget target, bool bipolar) {
        if (selectedModIndex_ >= 0 && onModLinkBipolarChanged) {
            onModLinkBipolarChanged(selectedModIndex_, target, bipolar);
        }
    };
    modMatrixContent_.onAmountChanged = [this](aidaw::ControlTarget target, float amount) {
...
ModulatorEditorPanel::~ModulatorEditorPanel() {
    ...
    aidaw::BindingRegistry::getInstance().removeListener(this);
    aidaw::MidiLearnCoordinator::getInstance().removeListener(this);
    stopTimer();
}

void ModulatorEditorPanel::setModInfo(const aidaw::ModInfo& mod, const aidaw::ModInfo* liveMod,
                                      std::function<const aidaw::ModInfo*()> liveModGetter) {
    currentMod_ = mod;
    liveModPtr_ = liveMod;
...
void ModulatorEditorPanel::setOwnerPath(aidaw::TrackId trackId,
...
void ModulatorEditorPanel::updateRateAutomationTarget() {
    if (ownerTrackId_ == aidaw::INVALID_TRACK_ID || currentMod_.id == aidaw::INVALID_MOD_ID) {
    ...
    target.kind = aidaw::ControlTarget::Kind::ModParam;
...
void ModulatorEditorPanel::showRateSliderContextMenu() {
    ...
    struct UnlinkEntry {
        SrcKind kind;
        aidaw::ChainNodePath parentPath;
        int index;
    };
    std::vector<UnlinkEntry> unlinks;

    const auto& tm = aidaw::TrackManager::getInstance();
    const auto* track = tm.getTrack(ownerTrackId_);
    if (track) {
        auto modId = currentMod_.id;
        auto considerMacros = [&](const std::vector<aidaw::MacroInfo>& macros,
                                  const aidaw::ChainNodePath& parent) {
            for (size_t i = 0; i < macros.size(); ++i) {
                for (const auto& l : macros[i].links) {
                    if (l.target.kind == aidaw::ControlTarget::Kind::ModParam &&
        ...
        auto considerMods = [&](const std::vector<aidaw::ModInfo>& mods,
                                const aidaw::ChainNodePath& parent) {
        ...
        considerMacros(track->macros, aidaw::ChainNodePath::trackLevel(ownerTrackId_));
        considerMods(track->mods, aidaw::ChainNodePath::trackLevel(ownerTrackId_));
    ...
    {
        auto& reg = aidaw::BindingRegistry::getInstance();
        auto& learn = aidaw::MidiLearnCoordinator::getInstance();
        const bool isLearning = learn.isLearning(
            aidaw::ControlTarget::modParam(ownerDevicePath_, currentMod_.id, /*modParamIndex=*/0));
        const int mappingCount = static_cast<int>(
            reg.findFor(aidaw::ControlTarget::modParam(ownerDevicePath_, currentMod_.id,
                                                       /*modParamIndex=*/0))
                .size());
    ...
    menu.showMenuAsync(juce::PopupMenu::Options(), [safeThis, target, unlinks, rateModId, learnPath,
        ...
        if (result == kShowLaneId) {
            ...
            auto laneId = mgr.getOrCreateLane(target, aidaw::AutomationLaneType::Absolute);
        ...
        if (result == kLearnId) {
            auto& learn = aidaw::MidiLearnCoordinator::getInstance();
            const auto target =
                aidaw::ControlTarget::modParam(learnPath, rateModId, /*modParamIndex=*/0);
        ...
        if (result == kClearMidiId) {
            aidaw::MidiLearnCoordinator::getInstance().clearMappings(
                aidaw::ControlTarget::modParam(learnPath, rateModId, /*modParamIndex=*/0));
            return;
        }
        ...
        auto& tmgr = aidaw::TrackManager::getInstance();
        if (e.kind == SrcKind::Macro) {
            aidaw::ControlTarget t;
            t.kind = aidaw::ControlTarget::Kind::ModParam;
            ...
            } else if (e.parentPath.getType() == aidaw::ChainNodeType::Rack) {
...
void ModulatorEditorPanel::updateFromMod() {
    ...
    isCurveMode_ = (currentMod_.waveform == aidaw::LFOWaveform::Custom);
    ...
    bool hasSidechainConfig = (currentMod_.triggerMode == aidaw::LFOTriggerMode::MIDI ||
                               currentMod_.triggerMode == aidaw::LFOTriggerMode::Audio);
    advancedButton_->setEnabled(hasSidechainConfig);

    // Audio envelope sliders (only visible when trigger mode = Audio)
    bool isAudioTrigger = (currentMod_.triggerMode == aidaw::LFOTriggerMode::Audio);
    audioAttackSlider_.setVisible(isAudioTrigger);
    audioReleaseSlider_.setVisible(isAudioTrigger);
    if (isAudioTrigger) {
...
void ModulatorEditorPanel::paintOverChildren(juce::Graphics& g) {
    ...
    if (linkModeActiveAndInScope_) {
        ...
        auto sb = visSlider.getBounds();
        if (!sb.isEmpty()) {
            auto& lmm = aidaw::LinkModeManager::getInstance();
            bool macro = (lmm.getLinkModeType() == aidaw::LinkModeType::Macro);
    ...
    if (currentMod_.id == aidaw::INVALID_MOD_ID || ownerTrackId_ == aidaw::INVALID_TRACK_ID)
        return;
    ...
    if (!track)
        return;
    const std::vector<aidaw::MacroInfo>* macros = nullptr;
    const std::vector<aidaw::ModInfo>* mods = nullptr;
    if (ownerDevicePath_.isValid()) {
        auto resolved = tm.resolvePath(ownerDevicePath_);
        if (resolved.valid && resolved.rack) {
...
void ModulatorEditorPanel::mouseDown(const juce::MouseEvent& e) {
    ...
    if (linkModeActiveAndInScope_ && e.mods.isLeftButtonDown()) {
        ...
        if (visibleSlider.getBounds().contains(e.getPosition())) {
            ...
            // resolvePath.
            auto findMacrosAt =
                [&](const aidaw::ChainNodePath& p) -> const std::vector<aidaw::MacroInfo>* {
                if (p.isTrackLevel) {
            ...
            auto findModsAt =
                [&](const aidaw::ChainNodePath& p) -> const std::vector<aidaw::ModInfo>* {
                if (p.isTrackLevel) {
                    if (const auto* t = tm.getTrack(p.trackId))
                        return &t->mods;
                    return nullptr;
            ...
            if (lmm.getLinkModeType() == aidaw::LinkModeType::Macro) {
            ...
            } else if (lmm.getLinkModeType() == aidaw::LinkModeType::Mod) {
            ...
            bool macroSrc = (lmm.getLinkModeType() == aidaw::LinkModeType::Macro);
...
void ModulatorEditorPanel::macroLinkModeChanged(bool active,
    ...
    auto ownerScope = ownerDevicePath_.isValid() ? ownerDevicePath_
                                                 : aidaw::ChainNodePath::trackLevel(ownerTrackId_);
    bool inScope = active && ownerScope.isValid() &&
                   aidaw::daw::ui::isInScopeOf(ownerScope, selection.parentPath);
    linkModeActiveAndInScope_ = inScope;
    applyLinkModeToRateSliders();
    repaint();
}

void ModulatorEditorPanel::modLinkModeChanged(bool active, const aidaw::ModSelection& selection) {
    auto ownerScope = ownerDevicePath_.isValid() ? ownerDevicePath_
                                                 : aidaw::ChainNodePath::trackLevel(ownerTrackId_);
...
void ModulatorEditorPanel::writeLinkAmountFromActiveSource(float amount) {
    if (currentMod_.id == aidaw::INVALID_MOD_ID)
        return;
    ...
    if (lmm.getLinkModeType() == aidaw::LinkModeType::Macro) {
        ...
        if (sel.parentPath.isTrackLevel) {
            tm.setMacroLinkAmount(ChainNodePath::trackLevel(sel.parentPath.trackId), sel.macroIndex,
                                  target, amount);
        } else if (sel.parentPath.getType() == aidaw::ChainNodeType::Rack) {
...
void ModulatorEditorPanel::refreshRateMidiBindingState() {
    bool newState = false;
    if (currentMod_.id != aidaw::INVALID_MOD_ID) {
        newState = aidaw::BindingRegistry::getInstance().hasActiveBindingFor(
            aidaw::ControlTarget::modParam(ownerDevicePath_, currentMod_.id, /*modParamIndex=*/0));
    }
...
void ModulatorEditorPanel::bindingRegistryChanged(aidaw::BindingScope) {
    refreshRateMidiBindingState();
}

void ModulatorEditorPanel::midiLearnStateChanged(const aidaw::ChainNodePath& path, int paramIndex,
                                                 aidaw::ControlTarget::Kind owner, bool learning) {
    if (owner != aidaw::ControlTarget::Kind::ModParam)
        return;
    bool isMe =
        (path == ownerDevicePath_ && paramIndex == 0 && currentMod_.id != aidaw::INVALID_MOD_ID);
    isRateInMidiLearnMode_ = learning && isMe;
    repaint();
}

void ModulatorEditorPanel::midiLearnCompleted(const aidaw::ChainNodePath& path, int paramIndex,
...
void ModulatorEditorPanel::midiLearnCleared(const aidaw::ChainNodePath& path, int paramIndex,
                                            aidaw::ControlTarget::Kind owner, int) {
    if (owner != aidaw::ControlTarget::Kind::ModParam)
        return;
    if (path == ownerDevicePath_ && paramIndex == 0)
        refreshRateMidiBindingState();
