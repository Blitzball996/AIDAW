...
juce::String buildQualifiedMacroName(const aidaw::ChainNodePath& path, int macroIndex,
...
bool findDevicePathInElements(const std::vector<aidaw::ChainElement>& elements,
                              const aidaw::ChainNodePath& parentPath, aidaw::DeviceId deviceId,
                              aidaw::ChainNodePath& outPath) {
    for (const auto& element : elements) {
        if (aidaw::isDevice(element)) {
            const auto& device = aidaw::getDevice(element);
            if (device.id == deviceId) {
                outPath = parentPath.isTrackLevel
                              ? aidaw::ChainNodePath::topLevelDevice(parentPath.trackId, deviceId)
            ...
            auto rackPath = parentPath.isTrackLevel
                                ? aidaw::ChainNodePath::rack(parentPath.trackId, rack.id)
...
aidaw::ChainNodePath resolveTargetDevicePath(const aidaw::ChainNodePath& parentPath,
                                             aidaw::DeviceId deviceId) {
    if (parentPath.getDeviceId() == deviceId)
        return parentPath;

    auto& tm = aidaw::TrackManager::getInstance();
    aidaw::ChainNodePath resolved;
    ...
    } else if (parentPath.getType() == aidaw::ChainNodeType::Rack) {
...
MacroKnobComponent::MacroKnobComponent(int macroIndex) : macroIndex_(macroIndex) {
    ...
    linkButton_ = std::make_unique<aidaw::SvgButton>("Link", BinaryData::link_flat_svg,
...
void MacroKnobComponent::updateAutomationTarget() {
    ...
    if (parentPath_.trackId == aidaw::INVALID_TRACK_ID || macroIndex_ < 0) {
        valueSlider_.clearAutomationTarget();
        return;
    }

    aidaw::AutomationTarget target;
    target.kind = aidaw::ControlTarget::Kind::DeviceMacro;
...
aidaw::AutomationTarget MacroKnobComponent::makeAutomationTarget() const {
    aidaw::AutomationTarget target;
    target.kind = aidaw::ControlTarget::Kind::DeviceMacro;
    target.devicePath = parentPath_;
    target.paramIndex = macroIndex_;
...
void MacroKnobComponent::beginAutomationGesture() {
    ...
    if (mgr.getLaneForTarget(target) != aidaw::INVALID_AUTOMATION_LANE_ID)
...
void MacroKnobComponent::refreshAutomapState() {
    ...
    bool active =
        reg.hasActiveBindingFor(aidaw::ControlTarget::deviceMacro(parentPath_, macroIndex_));
...
void MacroKnobComponent::setMacroInfo(const aidaw::MacroInfo& macro) {
    currentMacro_ = macro;
...
void MacroKnobComponent::setAvailableTargets(
    const std::vector<std::pair<aidaw::DeviceId, juce::String>>& devices) {
    availableTargets_ = devices;
}

void MacroKnobComponent::setDeviceParamNames(
    const std::map<aidaw::DeviceId, std::vector<juce::String>>& paramNames) {
    deviceParamNames_ = paramNames;
}

void MacroKnobComponent::setAvailableModifiers(
    const std::vector<std::pair<aidaw::ModId, juce::String>>& mods) {
...
void MacroKnobComponent::paint(juce::Graphics& g) {
    ...
    bool isInLinkMode =
        aidaw::LinkModeManager::getInstance().isMacroInLinkMode(parentPath_, macroIndex_);
...
void MacroKnobComponent::macroLinkModeChanged(bool active, const aidaw::MacroSelection& selection) {
...
void MacroKnobComponent::onLinkButtonClicked() {
    ...
    aidaw::LinkModeManager::getInstance().toggleMacroLinkMode(parentPath_, macroIndex_);
}
...
void MacroKnobComponent::showLinkMenu() {
    ...
    std::vector<aidaw::ControlTarget> unlinkTargets;
    ...
    if (parentPath_.isValid()) {
        ...
        const bool isLearning =
            learn.isLearning(aidaw::ControlTarget::deviceMacro(parentPath_, macroIndex_));
        const int mappingCount = static_cast<int>(
            reg.findFor(aidaw::ControlTarget::deviceMacro(parentPath_, macroIndex_)).size());
    ...
    menu.showMenuAsync(juce::PopupMenu::Options(), [safeThis, targets, paramNames, modifiers,
        ...
        if (result == kLearnId) {
            auto& learn = aidaw::MidiLearnCoordinator::getInstance();
            const auto target = aidaw::ControlTarget::deviceMacro(parentPath, macroIndex);
        ...
        if (result == kClearMidiId) {
            aidaw::MidiLearnCoordinator::getInstance().clearMappings(
                aidaw::ControlTarget::deviceMacro(parentPath, macroIndex));
            return;
        }
