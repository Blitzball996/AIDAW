...
class AddModButton : public juce::Component {
  public:
    ...
    std::function<void(aidaw::ModType, aidaw::LFOWaveform)> onAddMod;
...
class ModsPanelComponent : public PagedControlPanel {
  public:
    ...
    void setMods(const aidaw::ModArray& mods);

    // Set available devices for linking (devices in this rack/chain)
    void setAvailableDevices(const std::vector<std::pair<aidaw::DeviceId, juce::String>>& devices);
    ...
    void setAvailableModifiers(const std::vector<std::pair<aidaw::ModId, juce::String>>& modifiers);
    ...
    // Callbacks
    std::function<void(int modIndex, aidaw::ControlTarget target)> onModTargetChanged;
    ...
    std::function<void(int slotIndex, aidaw::ModType type, aidaw::LFOWaveform waveform)>
  ...
  private:
    ...
    std::vector<std::pair<aidaw::DeviceId, juce::String>> availableDevices_;
    std::vector<std::pair<aidaw::ModId, juce::String>> availableModifiers_;
    aidaw::ChainNodePath parentPath_;
