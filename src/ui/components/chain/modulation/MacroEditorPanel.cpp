...
MacroEditorPanel::MacroEditorPanel() {
    ...
    linkMatrixContent_.onDeleteLink = [this](aidaw::ControlTarget target) {
        if (onLinkRemoved)
            onLinkRemoved(target);
    };
    linkMatrixContent_.onAmountChanged = [this](aidaw::ControlTarget target, float amount) {
        if (onLinkAmountChanged)
            onLinkAmountChanged(target, amount);
    };
    linkMatrixContent_.onToggleBipolar = [this](aidaw::ControlTarget target, bool bipolar) {
...
void MacroEditorPanel::setMacroInfo(const aidaw::MacroInfo& macro) {
    currentMacro_ = macro;
    updateFromMacro();
}
...
void MacroEditorPanel::updateFromMacro() {
    ...
    for (const auto& link : currentMacro_.links) {
        ...
        if (link.target.kind == aidaw::ControlTarget::Kind::ModParam) {
