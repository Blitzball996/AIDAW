...
class ChainPanel : public NodeComponent, private juce::Timer {
  public:
    ...
    void showChain(const aidaw::ChainNodePath& chainPath);

    // Legacy: show a chain by IDs (computes path internally)
    void showChain(aidaw::TrackId trackId, aidaw::RackId rackId, aidaw::ChainId chainId);

    // Get the current chain path (for nested components)
    const aidaw::ChainNodePath& getChainPath() const {
        return chainPath_;
    }
    ...
    aidaw::DeviceId getSelectedDeviceId() const {
  ...
  private:
    ...
    aidaw::ChainNodePath chainPath_;  // Full path to this chain
