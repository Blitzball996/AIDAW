...
ChainRowComponent::ChainRowComponent(RackComponent& owner, aidaw::TrackId trackId,
                                     aidaw::RackId rackId, const aidaw::ChainInfo& chain)
    : owner_(owner), trackId_(trackId), rackId_(rackId), chainId_(chain.id) {
    // Set up node path for centralized selection
    nodePath_ = aidaw::ChainNodePath::chain(trackId, rackId, chain.id);
    ...
    gainLabel_.setFormat(aidaw::DraggableValueLabel::Format::Decibels);
    ...
    gainLabel_.onValueChange = [this]() {
        aidaw::TrackManager::getInstance().setChainVolume(
            trackId_, rackId_, chainId_, static_cast<float>(gainLabel_.getValue()));
    };
    addAndMakeVisible(gainLabel_);

    // Pan label (L/C/R format, draggable)
    panLabel_.setFormat(aidaw::DraggableValueLabel::Format::Pan);
    ...
    panLabel_.onValueChange = [this]() {
        aidaw::TrackManager::getInstance().setChainPan(trackId_, rackId_, chainId_,
...
void ChainRowComponent::mouseUp(const juce::MouseEvent& event) {
    ...
    aidaw::SelectionManager::getInstance().selectChainNode(nodePath_);
}
...
void ChainRowComponent::selectionTypeChanged(aidaw::SelectionType /*newType*/) {
    // Selection type changed - chainNodeSelectionChanged will handle visual update
}

void ChainRowComponent::chainNodeSelectionChanged(const aidaw::ChainNodePath& path) {
...
void ChainRowComponent::setNodePath(const aidaw::ChainNodePath& path) {
    nodePath_ = path;
    ...
    const auto& currentSelection = aidaw::SelectionManager::getInstance().getSelectedChainNode();
...
void ChainRowComponent::updateFromChain(const aidaw::ChainInfo& chain) {
    nameLabel_.setText(chain.name, juce::dontSendNotification);
    muteButton_.setToggleState(chain.muted, juce::dontSendNotification);
    soloButton_.setToggleState(chain.solo, juce::dontSendNotification);
    gainLabel_.setValue(chain.volume, juce::dontSendNotification);
...
void ChainRowComponent::onMuteClicked() {
    aidaw::TrackManager::getInstance().setChainMuted(trackId_, rackId_, chainId_,
                                                     muteButton_.getToggleState());
}

void ChainRowComponent::onSoloClicked() {
    aidaw::TrackManager::getInstance().setChainSolo(trackId_, rackId_, chainId_,
                                                    soloButton_.getToggleState());
}

void ChainRowComponent::onBypassClicked() {
    bool active = onButton_->getToggleState();
    aidaw::TrackManager::getInstance().setChainBypassed(trackId_, rackId_, chainId_, !active);
}

void ChainRowComponent::onDeleteClicked() {
    // Use path-based removal to support nested chains
    if (nodePath_.isValid()) {
        aidaw::TrackManager::getInstance().removeChainByPath(nodePath_);
    } else {
        // Fallback to flat ID removal for top-level chains
        aidaw::TrackManager::getInstance().removeChainFromRack(trackId_, rackId_, chainId_);
    }
}
