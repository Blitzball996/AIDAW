...
class RackComponent : public NodeComponent, public juce::Timer {
  public:
    ...
    RackComponent(aidaw::TrackId trackId, const aidaw::RackInfo& rack);

    // Constructor for nested rack (in chain) - with full path context
    RackComponent(const aidaw::ChainNodePath& rackPath, const aidaw::RackInfo& rack);
    ...
    aidaw::RackId getRackId() const {
        return rackId_;
    }

    void updateFromRack(const aidaw::RackInfo& rack);
  ...
  protected:
    ...
    const aidaw::ChainNodePath& getRackPath() const {
        return rackPath_;
    }
  ...
  private:
    void initializeCommon(const aidaw::RackInfo& rack);
    void onAddChainClicked();
    void openMacroPanelForSelectionIfNeeded();

    aidaw::ChainNodePath rackPath_;  // Full path to this rack
    aidaw::TrackId trackId_;
    aidaw::RackId rackId_;

    // Header extra controls
    std::unique_ptr<aidaw::SvgButton> modButton_;     // Modulators toggle
    std::unique_ptr<aidaw::SvgButton> macroButton_;   // Macros toggle
    std::unique_ptr<aidaw::SvgButton> presetButton_;  // MAGDA rack presets menu
    ...
    aidaw::ChainId selectedChainId_ = aidaw::INVALID_CHAIN_ID;
