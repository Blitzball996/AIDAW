...
class DeviceSlotComponent : public NodeComponent,
                            ...
                            public aidaw::AutomationManagerListener {
  public:
    ...
    DeviceSlotComponent(const aidaw::DeviceInfo& device);
    ~DeviceSlotComponent() override;

    aidaw::DeviceId getDeviceId() const {
        return device_.id;
    }
    int getPreferredWidth() const override;

    // Override to update param slots when path is set
    void setNodePath(const aidaw::ChainNodePath& path) override;
  ...
  protected:
    ...
    const aidaw::ModArray* getModsData() const override;
    const aidaw::MacroArray* getMacrosData() const override;
    std::vector<std::pair<aidaw::DeviceId, juce::String>> getAvailableDevices() const override;
    std::map<aidaw::DeviceId, std::vector<juce::String>> getDeviceParamNames() const override;

    // Mod/macro callbacks
    void onModTargetChangedInternal(int modIndex, aidaw::ControlTarget target) override;
    void onModNameChangedInternal(int modIndex, const juce::String& name) override;
    void onModTypeChangedInternal(int modIndex, aidaw::ModType type) override;
    void onModWaveformChangedInternal(int modIndex, aidaw::LFOWaveform waveform) override;
    ...
    void onModSyncDivisionChangedInternal(int modIndex, aidaw::SyncDivision division) override;
    void onModTriggerModeChangedInternal(int modIndex, aidaw::LFOTriggerMode mode) override;
    ...
    void onMacroTargetChangedInternal(int macroIndex, aidaw::ControlTarget target) override;
    ...
    void onMacroLinkAmountChangedInternal(int macroIndex, aidaw::ControlTarget target,
                                          float amount) override;
    void onMacroNewLinkCreatedInternal(int macroIndex, aidaw::ControlTarget target,
                                       float amount) override;
    void onMacroLinkRemovedInternal(int macroIndex, aidaw::ControlTarget target) override;
    void onMacroLinkBipolarChangedInternal(int macroIndex, aidaw::ControlTarget target,
                                           bool bipolar) override;
    void onModClickedInternal(int modIndex) override;
    void onMacroClickedInternal(int macroIndex) override;
    void onAddModRequestedInternal(int slotIndex, aidaw::ModType type,
                                   aidaw::LFOWaveform waveform) override;
    ...
    void onModLinkAmountChangedInternal(int modIndex, aidaw::ControlTarget target,
                                        float amount) override;
    void onModNewLinkCreatedInternal(int modIndex, aidaw::ControlTarget target,
                                     float amount) override;
    void onModLinkRemovedInternal(int modIndex, aidaw::ControlTarget target) override;
    ...
    void modSelectionChanged(const aidaw::ModSelection& selection) override;
    void macroSelectionChanged(const aidaw::MacroSelection& selection) override;
    void paramSelectionChanged(const aidaw::ParamSelection& selection) override;
    ...
    void automationValueChanged(aidaw::AutomationLaneId laneId, double normalizedValue) override;

  private:
    aidaw::DeviceInfo device_;
    DeviceSlotTraits traits_;
    std::unique_ptr<juce::Drawable> tracktionLogo_;

    // Header controls
    std::unique_ptr<aidaw::SvgButton> modButton_;
    std::unique_ptr<aidaw::SvgButton> macroButton_;
    std::unique_ptr<aidaw::SvgButton> aiButton_;
    aidaw::DraggableValueLabel gainLabel_{aidaw::DraggableValueLabel::Format::Decibels};
    std::unique_ptr<juce::TextButton> scButton_;        // Sidechain source selector
    std::unique_ptr<aidaw::SvgButton> multiOutButton_;  // Multi-output routing
    std::unique_ptr<aidaw::SvgButton> uiButton_;
    std::unique_ptr<aidaw::SvgButton> learnButton_;
    std::unique_ptr<aidaw::SvgButton> onButton_;
    std::unique_ptr<aidaw::SvgButton> exportClipButton_;  // Export pattern/chords as MIDI clip
    ...
    aidaw::LevelMeter levelMeter_;
    aidaw::MidiNoteStrip midiNoteStrip_;

    // MAGDA preset menu button (lives in top header, replaces gainLabel_ slot)
    std::unique_ptr<aidaw::SvgButton> presetButton_;
    ...
    aidaw::DeviceInfo snapshotForPreset();
    ...
    bool isInternalDevice() const {
        return device_.format == aidaw::PluginFormat::Internal;
    }
