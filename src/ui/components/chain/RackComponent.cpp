...
RackComponent::RackComponent(aidaw::TrackId trackId, const aidaw::RackInfo& rack)
    : rackPath_(aidaw::ChainNodePath::rack(trackId, rack.id)), trackId_(trackId), rackId_(rack.id) {
    onDeleteClicked = [this]() {
        aidaw::TrackManager::getInstance().removeRackFromTrack(trackId_, rackId_);
    };
    initializeCommon(rack);
}

// Constructor for nested rack (in chain) - with full path context
RackComponent::RackComponent(const aidaw::ChainNodePath& rackPath, const aidaw::RackInfo& rack)
    : rackPath_(rackPath), trackId_(rackPath.trackId), rackId_(rack.id) {
    onDeleteClicked = [this]() {
        aidaw::TrackManager::getInstance().removeRackFromChainByPath(rackPath_);
    };
    initializeCommon(rack);
}

void RackComponent::initializeCommon(const aidaw::RackInfo& rack) {
    ...
    onCollapsedChanged = [this](bool collapsed) {
        if (auto* rackInfo = aidaw::TrackManager::getInstance().getRackByPath(rackPath_))
            rackInfo->expanded = !collapsed;
    };

    onBypassChanged = [this](bool bypassed) {
        aidaw::TrackManager::getInstance().setRackBypassed(trackId_, rackId_, bypassed);
    };
    ...
    gainSlider_->onValueChange = [this]() {
        aidaw::TrackManager::getInstance().setRackVolume(
    ...
    chainPanel_->onDeviceSelected = [this](aidaw::DeviceId deviceId) {
...
void RackComponent::updateFromRack(const aidaw::RackInfo& rack) {
    setNodeName(rack.name);
    setBypassed(rack.bypassed);
    if (gainSlider_)
        gainSlider_->setValue(rack.volume, juce::dontSendNotification);
    ...
    if (chainPanel_ && chainPanel_->isVisible() && selectedChainId_ != aidaw::INVALID_CHAIN_ID) {
    ...
    if (selectedChainId_ == aidaw::INVALID_CHAIN_ID && rack.chains.size() == 1) {
        if (!rack.chains[0].elements.empty()) {
            ...
            juce::MessageManager::callAsync([safeThis, chainId]() {
                if (safeThis != nullptr && safeThis->selectedChainId_ == aidaw::INVALID_CHAIN_ID)
...
void RackComponent::rebuildChainRows() {
    ...
    const auto* rack = aidaw::TrackManager::getInstance().getRackByPath(rackPath_);
    if (!rack) {
    ...
    for (const auto& chain : rack->chains) {
        ...
        if (existingRow) {
            ...
            row->onDoubleClick = [this](aidaw::ChainId chainId) {
...
void RackComponent::chainNodeSelectionChanged(const aidaw::ChainNodePath& path) {
    ...
    if (path.steps.back().type != aidaw::ChainStepType::Chain) {
        return;  // Not a chain
    }

    aidaw::ChainId chainId = path.steps.back().id;
...
void RackComponent::onAddChainClicked() {
    auto newChainId = aidaw::TrackManager::getInstance().addChainToRack(rackPath_);

    // Auto-select the newly created chain
    if (newChainId != aidaw::INVALID_CHAIN_ID) {
        auto newChainPath = rackPath_.withChain(newChainId);
        aidaw::SelectionManager::getInstance().selectChainNode(newChainPath);
    }
}

void RackComponent::showChainPanel(aidaw::ChainId chainId) {
...
void RackComponent::hideChainPanel() {
    selectedChainId_ = aidaw::INVALID_CHAIN_ID;
...
const aidaw::ModArray* RackComponent::getModsData() const {
    const auto* rack = aidaw::TrackManager::getInstance().getRackByPath(rackPath_);
    return rack ? &rack->mods : nullptr;
}

const aidaw::MacroArray* RackComponent::getMacrosData() const {
    const auto* rack = aidaw::TrackManager::getInstance().getRackByPath(rackPath_);
    return rack ? &rack->macros : nullptr;
}

std::vector<std::pair<aidaw::DeviceId, juce::String>> RackComponent::getAvailableDevices() const {
    ...
    std::vector<std::pair<aidaw::DeviceId, juce::String>> availableDevices;
    const auto* rack = aidaw::TrackManager::getInstance().getRackByPath(rackPath_);
    if (rack) {
        for (const auto& chain : rack->chains) {
            availableDevices.emplace_back(aidaw::INVALID_DEVICE_ID, chain.name);
...
std::map<aidaw::DeviceId, std::vector<juce::String>> RackComponent::getDeviceParamNames() const {
    ...
    std::map<aidaw::DeviceId, std::vector<juce::String>> result;
    const auto* rack = aidaw::TrackManager::getInstance().getRackByPath(rackPath_);
    if (rack == nullptr)
        return result;
    for (const auto& chain : rack->chains) {
...
void RackComponent::onModTargetChangedInternal(int modIndex, aidaw::ControlTarget target) {
    aidaw::TrackManager::getInstance().setModTarget(rackPath_, modIndex, target);
}

void RackComponent::onModNameChangedInternal(int modIndex, const juce::String& name) {
    aidaw::TrackManager::getInstance().setModName(rackPath_, modIndex, name);
}

void RackComponent::onModTypeChangedInternal(int modIndex, aidaw::ModType type) {
    aidaw::TrackManager::getInstance().setModType(rackPath_, modIndex, type);
}

void RackComponent::onModWaveformChangedInternal(int modIndex, aidaw::LFOWaveform waveform) {
    aidaw::TrackManager::getInstance().setModWaveform(rackPath_, modIndex, waveform);
}

void RackComponent::onModRateChangedInternal(int modIndex, float rate) {
    aidaw::TrackManager::getInstance().setModRate(rackPath_, modIndex, rate);
}

void RackComponent::onModPhaseOffsetChangedInternal(int modIndex, float phaseOffset) {
    aidaw::TrackManager::getInstance().setModPhaseOffset(rackPath_, modIndex, phaseOffset);
}

void RackComponent::onModTempoSyncChangedInternal(int modIndex, bool tempoSync) {
    aidaw::TrackManager::getInstance().setModTempoSync(rackPath_, modIndex, tempoSync);
}

void RackComponent::onModSyncDivisionChangedInternal(int modIndex, aidaw::SyncDivision division) {
    aidaw::TrackManager::getInstance().setModSyncDivision(rackPath_, modIndex, division);
}

void RackComponent::onModTriggerModeChangedInternal(int modIndex, aidaw::LFOTriggerMode mode) {
    aidaw::TrackManager::getInstance().setModTriggerMode(rackPath_, modIndex, mode);
}

void RackComponent::onModAudioAttackChangedInternal(int modIndex, float ms) {
    aidaw::TrackManager::getInstance().setModAudioAttack(rackPath_, modIndex, ms);
}

void RackComponent::onModAudioReleaseChangedInternal(int modIndex, float ms) {
    aidaw::TrackManager::getInstance().setModAudioRelease(rackPath_, modIndex, ms);
}
...
void RackComponent::onModCurveChangedInternal(int /*modIndex*/) {
    ...
    aidaw::TrackManager::getInstance().notifyModCurveChanged(rackPath_);
}

void RackComponent::onMacroValueChangedInternal(int macroIndex, float value) {
    aidaw::TrackManager::getInstance().setMacroValue(rackPath_, macroIndex, value);
...
void RackComponent::onMacroTargetChangedInternal(int macroIndex, aidaw::ControlTarget target) {
    aidaw::TrackManager::getInstance().setMacroTarget(rackPath_, macroIndex, target);
}

void RackComponent::onMacroNameChangedInternal(int macroIndex, const juce::String& name) {
    aidaw::TrackManager::getInstance().setMacroName(rackPath_, macroIndex, name);
}

void RackComponent::onModClickedInternal(int modIndex) {
    // Select this mod in the SelectionManager for inspector display
    aidaw::SelectionManager::getInstance().selectMod(rackPath_, modIndex);
}

void RackComponent::onMacroClickedInternal(int macroIndex) {
    // Select this macro in the SelectionManager for inspector display
    aidaw::SelectionManager::getInstance().selectMacro(rackPath_, macroIndex);
    DBG("Macro clicked: " << macroIndex << " on path: " << rackPath_.toString());
}

// === Virtual callbacks for page management ===

void RackComponent::onAddModRequestedInternal(int slotIndex, aidaw::ModType type,
                                              aidaw::LFOWaveform waveform) {
    aidaw::TrackManager::getInstance().addMod(rackPath_, slotIndex, type, waveform);
...
void RackComponent::onModRemoveRequestedInternal(int modIndex) {
    aidaw::TrackManager::getInstance().removeMod(rackPath_, modIndex);
    refreshPanels();
}

void RackComponent::onModEnableToggledInternal(int modIndex, bool enabled) {
    aidaw::TrackManager::getInstance().setModEnabled(rackPath_, modIndex, enabled);
}
...
void RackComponent::onMacroPageAddRequested(int /*itemsToAdd*/) {
    aidaw::TrackManager::getInstance().addMacroPage(rackPath_);
}

void RackComponent::onMacroPageRemoveRequested(int /*itemsToRemove*/) {
    aidaw::TrackManager::getInstance().removeMacroPage(rackPath_);
}
    ...
    menu.showMenuAsync(
        ...
        [this, indexCopy](int chosen) {
            ...
            if (chosen == kSaveAs) {
                ...
                aidaw::PresetManager::getInstance().getRacksDirectory().revealToUser();
...
void RackComponent::showSaveRackPresetDialog() {
    const auto* live = aidaw::TrackManager::getInstance().getRackByPath(rackPath_);
        ...
        true, juce::ModalCallbackFunction::create([aw, self](int result) {
            ...
            auto doSave = [name, self]() {
                if (self == nullptr)
                    return;
                const auto* fresh =
                    aidaw::TrackManager::getInstance().getRackByPath(self->rackPath_);
                if (fresh == nullptr) {
            ...
            if (aidaw::PresetManager::getInstance().getRackPresets().contains(name)) {
