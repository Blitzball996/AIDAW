...
void showParamLinkMenu(juce::Component* anchor, const ParamLinkContext& ctx,
    ...
    aidaw::ControlTarget thisTarget =
        aidaw::ControlTarget::pluginParam(ctx.devicePath, ctx.paramIndex);
    ...
    menu.addSeparator();
    {
        bool isLearning = aidaw::MidiLearnCoordinator::getInstance().isLearning(
            aidaw::ControlTarget::pluginParam(ctx.devicePath, ctx.paramIndex));
        int mappingCount = static_cast<int>(
            aidaw::BindingRegistry::getInstance()
                .findFor(aidaw::ControlTarget::pluginParam(ctx.devicePath, ctx.paramIndex))
                .size());
    ...
    menu.showMenuAsync(
        juce::PopupMenu::Options(), [safeAnchor, paramIdx, devicePath, cbs](int result) {
            ...
            aidaw::ControlTarget target = aidaw::ControlTarget::pluginParam(devicePath, paramIdx);
            ...
            if (result >= 1700 && result < 1800) {
                ...
                aidaw::ControlTarget macroTarget =
                    aidaw::ControlTarget::pluginParam(devicePath, paramIdx);
                ...
                bool isCurrentlyLearning = aidaw::MidiLearnCoordinator::getInstance().isLearning(
                    aidaw::ControlTarget::pluginParam(devicePath, paramIdx));
