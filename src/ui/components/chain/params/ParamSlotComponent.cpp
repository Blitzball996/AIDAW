...
ParamSlotComponent::ParamSlotComponent(int paramIndex) : paramIndex_(paramIndex) {
    ...
    valueSlider_.onClicked = [this]() {
        if (devicePath_.isValid()) {
            aidaw::SelectionManager::getInstance().selectParam(devicePath_, paramIndex_);
        }
    };
    ...
    onMidiLearn = [this](aidaw::ChainNodePath path, int paramIdx, juce::String paramName) {
        ...
        aidaw::MidiLearnCoordinator::getInstance().beginLearn(
            aidaw::ControlTarget::pluginParam(path, paramIdx), displayName);
    };
...
void ParamSlotComponent::modLinkModeChanged(bool active, const aidaw::ModSelection& selection) {
...
void ParamSlotComponent::macroLinkModeChanged(bool active, const aidaw::MacroSelection& selection) {
    bool isInScope = isInScopeOf(devicePath_, selection.parentPath);
    isInLinkMode_ = active && isInScope;
...
void ParamSlotComponent::midiLearnStateChanged(const aidaw::ChainNodePath& path, int paramIndex,
                                               aidaw::ControlTarget::Kind owner, bool learning) {
    if (owner != aidaw::ControlTarget::Kind::PluginParam)
...
void ParamSlotComponent::midiLearnCompleted(const aidaw::ChainNodePath& path, int paramIndex,
...
void ParamSlotComponent::midiLearnCleared(const aidaw::ChainNodePath& path, int paramIndex,
...
void ParamSlotComponent::bindingRegistryChanged(aidaw::BindingScope) {
    // Any change to any scope could add/remove a binding for this param.
    refreshMidiBindingState();
}

void ParamSlotComponent::refreshMidiBindingState() {
    bool newState = aidaw::BindingRegistry::getInstance().hasActiveBindingFor(
        aidaw::ControlTarget::pluginParam(devicePath_, paramIndex_));
    ...
    if (!linkModeSlider_) {
        ...
        linkModeSlider_->onValueChange = [safeThis]() {
            ...
            if (safeThis->activeMod_.isValid() && safeThis->onModAmountChanged) {
                aidaw::ControlTarget thisTarget =
                    aidaw::ControlTarget::pluginParam(safeThis->devicePath_, safeThis->paramIndex_);
...
void ParamSlotComponent::refreshLinkModeState() {
    auto& manager = aidaw::LinkModeManager::getInstance();
    if (manager.getLinkModeType() == aidaw::LinkModeType::Mod) {
        modLinkModeChanged(true, manager.getModInLinkMode());
    } else if (manager.getLinkModeType() == aidaw::LinkModeType::Macro) {
...
void ParamSlotComponent::setParameterInfo(const aidaw::ParameterInfo& info) {
    cancelGesture();
    paramInfo_ = info;
    ...
    if (info.scale == aidaw::ParameterScale::Boolean) {
        if (!boolToggle_) {
    ...
    } else if (info.scale == aidaw::ParameterScale::Discrete && !info.choices.empty()) {
        if (!discreteCombo_) {
...
void ParamSlotComponent::paintOverChildren(juce::Graphics& g) {
    ...
    const float normalizedParamValue =
        juce::jlimit(0.0f, 1.0f,
                     aidaw::ParameterUtils::realToNormalized(
...
void ParamSlotComponent::mouseUp(const juce::MouseEvent& /*e*/) {
    if (isLinkModeDrag_) {
        ...
        if (noDragHappened) {
            aidaw::ControlTarget modTarget =
                aidaw::ControlTarget::pluginParam(devicePath_, paramIndex_);
            aidaw::ControlTarget macroTarget =
                aidaw::ControlTarget::pluginParam(devicePath_, paramIndex_);
            if (activeMod_.isValid()) {
                const auto* modPtr = resolveModPtr(activeMod_, devicePath_, availableMods_,
                                                   availableRackMods_, availableTrackMods_);
...
void ParamSlotComponent::itemDropped(const SourceDetails& details) {
    ...
    if (desc.startsWith("mod_drag:")) {
        ...
        aidaw::ControlTarget target = aidaw::ControlTarget::pluginParam(devicePath_, paramIndex_);
        if (onModLinkedWithAmount) {
