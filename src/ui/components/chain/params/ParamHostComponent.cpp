...
void applyFilled(ParamSlotComponent& slot, const aidaw::ParameterInfo& param, const ParamCell& cell,
...
void ParamHostComponent::updateParameterSlots(
    const aidaw::DeviceInfo& device, int currentPage,
...
void ParamHostComponent::updateParameterValues(const aidaw::DeviceInfo& device, int currentPage) {
    for (int i = 0; i < cellCount_; ++i) {
        const auto cell = layout_->cellFor(device, i, currentPage);
...
void ParamHostComponent::refreshEnabledStates(const aidaw::DeviceInfo& device, int currentPage) {
    for (int i = 0; i < cellCount_; ++i) {
        const auto cell = layout_->cellFor(device, i, currentPage);
        if (cell.mode != ParamCell::Mode::Filled)
            continue;
...
void ParamHostComponent::updateParamModulation(
    const aidaw::ModArray* mods, const aidaw::MacroArray* macros, const aidaw::ModArray* rackMods,
    const aidaw::MacroArray* rackMacros, const aidaw::ModArray* trackMods,
    const aidaw::MacroArray* trackMacros, aidaw::DeviceId deviceId,
    const aidaw::ChainNodePath& devicePath, int selectedModIndex, int selectedMacroIndex) {
    for (int i = 0; i < cellCount_; ++i) {
