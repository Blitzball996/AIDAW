...
namespace aidaw::daw::ui::drum_grid_slot {
...
void wireLinkableControl(Control& control, PadChainLinkCallbacks callbacks) {
    control.onModLinkedWithAmount = [callbacks](int modIndex, aidaw::ControlTarget target,
                                                float amount) {
        const auto nodePath =
            callbacks.getNodePath ? callbacks.getNodePath() : aidaw::ChainNodePath{};
        const auto activeModSelection = aidaw::LinkModeManager::getInstance().getModInLinkMode();
        if (activeModSelection.isValid() && activeModSelection.parentPath == nodePath) {
            ...
            aidaw::TrackManager::getInstance().setModTarget(
                aidaw::ChainNodePath::trackLevel(trackId), modIndex, target);
            aidaw::TrackManager::getInstance().setModLinkAmount(
                aidaw::ChainNodePath::trackLevel(trackId), modIndex, target, amount);
        } else if (activeModSelection.isValid()) {
    ...
    control.onModUnlinked = [callbacks](int modIndex, aidaw::ControlTarget target) {
        const auto nodePath =
            callbacks.getNodePath ? callbacks.getNodePath() : aidaw::ChainNodePath{};
    ...
    control.onRackModUnlinked = [callbacks](int modIndex, aidaw::ControlTarget target) {
        const auto nodePath =
            callbacks.getNodePath ? callbacks.getNodePath() : aidaw::ChainNodePath{};
    ...
    control.onTrackModUnlinked = [callbacks](int modIndex, aidaw::ControlTarget target) {
        const auto nodePath =
            callbacks.getNodePath ? callbacks.getNodePath() : aidaw::ChainNodePath{};
    ...
    control.onModAmountChanged = [callbacks](int modIndex, aidaw::ControlTarget target,
                                             float amount) {
        const auto nodePath =
            callbacks.getNodePath ? callbacks.getNodePath() : aidaw::ChainNodePath{};
        const auto activeModSelection = aidaw::LinkModeManager::getInstance().getModInLinkMode();
        if (activeModSelection.isValid() && activeModSelection.parentPath == nodePath) {
            ...
            if (callbacks.updateModsPanel)
                callbacks.updateModsPanel();
                   ...
                   activeModSelection.parentPath.getType() == aidaw::ChainNodeType::Track) {
            aidaw::TrackManager::getInstance().setModLinkAmount(
                aidaw::ChainNodePath::trackLevel(activeModSelection.parentPath.trackId), modIndex,
                target, amount);
        } else if (activeModSelection.isValid()) {
            aidaw::TrackManager::getInstance().setModLinkAmount(activeModSelection.parentPath,
                                                                modIndex, target, amount);
    ...
    control.onMacroLinkedWithAmount = [callbacks](int macroIndex, aidaw::ControlTarget target,
                                                  float amount) {
        const auto nodePath =
            callbacks.getNodePath ? callbacks.getNodePath() : aidaw::ChainNodePath{};
        ...
        if (activeMacroSelection.isValid() && activeMacroSelection.parentPath == nodePath) {
            ...
            aidaw::TrackManager::getInstance().setMacroTarget(
                aidaw::ChainNodePath::trackLevel(trackId), macroIndex, target);
            aidaw::TrackManager::getInstance().setMacroLinkAmount(
                aidaw::ChainNodePath::trackLevel(trackId), macroIndex, target, amount);
        } else if (activeMacroSelection.isValid()) {
    ...
    control.onMacroLinked = [callbacks](int macroIndex, aidaw::ControlTarget target) {
    ...
    control.onMacroUnlinked = [callbacks](int macroIndex, aidaw::ControlTarget target) {
        const auto nodePath =
            callbacks.getNodePath ? callbacks.getNodePath() : aidaw::ChainNodePath{};
        aidaw::TrackManager::getInstance().removeMacroLink(nodePath, macroIndex, target);
        refreshMacroUi(callbacks);
    };

    control.onTrackMacroUnlinked = [callbacks](int macroIndex, aidaw::ControlTarget target) {
        const auto nodePath =
            callbacks.getNodePath ? callbacks.getNodePath() : aidaw::ChainNodePath{};
        auto trackId = nodePath.trackId;
        if (trackId != aidaw::INVALID_TRACK_ID)
    ...
    control.onMacroAmountChanged = [callbacks](int macroIndex, aidaw::ControlTarget target,
                                               float amount) {
        const auto nodePath =
            callbacks.getNodePath ? callbacks.getNodePath() : aidaw::ChainNodePath{};
        const auto activeMacroSelection =
        ...
        if (activeMacroSelection.isValid() && activeMacroSelection.parentPath == nodePath) {
            ...
            if (callbacks.updateMacroPanel)
                callbacks.updateMacroPanel();
                   ...
                   activeMacroSelection.parentPath.getType() == aidaw::ChainNodeType::Track) {
            aidaw::TrackManager::getInstance().setMacroLinkAmount(
                aidaw::ChainNodePath::trackLevel(activeMacroSelection.parentPath.trackId),
                macroIndex, target, amount);
        } else if (activeMacroSelection.isValid()) {
            aidaw::TrackManager::getInstance().setMacroLinkAmount(activeMacroSelection.parentPath,
                                                                  macroIndex, target, amount);
...
bool shouldShowModButton(bool isDrumGrid, aidaw::DeviceType deviceType) {
    return deviceType != aidaw::DeviceType::MIDI || isDrumGrid;
}

bool shouldShowMacroButton(bool isDrumGrid, aidaw::DeviceType deviceType, bool isArpeggiator,
                           bool isStepSequencer) {
    return deviceType != aidaw::DeviceType::MIDI || isArpeggiator || isStepSequencer || isDrumGrid;
}
...
void setPadChainLinkContext(DrumGridUI* drumGridUI, const aidaw::ChainNodePath& nodePath,
                            ...
                            const aidaw::MacroArray* trackMacros,
                            const aidaw::ModArray* trackMods) {
    if (drumGridUI != nullptr)
