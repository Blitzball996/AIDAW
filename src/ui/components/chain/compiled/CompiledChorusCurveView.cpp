...
void CompiledChorusCurveView::setCompiledPlugin(
    aidaw::daw::audio::compiled::MagdaChorusCompiledPlugin* plugin) {
    compiledPlugin_ = plugin;
}

void CompiledChorusCurveView::bindPlugin(te::Plugin* plugin) {
    setCompiledPlugin(
        dynamic_cast<aidaw::daw::audio::compiled::MagdaChorusCompiledPlugin*>(plugin));
}

void CompiledChorusCurveView::updateFromDevice(const aidaw::DeviceInfo& device) {
...
void CompiledChorusCurveView::timerCallback() {
    using Chorus = aidaw::daw::audio::compiled::MagdaChorusCompiledPlugin;
    ...
    if (sync_) {
        if (compiledPlugin_ != nullptr) {
            const auto& info = compiledPlugin_->getSlotInfo(
                aidaw::daw::audio::compiled::MagdaChorusCompiledPlugin::kDivisionSlot);
...
const CompiledPresentationSpec& getMagdaChorusPresentation() {
    ...
    static const CompiledPresentationSpec kSpec{
        .pluginId = aidaw::daw::audio::compiled::MagdaChorusCompiledPlugin::xmlTypeName,
        .layoutCellCount = 8,
        .layoutCellsPerRow = 8,
