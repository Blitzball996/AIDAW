...
void CompiledFlangerCurveView::setCompiledPlugin(
    aidaw::daw::audio::compiled::MagdaFlangerCompiledPlugin* plugin) {
    compiledPlugin_ = plugin;
}

void CompiledFlangerCurveView::bindPlugin(te::Plugin* plugin) {
    setCompiledPlugin(
        dynamic_cast<aidaw::daw::audio::compiled::MagdaFlangerCompiledPlugin*>(plugin));
}

void CompiledFlangerCurveView::updateFromDevice(const aidaw::DeviceInfo& device) {
...
void CompiledFlangerCurveView::timerCallback() {
    using Flanger = aidaw::daw::audio::compiled::MagdaFlangerCompiledPlugin;
    ...
    if (sync_) {
        if (compiledPlugin_ != nullptr) {
            const auto& info = compiledPlugin_->getSlotInfo(
                aidaw::daw::audio::compiled::MagdaFlangerCompiledPlugin::kDivisionSlot);
...
const CompiledPresentationSpec& getMagdaFlangerPresentation() {
    static const CompiledPresentationSpec kSpec{
        .pluginId = aidaw::daw::audio::compiled::MagdaFlangerCompiledPlugin::xmlTypeName,
        .layoutCellCount = 7,
        .layoutCellsPerRow = 7,
