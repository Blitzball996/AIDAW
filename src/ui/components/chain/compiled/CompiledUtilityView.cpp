...
namespace {
float valueForSlot(const aidaw::DeviceInfo& device, int slotIndex, float fallback) {
...
const aidaw::ParameterInfo* parameterForSlot(const aidaw::DeviceInfo& device, int slotIndex) {
    for (const auto& param : device.parameters)
        if (param.paramIndex == slotIndex)
            return &param;
    return nullptr;
...
CompiledUtilityView::CompiledUtilityView(juce::String /*pluginId*/) {
    using Util = aidaw::daw::audio::compiled::MagdaUtilityCompiledPlugin;
...
void CompiledUtilityView::updateFromDevice(const aidaw::DeviceInfo& device) {
...
void CompiledUtilityView::configureLinkSlots() {
    ...
    auto configure = [this](ParamSlotComponent& slot, int slotIndex, const char* name,
        ...
        slot.onModLinkedWithAmount = [this](int, aidaw::ControlTarget target, float amount) {
            if (onLinkRequested)
                onLinkRequested(target.paramIndex, amount);
        };
        slot.onModAmountChanged = [this](int, aidaw::ControlTarget target, float amount) {
            if (onLinkAmountChanged)
                onLinkAmountChanged(target.paramIndex, amount);
        };
        slot.onMacroLinked = [this](int, aidaw::ControlTarget target) {
            if (onLinkRequested)
                onLinkRequested(target.paramIndex, 0.3f);
        };
        slot.onMacroLinkedWithAmount = [this](int, aidaw::ControlTarget target, float amount) {
            if (onLinkRequested)
                onLinkRequested(target.paramIndex, amount);
        };
        slot.onMacroAmountChanged = [this](int, aidaw::ControlTarget target, float amount) {
            if (onLinkAmountChanged)
                onLinkAmountChanged(target.paramIndex, amount);
        };
    };
...
void CompiledUtilityView::refreshLinkSlotContext() {
    auto apply = [this](ParamSlotComponent& slot) {
        slot.setDeviceId(hasLinkContext_ ? linkContext_.deviceId : aidaw::INVALID_DEVICE_ID);
        slot.setDevicePath(hasLinkContext_ ? linkContext_.devicePath : aidaw::ChainNodePath{});
...
void CompiledUtilityView::bindPlugin(te::Plugin* plugin) {
    compiledPlugin_ =
        dynamic_cast<aidaw::daw::audio::compiled::MagdaUtilityCompiledPlugin*>(plugin);
}

const CompiledPresentationSpec& getMagdaUtilityPresentation() {
    static const CompiledPresentationSpec kSpec{
        .pluginId = aidaw::daw::audio::compiled::MagdaUtilityCompiledPlugin::xmlTypeName,
        .layoutCellCount = 0,
        .layoutCellsPerRow = 0,
