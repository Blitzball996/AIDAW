...
MagdaDriveCurveView::MagdaDriveCurveView(aidaw::daw::audio::FaustPlugin& plugin) : plugin_(plugin) {
...
const aidaw::daw::audio::FaustParamSlot* MagdaDriveCurveView::findSlot(
    const juce::String& label) const {
    const auto& pool = plugin_.getPool();
    for (int i = 0; i < aidaw::daw::audio::FaustParamPool::kSize; ++i) {
...
float MagdaDriveCurveView::readSlotValue(const aidaw::daw::audio::FaustParamSlot* slot) {
    if (slot == nullptr)
        return 0.0f;
...
void registerBuiltInFaustCustomViews() {
    ...
    registry.registerView(FaustCustomViewKind::MagdaDrive,
                          [](aidaw::daw::audio::FaustPlugin& plugin) {
