...
const aidaw::ParameterInfo* paramForSlot(const aidaw::DeviceInfo& device, int slotIndex) {
    for (const auto& param : device.parameters) {
        if (param.paramIndex == slotIndex)
            return &param;
    }
...
float modulatedValueForSlot(const aidaw::DeviceInfo& device, int slotIndex, float fallback,
                            const ParamLinkContext* linkContext,
                            aidaw::daw::audio::compiled::MagdaFilterCompiledPlugin* plugin) {
    if (plugin != nullptr) {
    ...
    const float baseNorm = aidaw::ParameterUtils::realToNormalized(param->currentValue, *param);
    const float modNorm =
    ...
    return aidaw::ParameterUtils::normalizedToReal(effectiveNorm, *param);
}
...
void CompiledFilterCurveView::updateFromDevice(const aidaw::DeviceInfo& device,
                                               const ParamLinkContext* linkContext) {
...
void CompiledFilterCurveView::updateTargetValues() {
    ...
    using Filter = aidaw::daw::audio::compiled::MagdaFilterCompiledPlugin;
...
const CompiledPresentationSpec& getMagdaFilterPresentation() {
    static const CompiledPresentationSpec kSpec{
        .pluginId = aidaw::daw::audio::compiled::MagdaFilterCompiledPlugin::xmlTypeName,
        .layoutCellCount = 6,
        .layoutCellsPerRow = 6,
...
void CompiledFilterCurveView::bindPlugin(te::Plugin* plugin) {
    setCompiledPlugin(
        dynamic_cast<aidaw::daw::audio::compiled::MagdaFilterCompiledPlugin*>(plugin));
}
