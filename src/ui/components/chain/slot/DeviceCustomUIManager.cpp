...
bool isLegacyTeCompressorPluginId(const juce::String& pluginId) {
    return aidaw::classifyInternalDevice(pluginId) == aidaw::InternalDeviceKind::TeCompressor;
}
...
void DeviceCustomUIManager::readAndPushModMatrix(aidaw::DeviceId deviceId) {
    if (!fourOscUI_)
        return;
...
void DeviceCustomUIManager::refreshParameterValues(const aidaw::DeviceInfo& device) {
...
void DeviceCustomUIManager::create(const aidaw::DeviceInfo& device, juce::Component* parent,
    ...
    if (device.pluginId.containsIgnoreCase("tone")) {
        ...
        drumGridUI_->onNotePreview = [cb = callbacks, getDrumGrid](int padIndex, bool isNoteOn) {
            ...
            aidaw::TrackManager::getInstance().previewNote(nodePath.trackId, noteNumber,
        ...
        padChain.onDeviceClicked = [cb = callbacks](const juce::String& pluginName,
            ...
            auto nodePath = cb.getNodePath();
            if (nodePath.isValid()) {
                aidaw::SelectionManager::getInstance().selectChainNode(nodePath, pluginName,
                                                                       pluginType);
            }
        };
        ...
        if (auto* audioEngine = aidaw::TrackManager::getInstance().getAudioEngine()) {
            if (auto* bridge = audioEngine->getAudioBridge()) {
                ...
                if (auto* cp = dynamic_cast<daw::audio::MidiChordEnginePlugin*>(plugin.get())) {
                    chordEngineUI_->setChordEngine(cp, aidaw::INVALID_TRACK_ID);
                    chordPlugin_ = cp;
                }
            }
        }
...
void DeviceCustomUIManager::update(const aidaw::DeviceInfo& device) {
