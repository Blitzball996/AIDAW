...
aidaw::AudioBridge* getAudioBridge() {
...
void buildPluginPresetSubmenu(juce::PopupMenu& menu,
                              const std::vector<aidaw::PluginPresetScanner::Entry>& entries,
...
void showMagdaPresetMenu(juce::Component* targetComponent, const juce::String& pluginFolder,
    ...
    auto& presetManager = aidaw::PresetManager::getInstance();
...
void showSaveMagdaPresetDialog(const aidaw::DeviceInfo& device,
    ...
    if (defaultPath.isEmpty())
        defaultPath = aidaw::PresetManager::getInstance().getSuggestedPresetName(device.id);
    if (defaultPath.isEmpty())
    ...
    alert->enterModalState(
        true, juce::ModalCallbackFunction::create([alert, pluginFolder = device.name,
            ...
            if (aidaw::PresetManager::getInstance()
...
bool hasPluginPresetsAvailable(const aidaw::DeviceInfo& device, bool isInternalDevice) {
    if (isInternalDevice || device.loadState != aidaw::DeviceLoadState::Loaded)
        return false;

    return !aidaw::PluginPresetScanner::getInstance().getPresets(device).empty();
}
...
void showPluginPresetMenu(juce::Component* targetComponent, const aidaw::DeviceInfo& device,
    ...
    auto& scanner = aidaw::PluginPresetScanner::getInstance();
    ...
    menu.showMenuAsync(
        ...
        [device, indexedPresetFiles, actions = std::move(actions)](int chosen) {
            ...
            if (chosen == kRevealPluginUserDir) {
                auto dir = aidaw::PluginPresetScanner::getInstance().getUserPresetDirectory(device);
                if (!dir.exists())
                    dir.createDirectory();
                if (dir.exists())
                    dir.revealToUser();
            ...
            if (chosen == kRescanPluginPresets) {
                aidaw::PluginPresetScanner::getInstance().rescan(device);
                return;
            }
