...
std::vector<aidaw::ParameterInfo>::iterator findParameterInfo(aidaw::DeviceInfo& device,
                                                              int paramIndex) {
    auto it = std::find_if(
        device.parameters.begin(), device.parameters.end(),
        [paramIndex](const aidaw::ParameterInfo& param) { return param.paramIndex == paramIndex; });
...
int visibleIndexForParameter(const aidaw::DeviceInfo& device, int paramIndex) {
...
void updateCurrentPageParameterSlotValue(const aidaw::DeviceInfo& device,
    ...
    const auto findIt = std::find_if(
        device.parameters.begin(), device.parameters.end(),
        [paramIndex](const aidaw::ParameterInfo& p) { return p.paramIndex == paramIndex; });
