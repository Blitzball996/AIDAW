...
class DeviceCustomUIManager {
  public:
     ...
     */
    void create(const aidaw::DeviceInfo& device, juce::Component* parent,
                const Callbacks& callbacks);

    /**
     * Update the custom UI to reflect new device state (parameter values, etc.).
     */
    void update(const aidaw::DeviceInfo& device);
     ...
     */
    void refreshParameterValues(const aidaw::DeviceInfo& device);

    /**
     * Read the FourOsc mod matrix from the plugin and push it to FourOscUI.
     */
    void readAndPushModMatrix(aidaw::DeviceId deviceId);
