...
aidaw::ChainNodePath nearestRackPathForDevicePath(const aidaw::ChainNodePath& devicePath) {
    aidaw::ChainNodePath rackPath;
    rackPath.trackId = devicePath.trackId;
    int rackStepIndex = -1;
    for (int i = 0; i < static_cast<int>(devicePath.steps.size()); ++i) {
        if (devicePath.steps[static_cast<size_t>(i)].type == aidaw::ChainStepType::Rack)
            rackStepIndex = i;
    }
    if (rackStepIndex >= 0) {
...
DeviceSlotComponent::DeviceSlotComponent(const aidaw::DeviceInfo& device) : device_(device) {
    // Register as TrackManager listener for parameter updates from plugin
    aidaw::TrackManager::getInstance().addListener(this);

    // Register for automation value updates so param slot knobs follow curve
    // edits and playback without polling.
    aidaw::AutomationManager::getInstance().addListener(this);
    ...
    onDeleteClicked = [this]() {
        ...
        juce::MessageManager::callAsync([pathToDelete, callback]() {
            ...
            if (pathToDelete.topLevelDeviceId != aidaw::INVALID_DEVICE_ID) {
                aidaw::UndoManager::getInstance().executeCommand(
                    std::make_unique<aidaw::RemoveDeviceFromTrackCommand>(
                        pathToDelete.trackId, pathToDelete.topLevelDeviceId));
            } else {
                aidaw::TrackManager::getInstance().removeDeviceFromChainByPath(pathToDelete);
            }
            if (callback) {
                callback();
            }
    ...
    onModPanelToggled = [this](bool visible) {
        if (auto* dev = aidaw::TrackManager::getInstance().getDeviceInChainByPath(nodePath_)) {
    ...
    modButton_ = std::make_unique<aidaw::SvgButton>("Mod", BinaryData::bare_sine_svg,
    ...
    macroButton_ =
        std::make_unique<aidaw::SvgButton>("Macro", BinaryData::knob_svg, BinaryData::knob_svgSize);
    ...
    aiButton_ =
        std::make_unique<aidaw::SvgButton>("AI", BinaryData::ai_svg, BinaryData::ai_svgSize);
    ...
    gainLabel_.onValueChange = [this]() {
        ...
        aidaw::TrackManager::getInstance().setDeviceGainDb(
            nodePath_, static_cast<float>(gainLabel_.getValue()));
    };
    addAndMakeVisible(gainLabel_);

    // ----- MOCK UI (no wiring): MAGDA preset menu button (top header) -----
    presetButton_ = std::make_unique<aidaw::SvgButton>("Presets", BinaryData::preset_svg,
    ...
    multiOutButton_ = std::make_unique<aidaw::SvgButton>("MultiOut", BinaryData::multiout_svg,
    ...
    uiButton_ = std::make_unique<aidaw::SvgButton>("UI", BinaryData::open_in_new_svg,
    ...
    uiButton_->onClick = [this]() {
        ...
        auto* audioEngine = aidaw::TrackManager::getInstance().getAudioEngine();
        if (audioEngine) {
    ...
    learnButton_ = std::make_unique<aidaw::SvgButton>("Learn", BinaryData::learn_svg,
    ...
    onButton_ = std::make_unique<aidaw::SvgButton>("Power", BinaryData::power_on_svg,
    ...
    onButton_->onClick = [this]() {
        ...
        setBypassed(!active);
        aidaw::TrackManager::getInstance().setDeviceInChainBypassedByPath(nodePath_, !active);
    ...
    if (traits_.isStepSequencer) {
        exportClipButton_ = std::make_unique<aidaw::SvgButton>("ExportClip", BinaryData::copy_svg,
    ...
    for (int i = 0; i < paramGrid_->getSlotCount(); ++i) {
        ...
        paramSlot->onModLinked = [safeThis = juce::Component::SafePointer(this)](
                                     int modIndex, aidaw::ControlTarget target) {
            auto self = safeThis;
            if (!self)
                return;
        ...
        paramSlot->onModLinkedWithAmount = [safeThis = juce::Component::SafePointer(this)](
            ...
            auto activeModSelection = aidaw::LinkModeManager::getInstance().getModInLinkMode();
            if (activeModSelection.isValid() && activeModSelection.parentPath == nodePath) {
                ...
                aidaw::TrackManager::getInstance().setModTarget(nodePath, modIndex, target);
                aidaw::TrackManager::getInstance().setModLinkAmount(nodePath, modIndex, target,
                                                                    amount);
                if (!self)
                    return;
                ...
                aidaw::SelectionManager::getInstance().selectMod(nodePath, modIndex);
            } else if (activeModSelection.isValid() &&
                       activeModSelection.parentPath.getType() == aidaw::ChainNodeType::Track) {
                // Track-level mod
                auto trackId = activeModSelection.parentPath.trackId;
                aidaw::TrackManager::getInstance().setModTarget(ChainNodePath::trackLevel(trackId),
                                                                modIndex, target);
                aidaw::TrackManager::getInstance().setModLinkAmount(
                    ChainNodePath::trackLevel(trackId), modIndex, target, amount);
            } else if (activeModSelection.isValid()) {
                // Rack-level mod (use the parent path from the active selection)
                aidaw::TrackManager::getInstance().setModTarget(activeModSelection.parentPath,
                                                                modIndex, target);
                aidaw::TrackManager::getInstance().setModLinkAmount(activeModSelection.parentPath,
        ...
        paramSlot->onModUnlinked = [safeThis = juce::Component::SafePointer(this)](
            ...
            if (!self)
                return;
            auto nodePath = self->nodePath_;
            aidaw::TrackManager::getInstance().removeModLink(nodePath, modIndex, target);
            if (!self)
                return;
            self->updateParamModulation();
            self->updateModsPanel();
        ...
        paramSlot->onRackModUnlinked = [safeThis = juce::Component::SafePointer(this)](
            ...
            if (rackPath.isValid())
                aidaw::TrackManager::getInstance().removeModLink(rackPath, modIndex, target);
            if (!self)
                return;
            self->updateParamModulation();
            self->updateModsPanel();
        ...
        paramSlot->onTrackModUnlinked = [safeThis = juce::Component::SafePointer(this)](
            ...
            if (trackId != aidaw::INVALID_TRACK_ID)
                aidaw::TrackManager::getInstance().removeModLink(ChainNodePath::trackLevel(trackId),
                                                                 modIndex, target);
            if (!self)
                return;
            self->updateParamModulation();
        ...
        paramSlot->onMacroLinked = [safeThis = juce::Component::SafePointer(this)](
                                       int macroIndex, aidaw::ControlTarget target) {
            auto self = safeThis;
            if (!self)
                return;
            ...
            if (target.isValid()) {
                auto activeMacroSelection =
                    aidaw::LinkModeManager::getInstance().getMacroInLinkMode();
        ...
        paramSlot->onMacroLinkedWithAmount = [safeThis = juce::Component::SafePointer(this)](
            ...
            if (!self)
                return;
            auto nodePath = self->nodePath_;
            auto activeMacroSelection = aidaw::LinkModeManager::getInstance().getMacroInLinkMode();
            if (activeMacroSelection.isValid() && activeMacroSelection.parentPath == nodePath) {
                aidaw::TrackManager::getInstance().setMacroTarget(nodePath, macroIndex, target);
                aidaw::TrackManager::getInstance().setMacroLinkAmount(nodePath, macroIndex, target,
                                                                      amount);
                if (!self)
                    return;
                ...
                // Keep device selection — don't switch to macro selection.
                       ...
                       activeMacroSelection.parentPath.getType() == aidaw::ChainNodeType::Track) {
                // Track-level macro
                auto trackId = activeMacroSelection.parentPath.trackId;
                aidaw::TrackManager::getInstance().setMacroTarget(
                    ChainNodePath::trackLevel(trackId), macroIndex, target);
                aidaw::TrackManager::getInstance().setMacroLinkAmount(
                    ChainNodePath::trackLevel(trackId), macroIndex, target, amount);
            } else if (activeMacroSelection.isValid()) {
                aidaw::TrackManager::getInstance().setMacroTarget(activeMacroSelection.parentPath,
                                                                  macroIndex, target);
                aidaw::TrackManager::getInstance().setMacroLinkAmount(
        ...
        paramSlot->onMacroUnlinked = [safeThis = juce::Component::SafePointer(this)](
            ...
            if (!self)
                return;
            aidaw::TrackManager::getInstance().removeMacroLink(self->nodePath_, macroIndex, target);
            if (self) {
                self->updateParamModulation();
                self->updateMacroPanel();
            }
        ...
        paramSlot->onTrackMacroUnlinked = [safeThis = juce::Component::SafePointer(this)](
            ...
            auto trackId = self->nodePath_.trackId;
            if (trackId != aidaw::INVALID_TRACK_ID)
                aidaw::TrackManager::getInstance().removeMacroLink(
                    ChainNodePath::trackLevel(trackId), macroIndex, target);
            if (self) {
                self->updateParamModulation();
                self->updateMacroPanel();
        ...
        paramSlot->onRackMacroLinked = [safeThis = juce::Component::SafePointer(this)](
            ...
            auto rackPath = nearestRackPathForDevicePath(self->nodePath_);
            if (rackPath.isValid())
                aidaw::TrackManager::getInstance().setMacroTarget(rackPath, macroIndex, target);
            if (self)
                self->updateParamModulation();
        };
        ...
        paramSlot->onRackMacroUnlinked = [safeThis = juce::Component::SafePointer(this)](
            ...
            auto rackPath = nearestRackPathForDevicePath(self->nodePath_);
            if (rackPath.isValid())
                aidaw::TrackManager::getInstance().removeMacroLink(rackPath, macroIndex, target);
            if (self) {
                self->updateParamModulation();
                self->updateMacroPanel();
            }
        ...
        paramSlot->onMacroValueChanged = [safeThis = juce::Component::SafePointer(this)](
            ...
            if (!self)
                return;
            aidaw::TrackManager::getInstance().setMacroValue(self->nodePath_, macroIndex, value);
            if (self)
                self->updateParamModulation();
        };
    ...
    if (!device_.uniqueId.isEmpty() && !device_.parameters.empty()) {
        aidaw::DeviceInfo tempDevice = device_;
        if (ParameterConfigDialog::applyConfigToDevice(tempDevice.uniqueId, tempDevice)) {
            ...
            if (!tempDevice.visibleParameters.empty()) {
                aidaw::TrackManager::getInstance().setDeviceVisibleParameters(
...
DeviceSlotComponent::~DeviceSlotComponent() {
    aidaw::TrackManager::getInstance().removeListener(this);
    aidaw::AutomationManager::getInstance().removeListener(this);
    stopTimer();
}
...
void DeviceSlotComponent::timerCallback() {
    ...
    if (traits_.isArpeggiator || traits_.isStepSequencer || traits_.isChordEngine) {
        ...
        aidaw::DeviceMeteringManager::DeviceMeterData data;
...
void DeviceSlotComponent::deviceParameterChanged(aidaw::DeviceId deviceId, int paramIndex,
                                                 float newValue) {
...
void DeviceSlotComponent::showAutomationLaneForParam(int paramIndex) {
    ...
    if (trackId == aidaw::INVALID_TRACK_ID)
        return;
    aidaw::AutomationTarget target;
    target.kind = aidaw::ControlTarget::Kind::PluginParam;
    ...
    auto& automationMgr = aidaw::AutomationManager::getInstance();
    auto laneId = automationMgr.getOrCreateLane(target, aidaw::AutomationLaneType::Absolute);
    automationMgr.setLaneVisible(laneId, true);
}

void DeviceSlotComponent::automationValueChanged(aidaw::AutomationLaneId laneId,
    ...
    const auto* lane = aidaw::AutomationManager::getInstance().getLane(laneId);
    if (!lane)
        return;

    if (lane->target.kind != aidaw::ControlTarget::Kind::PluginParam)
        return;
    ...
    if (aidaw::AutomationManager::getInstance().getVisualState(lane->target) ==
        aidaw::AutomationVisualState::Overridden)
        return;
    ...
    const float modelValue =
        aidaw::ParameterUtils::normalizedToModelValue(
            aidaw::ParameterNormalizedValue::clamped(static_cast<float>(normalizedValue)), info)
            .value;
...
void DeviceSlotComponent::setNodePath(const aidaw::ChainNodePath& path) {
...
void DeviceSlotComponent::saveCurrentMagdaPreset() {
    juce::Component::SafePointer<DeviceSlotComponent> self(this);
    aidaw::daw::ui::saveCurrentMagdaPreset(currentPresetName_,
                                           [self]() -> std::optional<aidaw::DeviceInfo> {
                                               if (self == nullptr)
                                                   return std::nullopt;
                                               return self->snapshotForPreset();
...
void DeviceSlotComponent::loadMagdaPreset(const juce::String& presetRelativePath) {
    juce::Component::SafePointer<DeviceSlotComponent> self(this);
    aidaw::daw::ui::loadMagdaPreset(
        device_.name, nodePath_, presetRelativePath,
        [self](const aidaw::DeviceInfo& liveDevice, const juce::String& presetName) {
            if (self == nullptr)
                return;
...
bool DeviceSlotComponent::hasPluginPresetsAvailable() const {
    return aidaw::daw::ui::hasPluginPresetsAvailable(device_, isInternalDevice());
}
...
void DeviceSlotComponent::showPluginPresetMenu() {
    ...
    aidaw::daw::ui::showPluginPresetMenu(presetsButton_.get(), device_, isInternalDevice(),
                                         currentPluginPresetFile_, std::move(actions));
}

void DeviceSlotComponent::loadPluginPresetFile(const juce::File& file) {
    juce::Component::SafePointer<DeviceSlotComponent> self(this);
    aidaw::daw::ui::loadPluginPresetFile(
...
void DeviceSlotComponent::showSavePluginPresetDialog() {
    juce::Component::SafePointer<DeviceSlotComponent> self(this);
    aidaw::daw::ui::showSavePluginPresetDialog(
...
void DeviceSlotComponent::updateFromDevice(const aidaw::DeviceInfo& device) {
    ...
    if (device.pluginId != device_.pluginId) {
        ...
        if (auto* live = aidaw::TrackManager::getInstance().getDeviceInChainByPath(nodePath_))
    ...
    if (device_.loadState == aidaw::DeviceLoadState::Loaded && !isInternalDevice())
...
void DeviceSlotComponent::updateParamModulation() {
    ...
    if (auto rackPath = nearestRackPathForDevicePath(nodePath_); rackPath.isValid()) {
        if (auto* rack = aidaw::TrackManager::getInstance().getRackByPath(rackPath)) {
            rackMods = &rack->mods;
    ...
    if (nodePath_.trackId != aidaw::INVALID_TRACK_ID) {
        const auto* trackInfo = aidaw::TrackManager::getInstance().getTrack(nodePath_.trackId);
        if (trackInfo) {
    ...
    auto& selMgr = aidaw::SelectionManager::getInstance();
    ...
    if (compiledPanel_) {
        if (auto* audioEngine = aidaw::TrackManager::getInstance().getAudioEngine()) {
            if (auto* bridge = audioEngine->getAudioBridge()) {
...
const aidaw::ModArray* DeviceSlotComponent::getModsData() const {
    if (auto* dev = aidaw::TrackManager::getInstance().getDeviceInChainByPath(nodePath_)) {
        return &dev->mods;
    }
    return nullptr;
...
const aidaw::MacroArray* DeviceSlotComponent::getMacrosData() const {
    if (auto* dev = aidaw::TrackManager::getInstance().getDeviceInChainByPath(nodePath_)) {
        return &dev->macros;
    }
    return nullptr;
...
std::vector<std::pair<aidaw::DeviceId, juce::String>> DeviceSlotComponent::getAvailableDevices()
    const {
    std::vector<std::pair<aidaw::DeviceId, juce::String>> result = {{device_.id, device_.name}};
    drum_grid_slot::appendAvailableDevices(customUI_.getDrumGridUI(), result);
    return result;
}

std::map<aidaw::DeviceId, std::vector<juce::String>> DeviceSlotComponent::getDeviceParamNames()
    const {
    ...
    std::map<aidaw::DeviceId, std::vector<juce::String>> result = {{device_.id, std::move(names)}};
    drum_grid_slot::appendDeviceParamNames(customUI_.getDrumGridUI(), result);
    return result;
}

void DeviceSlotComponent::onModTargetChangedInternal(int modIndex, aidaw::ControlTarget target) {
    aidaw::TrackManager::getInstance().setModTarget(nodePath_, modIndex, target);
...
void DeviceSlotComponent::onModNameChangedInternal(int modIndex, const juce::String& name) {
    aidaw::TrackManager::getInstance().setModName(nodePath_, modIndex, name);
}

void DeviceSlotComponent::onModTypeChangedInternal(int modIndex, aidaw::ModType type) {
    aidaw::TrackManager::getInstance().setModType(nodePath_, modIndex, type);
}

void DeviceSlotComponent::onModWaveformChangedInternal(int modIndex, aidaw::LFOWaveform waveform) {
    aidaw::TrackManager::getInstance().setModWaveform(nodePath_, modIndex, waveform);
}

void DeviceSlotComponent::onModRateChangedInternal(int modIndex, float rate) {
    aidaw::TrackManager::getInstance().setModRate(nodePath_, modIndex, rate);
}

void DeviceSlotComponent::onModPhaseOffsetChangedInternal(int modIndex, float phaseOffset) {
    aidaw::TrackManager::getInstance().setModPhaseOffset(nodePath_, modIndex, phaseOffset);
}

void DeviceSlotComponent::onModTempoSyncChangedInternal(int modIndex, bool tempoSync) {
    aidaw::TrackManager::getInstance().setModTempoSync(nodePath_, modIndex, tempoSync);
}

void DeviceSlotComponent::onModSyncDivisionChangedInternal(int modIndex,
                                                           aidaw::SyncDivision division) {
    aidaw::TrackManager::getInstance().setModSyncDivision(nodePath_, modIndex, division);
}

void DeviceSlotComponent::onModTriggerModeChangedInternal(int modIndex,
                                                          aidaw::LFOTriggerMode mode) {
    aidaw::TrackManager::getInstance().setModTriggerMode(nodePath_, modIndex, mode);
}

void DeviceSlotComponent::onModAudioAttackChangedInternal(int modIndex, float ms) {
    aidaw::TrackManager::getInstance().setModAudioAttack(nodePath_, modIndex, ms);
}

void DeviceSlotComponent::onModAudioReleaseChangedInternal(int modIndex, float ms) {
    aidaw::TrackManager::getInstance().setModAudioRelease(nodePath_, modIndex, ms);
}
...
void DeviceSlotComponent::onModCurveChangedInternal(int /*modIndex*/) {
    ...
    aidaw::TrackManager::getInstance().notifyModCurveChanged(nodePath_);
}

void DeviceSlotComponent::onMacroValueChangedInternal(int macroIndex, float value) {
    aidaw::TrackManager::getInstance().setMacroValue(nodePath_, macroIndex, value);
...
void DeviceSlotComponent::onMacroTargetChangedInternal(int macroIndex,
    ...
    if (activeMacroSelection.isValid() && activeMacroSelection.parentPath == nodePath_) {
        aidaw::TrackManager::getInstance().setMacroTarget(nodePath_, macroIndex, target);
    } else if (activeMacroSelection.isValid()) {
        aidaw::TrackManager::getInstance().setMacroTarget(activeMacroSelection.parentPath,
                                                          macroIndex, target);
    } else {
...
void DeviceSlotComponent::onMacroNameChangedInternal(int macroIndex, const juce::String& name) {
    aidaw::TrackManager::getInstance().setMacroName(nodePath_, macroIndex, name);
}

void DeviceSlotComponent::onMacroAllLinksClearedInternal(int macroIndex) {
    aidaw::TrackManager::getInstance().clearAllMacroLinks(nodePath_, macroIndex);
    updateParamModulation();
    updateMacroPanel();
}
...
void DeviceSlotComponent::onMacroLinkAmountChangedInternal(int macroIndex,
                                                           aidaw::ControlTarget target,
                                                           float amount) {
    aidaw::TrackManager::getInstance().setMacroLinkAmount(nodePath_, macroIndex, target, amount);
    updateParamModulation();
}

void DeviceSlotComponent::onMacroNewLinkCreatedInternal(int macroIndex, aidaw::ControlTarget target,
                                                        float amount) {
    aidaw::TrackManager::getInstance().setMacroTarget(nodePath_, macroIndex, target);
    aidaw::TrackManager::getInstance().setMacroLinkAmount(nodePath_, macroIndex, target, amount);
    updateParamModulation();
    ...
    if (target.isValid())
        aidaw::SelectionManager::getInstance().selectParam(nodePath_, target.paramIndex);
}

void DeviceSlotComponent::onMacroLinkRemovedInternal(int macroIndex, aidaw::ControlTarget target) {
    aidaw::TrackManager::getInstance().removeMacroLink(nodePath_, macroIndex, target);
    updateMacroPanel();
    updateParamModulation();
}
...
void DeviceSlotComponent::onMacroLinkBipolarChangedInternal(int macroIndex,
                                                            aidaw::ControlTarget target,
                                                            bool bipolar) {
    aidaw::TrackManager::getInstance().setMacroLinkBipolar(nodePath_, macroIndex, target, bipolar);
    updateParamModulation();
}

void DeviceSlotComponent::onModClickedInternal(int modIndex) {
    aidaw::SelectionManager::getInstance().selectMod(nodePath_, modIndex);
}

void DeviceSlotComponent::onMacroClickedInternal(int macroIndex) {
    aidaw::SelectionManager::getInstance().selectMacro(nodePath_, macroIndex);
}

void DeviceSlotComponent::onModLinkAmountChangedInternal(int modIndex, aidaw::ControlTarget target,
                                                         float amount) {
    aidaw::TrackManager::getInstance().setModLinkAmount(nodePath_, modIndex, target, amount);
    updateParamModulation();
}

void DeviceSlotComponent::onModNewLinkCreatedInternal(int modIndex, aidaw::ControlTarget target,
                                                      float amount) {
    aidaw::TrackManager::getInstance().setModTarget(nodePath_, modIndex, target);
    aidaw::TrackManager::getInstance().setModLinkAmount(nodePath_, modIndex, target, amount);
    updateParamModulation();
...
void DeviceSlotComponent::onModLinkRemovedInternal(int modIndex, aidaw::ControlTarget target) {
    aidaw::TrackManager::getInstance().removeModLink(nodePath_, modIndex, target);
    updateModsPanel();
    updateParamModulation();
}

void DeviceSlotComponent::onAddModRequestedInternal(int slotIndex, aidaw::ModType type,
                                                    aidaw::LFOWaveform waveform) {
    aidaw::TrackManager::getInstance().addMod(nodePath_, slotIndex, type, waveform);
...
void DeviceSlotComponent::onModRemoveRequestedInternal(int modIndex) {
    aidaw::TrackManager::getInstance().removeMod(nodePath_, modIndex);
    refreshPanels();
}

void DeviceSlotComponent::onModEnableToggledInternal(int modIndex, bool enabled) {
    aidaw::TrackManager::getInstance().setModEnabled(nodePath_, modIndex, enabled);
}
...
void DeviceSlotComponent::onMacroPageAddRequested(int /*itemsToAdd*/) {
    aidaw::TrackManager::getInstance().addMacroPage(nodePath_);
}

void DeviceSlotComponent::onMacroPageRemoveRequested(int /*itemsToRemove*/) {
    aidaw::TrackManager::getInstance().removeMacroPage(nodePath_);
}
        ...
        device_, paramGrid_->getCurrentPage(), [safeThis](int paramIndex, double value) {
            ...
            auto paramIt =
                std::find_if(self->device_.parameters.begin(), self->device_.parameters.end(),
                             [paramIndex](const aidaw::ParameterInfo& param) {
            ...
            aidaw::TrackManager::getInstance().setDeviceParameterValue(self->nodePath_, paramIndex,
...
void DeviceSlotComponent::openMacroPanelForSelectionIfNeeded() {
    if (!aidaw::Config::getInstance().getOpenMacrosOnSelect() || paramPanelVisible_ ||
        !macroButton_ || !nodePath_.isValid()) {
        return;
    }

    const auto& selectedPath = aidaw::SelectionManager::getInstance().getSelectedChainNode();
...
void DeviceSlotComponent::chainNodeSelectionChanged(const aidaw::ChainNodePath& path) {
...
void DeviceSlotComponent::selectionTypeChanged(aidaw::SelectionType newType) {
    ...
    if (newType != aidaw::SelectionType::Param) {
...
void DeviceSlotComponent::modSelectionChanged(const aidaw::ModSelection& selection) {
...
void DeviceSlotComponent::macroSelectionChanged(const aidaw::MacroSelection& selection) {
    // Update param slots to show contextual indicators
    updateParamModulation();
...
void DeviceSlotComponent::paramSelectionChanged(const aidaw::ParamSelection& selection) {
...
void DeviceSlotComponent::showMultiOutMenu() {
    ...
    auto& tm = aidaw::TrackManager::getInstance();
    auto trackId = nodePath_.trackId;
...
void DeviceSlotComponent::showContextMenu() {
    juce::PopupMenu menu;
    auto& selection = aidaw::SelectionManager::getInstance();
    ...
    if (menuDevice.format != aidaw::PluginFormat::Internal) {
        menu.addSeparator();
        juce::PopupMenu classMenu;
        classMenu.addItem(200, "Instrument", menuDevice.deviceType != aidaw::DeviceType::Instrument,
                          menuDevice.deviceType == aidaw::DeviceType::Instrument);
        classMenu.addItem(201, "Effect", menuDevice.deviceType != aidaw::DeviceType::Effect,
                          menuDevice.deviceType == aidaw::DeviceType::Effect);
        classMenu.addItem(202, "MIDI Effect", menuDevice.deviceType != aidaw::DeviceType::MIDI,
                          menuDevice.deviceType == aidaw::DeviceType::MIDI);
    ...
    auto selectedPaths = hasMultiSelection ? selection.getSelectedChainNodes()
                                           : std::vector<aidaw::ChainNodePath>{path};
    ...
    menu.showMenuAsync(juce::PopupMenu::Options(), [safeThis, path, deviceId, callback,
        ...
        if (result == 1) {
            // Add to New Rack
            aidaw::UndoManager::getInstance().executeCommand(
                std::make_unique<aidaw::WrapChainElementsInRackCommand>(selectedPaths));
            ...
            switch (result) {
                case 200:
                    device->deviceType = aidaw::DeviceType::Instrument;
                    device->isInstrument = true;
                    break;
                case 201:
                    device->deviceType = aidaw::DeviceType::Effect;
                    device->isInstrument = false;
                    break;
                case 202:
                    device->deviceType = aidaw::DeviceType::MIDI;
                    device->isInstrument = false;
                    break;
            }
            ...
            juce::MessageManager::callAsync([path, callback]() {
                if (path.topLevelDeviceId != aidaw::INVALID_DEVICE_ID) {
                    aidaw::UndoManager::getInstance().executeCommand(
                        std::make_unique<aidaw::RemoveDeviceFromTrackCommand>(
                            path.trackId, path.topLevelDeviceId));
                } else {
                    aidaw::TrackManager::getInstance().removeDeviceFromChainByPath(path);
                }
                if (callback)
                    callback();
            });
...
void DeviceSlotComponent::createCustomUI() {
    ...
    callbacks.onParameterChanged = [this](int paramIndex, float value) {
        if (!nodePath_.isValid())
            return;
        aidaw::TrackManager::getInstance().setDeviceParameterValue(nodePath_, paramIndex, value);
    };
    ...
    callbacks.onCompiledParamLinkRequested = [this](int paramIndex, float amount) {
        ...
        const auto target = aidaw::ControlTarget::pluginParam(nodePath_, paramIndex);
        amount = juce::jlimit(-1.0f, 1.0f, amount);
        auto& linkMode = aidaw::LinkModeManager::getInstance();

        if (linkMode.getLinkModeType() == aidaw::LinkModeType::Mod) {
            const auto selection = linkMode.getModInLinkMode();
            if (!selection.isValid())
                return;

            const auto ownerPath =
                selection.parentPath.getType() == aidaw::ChainNodeType::Track
                    ? aidaw::ChainNodePath::trackLevel(selection.parentPath.trackId)
                    : selection.parentPath;
            aidaw::TrackManager::getInstance().setModTarget(ownerPath, selection.modIndex, target);
            aidaw::TrackManager::getInstance().setModLinkAmount(ownerPath, selection.modIndex,
                                                                target, amount);
            if (selection.parentPath == nodePath_) {
                ...
                aidaw::SelectionManager::getInstance().selectMod(nodePath_, selection.modIndex);
            }
            updateParamModulation();
            return;
        }

        if (linkMode.getLinkModeType() == aidaw::LinkModeType::Macro) {
            ...
            const auto ownerPath =
                selection.parentPath.getType() == aidaw::ChainNodeType::Track
                    ? aidaw::ChainNodePath::trackLevel(selection.parentPath.trackId)
                    : selection.parentPath;
            aidaw::TrackManager::getInstance().setMacroTarget(ownerPath, selection.macroIndex,
                                                              target);
            aidaw::TrackManager::getInstance().setMacroLinkAmount(ownerPath, selection.macroIndex,
                                                                  target, amount);
            if (selection.parentPath == nodePath_) {
                updateMacroPanel();
    ...
    callbacks.onCompiledParamLinkAmountChanged = [this](int paramIndex, float amount) {
        ...
        if (linkMode.getLinkModeType() == aidaw::LinkModeType::Mod) {
            ...
            aidaw::TrackManager::getInstance().setModLinkAmount(ownerPath, selection.modIndex,
                                                                target, amount);
            updateParamModulation();
        } else if (linkMode.getLinkModeType() == aidaw::LinkModeType::Macro) {
            const auto selection = linkMode.getMacroInLinkMode();
            if (!selection.isValid())
                return;
            const auto ownerPath =
...
void DeviceSlotComponent::wirePadChainLinkCallbacks() {
    ...
    callbacks.getNodePath = [safeThis]() {
        return safeThis != nullptr ? safeThis->nodePath_ : aidaw::ChainNodePath{};
    };
    ...
    callbacks.onMacroTargetChanged = [safeThis](int macroIndex, aidaw::ControlTarget target) {
        if (safeThis != nullptr)
...
void DeviceSlotComponent::setupCustomUILinking() {
    ...
    const aidaw::ModArray* rackMods = nullptr;
    const aidaw::MacroArray* rackMacros = nullptr;
    if (!nodePath_.steps.empty() && nodePath_.steps[0].type == aidaw::ChainStepType::Rack) {
        aidaw::ChainNodePath rackPath;
    ...
    for (int i = 0; i < static_cast<int>(sliders.size()); ++i) {
        ...
        slider->onModUnlinked = [safeThis = juce::Component::SafePointer(this)](
            ...
            if (!self)
                return;
            aidaw::TrackManager::getInstance().removeModLink(self->nodePath_, modIndex, target);
            if (!self)
                return;
            self->updateParamModulation();
            self->updateModsPanel();
        ...
        slider->onModAmountChanged = [safeThis = juce::Component::SafePointer(this)](
                                         int modIndex, aidaw::ControlTarget target, float amount) {
            auto self = safeThis;
            if (!self)
                return;
            auto nodePath = self->nodePath_;
...
void DeviceSlotComponent::showSidechainMenu() {
    ...
    aidaw::SidechainConfig currentSidechain;
    bool canAudio = device_.canSidechain;
    bool canMidi = device_.canReceiveMidi;
    if (auto* currentDevice =
            aidaw::TrackManager::getInstance().getDeviceInChainByPath(nodePath_)) {
    ...
    if (canAudio) {
        ...
        for (const auto& entry : *trackEntries) {
            bool isSelected = currentSidechain.isActive() &&
                              currentSidechain.type == aidaw::SidechainConfig::Type::Audio &&
    ...
    if (canMidi) {
        ...
        for (const auto& entry : *trackEntries) {
            bool isSelected = currentSidechain.isActive() &&
                              currentSidechain.type == aidaw::SidechainConfig::Type::MIDI &&
                              currentSidechain.sourceTrackId == entry.id;
            menu.addItem(itemId, entry.name, true, isSelected);
            ++itemId;
        }
    ...
    menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(scButton_.get()),
                       [deviceId, trackEntries, safeThis](int result) {
                           ...
                           if (result == 1) {
                               aidaw::TrackManager::getInstance().clearSidechain(deviceId);
                           } else if (result >= 100 && result < 200) {
                               // Audio sidechain
                               int index = result - 100;
                               if (index >= 0 && index < static_cast<int>(trackEntries->size())) {
                                   aidaw::TrackManager::getInstance().setSidechainSource(
                                       deviceId, (*trackEntries)[static_cast<size_t>(index)].id,
                                       aidaw::SidechainConfig::Type::Audio);
                               }
                           } else if (result >= 200) {
                               // MIDI sidechain
                               ...
                               if (index >= 0 && index < static_cast<int>(trackEntries->size())) {
                                   aidaw::TrackManager::getInstance().setSidechainSource(
                                       deviceId, (*trackEntries)[static_cast<size_t>(index)].id,
                                       aidaw::SidechainConfig::Type::MIDI);
                               }
                           }
                           ...
                           if (safeThis) {
                               if (auto* dev =
                                       aidaw::TrackManager::getInstance().getDeviceInChainByPath(
                                           safeThis->nodePath_)) {
...
void DeviceSlotComponent::updateScButtonState() {
    ...
    if (device_.sidechain.isActive()) {
        juce::String label =
            device_.sidechain.type == aidaw::SidechainConfig::Type::MIDI ? "MI" : "SC";
