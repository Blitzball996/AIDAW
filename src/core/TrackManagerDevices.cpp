...
void remapPresetLinksRecursive(std::vector<ChainElement>& elements, const PresetIdRemap& remap) {
    for (auto& element : elements) {
        if (aidaw::isDevice(element)) {
            auto& device = aidaw::getDevice(element);
            ...
            remapRackPresetLinks(aidaw::getRack(element), remap);
        }
    }
}
...
void TrackManager::removeDeviceFromChain(TrackId trackId, RackId rackId, ChainId chainId,
    ...
    if (auto* chain = getChain(trackId, rackId, chainId)) {
        ...
        auto it = std::find_if(elements.begin(), elements.end(), [deviceId](const ChainElement& e) {
            return aidaw::isDevice(e) && aidaw::getDevice(e).id == deviceId;
        });
        if (it != elements.end()) {
            DBG("Removed device: " << aidaw::getDevice(*it).name << " (id=" << deviceId
                                   << ") from chain " << chainId);
    ...
    if (auto* chain = getChain(trackId, rackId, chainId)) {
        for (auto& element : chain->elements) {
            if (aidaw::isDevice(element) && aidaw::getDevice(element).id == deviceId) {
                return &aidaw::getDevice(element);
            }
        }
    }
    return nullptr;
...
static void collectMovedDevicePaths(const ChainElement& element, const ChainNodePath& elementPath,
    ...
    if (aidaw::isDevice(element)) {
        movedPaths[aidaw::getDevice(element).id] = elementPath;
        return;
    }
    ...
    for (const auto& chain : rack.chains) {
        ...
        for (const auto& child : chain.elements) {
            if (aidaw::isDevice(child)) {
                collectMovedDevicePaths(child, chainPath.withDevice(aidaw::getDevice(child).id),
                                        movedPaths);
            } else if (aidaw::isRack(child)) {
                collectMovedDevicePaths(child, chainPath.withRack(aidaw::getRack(child).id),
                                        movedPaths);
            }
        }
    }
...
static void retargetLinksInElements(std::vector<ChainElement>& elements,
    ...
    for (auto& element : elements) {
        if (aidaw::isDevice(element)) {
            ...
            auto& rack = aidaw::getRack(element);
...
static ChainNodePath getInsertedElementPath(const ChainNodePath& destinationChainPath,
    ...
    if (aidaw::isDevice(element)) {
        const auto deviceId = aidaw::getDevice(element).id;
    ...
    return destinationChainPath.withRack(aidaw::getRack(element).id);
}
...
static bool elementContainsInstrument(const ChainElement& element) {
    if (aidaw::isDevice(element))
        return aidaw::getDevice(element).isInstrument;
...
static void logMoveElementState(const ChainElement& element, const juce::String& label) {
    if (aidaw::isDevice(element)) {
        logMoveDeviceState(aidaw::getDevice(element), label);
        return;
    }
    ...
    auto sourceIt = std::find_if(
        ...
        [sourceType, sourceId](const ChainElement& element) {
            if (sourceType == ChainStepType::Device)
                return aidaw::isDevice(element) && aidaw::getDevice(element).id == sourceId;
            return aidaw::isRack(element) && aidaw::getRack(element).id == sourceId;
        });
    ...
    for (const auto& path : uniquePaths) {
        ...
        auto it = std::find_if(elements->begin(), elements->end(), [type, id](const auto& element) {
            if (type == ChainStepType::Device)
                return aidaw::isDevice(element) && aidaw::getDevice(element).id == id;
            return aidaw::isRack(element) && aidaw::getRack(element).id == id;
        });
        if (it != elements->end())
...
RackId TrackManager::wrapChainElementsInRack(const std::vector<ChainNodePath>& paths,
    ...
    collectMovedDevicePaths(insertedRack, sourceChainPath.withRack(aidaw::getRack(insertedRack).id),
                            movedPaths);
    if (auto* track = getTrack(sourceChainPath.trackId))
        retargetMovedLinksInTrack(*track, movedPaths);

    notifyTrackDevicesChanged(sourceChainPath.trackId);
    return aidaw::getRack((*sourceElements)[static_cast<size_t>(insertIndex)]).id;
}
...
int TrackManager::getChainElementIndex(const ChainNodePath& elementPath) {
    ...
    for (int i = 0; i < static_cast<int>(elements->size()); ++i) {
        ...
        if (sourceType == ChainStepType::Device) {
            if (isDevice(element) && aidaw::getDevice(element).id == sourceId)
                return i;
        } else if (isRack(element) && aidaw::getRack(element).id == sourceId) {
            return i;
        }
    }
...
void TrackManager::removeDeviceFromChainByPath(const ChainNodePath& devicePath) {
    ...
    if (devicePath.topLevelDeviceId != INVALID_DEVICE_ID) {
        ...
        auto it =
            std::find_if(elements.begin(), elements.end(), [&devicePath](const ChainElement& e) {
                return aidaw::isDevice(e) && aidaw::getDevice(e).id == devicePath.topLevelDeviceId;
            });
        if (it != elements.end()) {
            DBG("Removed top-level device: " << aidaw::getDevice(*it).name
    ...
    if (auto* chain = getChainFromPath(*this, chainPath)) {
        ...
        if (it != elements.end()) {
            DBG("Removed nested device via path: " << aidaw::getDevice(*it).name
                                                   << " (id=" << deviceId << ")");
            SelectionManager::getInstance().clearSelectionForDeletedChainNode(devicePath);
            elements.erase(it);
            notifyTrackDevicesChanged(devicePath.trackId);
...
DeviceInfo* TrackManager::getDeviceInChainByPath(const ChainNodePath& devicePath) {
    ...
    if (devicePath.topLevelDeviceId != INVALID_DEVICE_ID) {
        ...
        for (auto& element : track->chainElements) {
            if (aidaw::isDevice(element) &&
                aidaw::getDevice(element).id == devicePath.topLevelDeviceId) {
                return &aidaw::getDevice(element);
            }
        }
        return nullptr;
...
ChainNodePath TrackManager::findDevicePath(DeviceId deviceId) const {
    ...
    for (const auto& track : tracks_) {
        for (const auto& element : track.chainElements) {
            ...
            if (aidaw::isRack(element)) {
                const auto& rack = aidaw::getRack(element);
                for (const auto& chain : rack.chains) {
                    for (const auto& chainElement : chain.elements) {
                        if (aidaw::isDevice(chainElement) &&
                            aidaw::getDevice(chainElement).id == deviceId)
...
void TrackManager::updateDeviceParameters(DeviceId deviceId,
    ...
    for (auto& element : masterTrack_.chainElements) {
        if (aidaw::isDevice(element) && aidaw::getDevice(element).id == deviceId) {
            aidaw::getDevice(element).parameters = params;
            return;
        }
    }
    ...
    for (auto& track : tracks_) {
        for (auto& element : track.chainElements) {
            ...
            if (aidaw::isRack(element)) {
                for (auto& chain : aidaw::getRack(element).chains) {
                    for (auto& chainElement : chain.elements) {
                        if (aidaw::isDevice(chainElement) &&
                            aidaw::getDevice(chainElement).id == deviceId) {
                            aidaw::getDevice(chainElement).parameters = params;
...
void TrackManager::setDeviceVisibleParameters(DeviceId deviceId,
    ...
    for (auto& element : masterTrack_.chainElements) {
        if (aidaw::isDevice(element) && aidaw::getDevice(element).id == deviceId) {
            aidaw::getDevice(element).visibleParameters = visibleParams;
            return;
        }
    }
            ...
            if (aidaw::isRack(element)) {
                for (auto& chain : aidaw::getRack(element).chains) {
                    for (auto& chainElement : chain.elements) {
                        if (aidaw::isDevice(chainElement) &&
                            aidaw::getDevice(chainElement).id == deviceId) {
                            aidaw::getDevice(chainElement).visibleParameters = visibleParams;
                            return;
                        }
                    }
                }
    ...
    reassignIds = [&](std::vector<ChainElement>& elements) {
        for (auto& element : elements) {
            if (aidaw::isDevice(element)) {
                ...
                auto& nested = aidaw::getRack(element);
...
double TrackManager::getTrackLatencySeconds(TrackId trackId) {
    ...
    for (const auto& element : track->chainElements) {
        if (aidaw::isDevice(element)) {
            total += getDeviceLatency(aidaw::getDevice(element));
        } else if (aidaw::isRack(element)) {
            ...
            for (const auto& chain : rack.chains) {
                ...
                for (const auto& chainElem : chain.elements) {
                    if (aidaw::isDevice(chainElem))
                        chainLatency += getDeviceLatency(aidaw::getDevice(chainElem));
                }
...
RackId TrackManager::wrapDeviceInRack(TrackId trackId, DeviceId deviceId,
    ...
    // Extract the device
    DeviceInfo extractedDevice = aidaw::getDevice(*it);
    elements.erase(it);
...
void TrackManager::removeRackFromChain(TrackId trackId, RackId parentRackId, ChainId chainId,
    ...
    if (auto* chain = getChain(trackId, parentRackId, chainId)) {
        ...
        for (auto it = elements.begin(); it != elements.end(); ++it) {
            if (aidaw::isRack(*it)) {
                DBG("    checking rack element id=" << aidaw::getRack(*it).id);
                if (aidaw::getRack(*it).id == nestedRackId) {
                    elements.erase(it);
                    notifyTrackDevicesChanged(trackId);
