    ...
    logElements = [&](const std::vector<ChainElement>& elements, int depth) {
        for (const auto& element : elements) {
            ...
            for (int d = 0; d < depth; ++d)
                indent += "  ";
            if (aidaw::isDevice(element)) {
                const auto& dev = aidaw::getDevice(element);
                ...
                for (auto& chain : rack.chains) {
                    for (auto& chainElement : chain.elements) {
                        if (aidaw::isDevice(chainElement)) {
                            auto& device = aidaw::getDevice(chainElement);
                            device.bypassed = bypassed;
                            affectedDevices.push_back(device.id);
                        }
                    }
...
void TrackManager::removeRackFromTrack(TrackId trackId, RackId rackId) {
    if (auto* track = getTrack(trackId)) {
        ...
        auto it = std::find_if(elements.begin(), elements.end(), [rackId](const ChainElement& e) {
            return aidaw::isRack(e) && aidaw::getRack(e).id == rackId;
        });
        if (it != elements.end()) {
            DBG("Removed rack: " << aidaw::getRack(*it).name << " (id=" << rackId << ") from track "
                                 << trackId);
            elements.erase(it);
            notifyTrackDevicesChanged(trackId);
        }
...
RackInfo* TrackManager::getRack(TrackId trackId, RackId rackId) {
    if (auto* track = getTrack(trackId)) {
        for (auto& element : track->chainElements) {
            if (aidaw::isRack(element) && aidaw::getRack(element).id == rackId) {
                return &aidaw::getRack(element);
            }
        }
    }
    return nullptr;
                ...
                if (currentChain == nullptr) {
                    ...
                    for (auto& element : track->chainElements) {
                        if (aidaw::isRack(element)) {
                            if (aidaw::getRack(element).id == step.id) {
                                currentRack = &aidaw::getRack(element);
                                break;
                            }
                        }
                    }
...
TrackManager::ResolvedPath TrackManager::resolvePath(const ChainNodePath& path) const {
    ...
    if (path.topLevelDeviceId != INVALID_DEVICE_ID) {
        for (const auto& element : track->chainElements) {
            if (aidaw::isDevice(element) && aidaw::getDevice(element).id == path.topLevelDeviceId) {
                result.valid = true;
                result.device = &aidaw::getDevice(element);
        ...
        switch (step.type) {
            case ChainStepType::Rack: {
                if (currentChain == nullptr) {
                    // Top-level rack in track's chainElements
                    for (const auto& element : track->chainElements) {
                        if (aidaw::isRack(element) && aidaw::getRack(element).id == step.id) {
                            currentRack = &aidaw::getRack(element);
            ...
            case ChainStepType::Device: {
                if (currentChain != nullptr) {
                    for (const auto& element : currentChain->elements) {
                        if (aidaw::isDevice(element) && aidaw::getDevice(element).id == step.id) {
                            result.device = &aidaw::getDevice(element);
