...
void applyRackInstanceState(te::Plugin::Ptr rackPlugin, const RackInfo& rackInfo) {
    ...
    rackInstance->leftOutDb->setParameter(leftDb, juce::dontSendNotification);
    rackInstance->rightOutDb->setParameter(rightDb, juce::dontSendNotification);
}
...
void RackSyncManager::ungateAllLFOs() {
    // setGated not in upstream TE — no-op
}

void RackSyncManager::regateTriggeredLFOs() {
    // setGated/getSkipNativeResync not in upstream TE — no-op
}
